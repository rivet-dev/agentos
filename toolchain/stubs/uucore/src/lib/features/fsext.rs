...
pub fn read_fs_list() -> UResult<Vec<MountInfo>> {
    ...
    {
        // No method to read mounts on this platform
        Ok(Vec::new())
    }
}
...
// ============================================================
// wasmVM: WASI stubs for filesystem metadata types
// ============================================================

/// WASI StatFs placeholder — WASI VFS doesn't have statfs
#[cfg(target_os = "wasi")]
...
#[cfg(target_os = "wasi")]
impl FsMeta for StatFs {
    fn fs_type(&self) -> i64 { self.f_type }
    fn io_size(&self) -> u64 { self.f_bsize }
    fn block_size(&self) -> i64 { self.f_bsize as i64 }
    fn total_blocks(&self) -> u64 { self.f_blocks }
    fn free_blocks(&self) -> u64 { self.f_bfree }
    fn avail_blocks(&self) -> u64 { self.f_bavail }
    fn total_file_nodes(&self) -> u64 { self.f_files }
    fn free_file_nodes(&self) -> u64 { self.f_ffree }
    fn fsid(&self) -> u64 { self.f_fsid }
    fn namelen(&self) -> u64 { self.f_namelen }
}

#[cfg(target_os = "wasi")]
pub fn statfs(_path: &std::ffi::OsStr) -> Result<StatFs, String> {
    // WASI VFS: return synthetic data
    Ok(StatFs {
        f_type: 0x5345_4F53, // "wasmVM"
        f_bsize: 4096,
        f_blocks: 1024 * 1024,
        f_bfree: 512 * 1024,
        f_bavail: 512 * 1024,
...
pub fn pretty_filetype(mode: u32, size: u64) -> String {
    use crate::features::fs::wasi_mode_consts::*;
    match mode & S_IFMT {
        v if v == S_IFREG => {
            if size == 0 { "regular empty file" } else { "regular file" }
        }
        v if v == S_IFDIR => "directory",
        v if v == S_IFLNK => "symbolic link",
        v if v == S_IFCHR => "character special file",
        v if v == S_IFBLK => "block special file",
        v if v == S_IFIFO => "fifo",
        v if v == S_IFSOCK => "socket",
        _ => return format!("weird file ({:07o})", mode & S_IFMT),
    }
    .to_owned()
}
