 ...
 *
 * This MUST be a no-op here. OpenSSH's ssh.c:main() calls
 * closefrom(STDERR_FILENO + 1) unconditionally at startup to drop leaked
 * descriptors before doing anything else. On WASI, however, the preopened
 * directory capability handles handed to the module by the runtime start at
 * fd 3 (see WASI preview1 `preopens`; wasi-libc
 * libc-bottom-half/sources/preopens.c populates its preopen table from
 * fd_prestat_get on fds >= 3). Closing fds 3+ would discard every preopen
 * and with them all path resolution — every subsequent open() fails with
 * ENOTCAPABLE. A faithful "close everything" implementation is therefore
 * strictly worse than doing nothing: guest processes in this runtime are
 * spawned with exactly the descriptors the kernel intends them to have, so
 * there are no inherited stray fds to drop and the security intent of the
 * call is already satisfied by construction.
 *
 * Providing the symbol also makes OpenSSH's configure AC_CHECK_FUNCS link
 * probe succeed (HAVE_CLOSEFROM), which keeps its openbsd-compat
 * bsd-closefrom.c replacement — a close() loop up to sysconf(_SC_OPEN_MAX)
 * that would destroy the preopens — out of the build entirely.
 */
