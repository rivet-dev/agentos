...
// Virtual/system filesystems whose `stat` hangs the VM sidecar — `stat`-ing
// these (e.g. when listing "/") wedges the whole actor, so we treat them as
// directories without touching them and never auto-expand them.
const VIRTUAL_DIRS = new Set<string>([
	// "/proc",
	// "/sys",
	// "/dev",
	// "/__secure_exec",
				...
				const entries = raw
					...
					.map((e): FsEntry => {
						...
						return {
							...
							path: p,
							dir: e.isDirectory,
							virtual: VIRTUAL_DIRS.has(p) ? true : undefined,
						};
					});
				return entries.sort(
