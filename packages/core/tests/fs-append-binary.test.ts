...
// appendFile* used to read the existing file as a UTF-8 string and stringify the
// appended data, so every non-UTF-8 byte already on disk was rewritten as U+FFFD
// and a plain Uint8Array was appended as its comma-joined decimal text. Linux
// appends the raw bytes. vm.readFile reads through the sidecar rather than the
