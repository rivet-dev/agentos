...
function skipReason(): string | false {
	...
	if (!existsSync(resolve(COMMANDS_DIR, "spawn-test-host")))
		return "spawn-test-host binary not built";
	return false;
}
...
class SimpleVFS {
	...
	async exists(path: string): Promise<boolean> {
		return (
			this.files.has(path) || this.dirs.has(path) || this.symlinks.has(path)
		);
	}
...
describeIf(
	!skipReason(),
	"wasi-spawn: WasiChild host_process integration",
	{ timeout: 60_000 },
	() => {
		let kernel: Kernel;
		let vfs: SimpleVFS;
		...
		it("spawns through the Tokio process bridge", async () => {
