...
// The CLI command path (`agentos-<kit> <binding>`) is gated by `toolPermissionMode`
...
describe("binding group permissions", () => {
	...
	test("rejects duplicate binding group registration with a conflict", async () => {
		await expect(
			AgentOs.create({
				bindingGroups: [mathBindingGroup, duplicateMathBindingGroup],
			}),
		).rejects.toThrow(/conflict: binding group already registered: math/);
	});

	test("allows binding group invocation with default permissions", async () => {
	...
	test("denies binding group invocation by default until binding permissions are granted", async () => {
		vm = await AgentOs.create({
			software: [common],
			bindingGroups: [mathBindingGroup],
			permissions: {
	...
	test("allows binding group invocation when a matching binding permission is granted", async () => {
		vm = await AgentOs.create({
			software: [common],
			bindingGroups: [mathBindingGroup],
			permissions: {
...
describe("binding group permissions — raw host_callback RPC path", () => {
	let vm: AgentOs | null = null;
