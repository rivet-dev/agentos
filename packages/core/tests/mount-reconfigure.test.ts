...
function createProxy(client: unknown) {
	const options = {
		...
		hostFunctionShimCommands: bootBindingShims,
...
describe("post-boot mount reconfiguration", () => {
	it("resends the boot packages and hostFunction shims on runtime mountFs", async () => {
		...
		expect(payload.hostFunctionShimCommands).toEqual(bootBindingShims);
		...
		expect(configureCalls[1].hostFunctionShimCommands).toEqual(bootBindingShims);
