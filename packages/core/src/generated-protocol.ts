...
function read40(bc: bare.ByteCursor): readonly ProcessOutputReplayEvent[] {
    const len = bare.readUintSafe(bc)
    if (len === 0) {
        return []
    }
...
function write40(bc: bare.ByteCursor, x: readonly ProcessOutputReplayEvent[]): void {
    bare.writeUintSafe(bc, x.length)
    for (let i = 0; i < x.length; i++) {
...
export function readResourceSnapshotResponse(bc: bare.ByteCursor): ResourceSnapshotResponse {
    return {
        ...
        wasmtimeProcessRetainedRssBytes: read13(bc),
...
export function writeResourceSnapshotResponse(bc: bare.ByteCursor, x: ResourceSnapshotResponse): void {
    ...
    write13(bc, x.wasmtimeProcessRetainedRssBytes)
