...
import type { Binding, BindingGroup } from "./host-bindings.js";
...
export interface AgentOsSandboxCommonOptions {
	/** Mount path inside the agentOS VM. Defaults to "/sandbox". */
	mountPath?: string;
...
export async function resolveSandboxOptions<T extends { sandbox?: AgentOsSandboxInput }>(
	...
	const mountPath = sandboxOptions.mountPath ?? "/sandbox";
	const mounts = [
		...(expanded.mounts ?? []),
		{
			path: mountPath,
