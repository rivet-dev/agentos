...
const CHILD_PROCESS_EVENT_ROUTES = Symbol.for(
	"agentos.childProcessEventRoutes",
);
...
function createIpcSerializationLimitError(limit) {
	const error = new Error(
		`ERR_RESOURCE_BUDGET_EXCEEDED: advanced child_process IPC message exceeds ${limit}`,
	);
...
function encodeAdvancedIpcMessage(message) {
	...
	function encode(value, depth) {
		if (depth > CHILD_PROCESS_IPC_MAX_GRAPH_DEPTH) {
			throw createIpcSerializationLimitError(
				`maximum graph depth ${CHILD_PROCESS_IPC_MAX_GRAPH_DEPTH}`,
			);
		}
		if (
			value === null ||
		...
		if (typeof value === "bigint")
			return { $t: "bigint", value: String(value) };
		if (typeof value === "function" || typeof value === "symbol") {
			const error = new TypeError(
				`${typeof value} could not be cloned by advanced child_process IPC`,
			);
		...
		if (nodes.length >= CHILD_PROCESS_IPC_MAX_GRAPH_NODES) {
			throw createIpcSerializationLimitError(
				`maximum graph nodes ${CHILD_PROCESS_IPC_MAX_GRAPH_NODES}`,
			);
		}
		...
		if (Array.isArray(value)) {
			node = {
				type: "array",
				values: value.map((entry) => encode(entry, depth + 1)),
			};
			...
			node = {
				type: "map",
				values: Array.from(value, ([key, entry]) => [
					encode(key, depth + 1),
			...
			node = {
				type: "set",
				values: Array.from(value, (entry) => encode(entry, depth + 1)),
			};
	...
	return JSON.stringify({
		__agentOSAdvancedIpc: 1,
		root: encode(message, 0),
		nodes,
	});
}
function decodeAdvancedIpcMessage(envelope) {
	if (
		...
		envelope.nodes.length > CHILD_PROCESS_IPC_MAX_GRAPH_NODES
	) {
		throw createIpcSerializationLimitError(
			`maximum graph nodes ${CHILD_PROCESS_IPC_MAX_GRAPH_NODES}`,
		);
		...
		switch (node?.type) {
			...
			case "buffer":
				return typeof Buffer !== "undefined"
					? Buffer.from(node.value, "base64")
					: Uint8Array.from(atob(node.value), (character) =>
			...
			case "typedarray": {
				...
				return typeof ctor === "function" && ctor.BYTES_PER_ELEMENT
					? new ctor(buffer)
	...
	function decode(token, depth) {
		...
		if (Object.hasOwn(token, "$r")) {
			if (
				!Number.isInteger(token.$r) ||
				token.$r < 0 ||
				token.$r >= shells.length
			) {
	...
	for (let index = 0; index < envelope.nodes.length; index += 1) {
		...
		if (node.type === "array") {
			...
			for (const [key, value] of node.values || [])
				shell[key] = decode(value, 1);
		} else if (node.type === "map") {
			for (const [key, value] of node.values)
				shell.set(decode(key, 1), decode(value, 1));
...
function encodeChildProcessIpcFrame(message, serialization) {
	...
	const json =
		mode === "advanced"
			? encodeAdvancedIpcMessage(message)
...
function decodeChildProcessIpcFramePayload(payload) {
	...
	return parsed?.__agentOSAdvancedIpc === 1
		? decodeAdvancedIpcMessage(parsed)
		: parsed;
}
function splitChildProcessIpcFrames(buffer, chunk) {
	...
	while (true) {
		...
		try {
			messages.push(
				decodeChildProcessIpcFramePayload(text.slice(payloadStart, frameEnd)),
			);
		} catch (error) {
		...
		const bytes =
			typeof output === "string"
				? typeof Buffer !== "undefined"
					? Buffer.from(output)
					: output
				: typeof Buffer !== "undefined" && Buffer.isBuffer(output)
					? output
					: typeof Buffer !== "undefined"
						? Buffer.from(output)
						: output;
...
function routeChildProcessEvent(sessionId, type, data) {
	...
	if (!child) {
		...
		if (!events) {
			if (earlyChildProcessEvents.size >= MAX_EARLY_CHILD_PROCESS_IDS) {
				earlyChildProcessEvents.delete(
					earlyChildProcessEvents.keys().next().value,
				);
			}
			events = [];
	...
	if (type === "stdout") {
		...
		if (child._ipcEnabled) {
			...
			const outBuf =
				typeof Buffer !== "undefined"
					? Buffer.from(parsed.output, "utf8")
					: parsed.output;
			if (
				writeChildOutputToInheritedFd(
					child._stdoutFd,
					outBuf,
					child._stdoutPath,
				)
		...
		if (writeChildOutputToInheritedFd(child._stdoutFd, buf, child._stdoutPath))
			return;
		child.stdout.emit("data", buf);
	} else if (type === "stderr") {
		const buf = typeof Buffer !== "undefined" ? Buffer.from(data) : data;
		if (writeChildOutputToInheritedFd(child._stderrFd, buf, child._stderrPath))
			return;
		child.stderr.emit("data", buf);
	} else if (type === "exit") {
		const signalCode =
			data && typeof data === "object" ? (data.signal ?? null) : null;
		...
		const finalizeExit = () => {
			...
			const publishExit = () => {
				...
				const waitingForLateReadableConsumer = [
					child.stdout,
					child.stderr,
				].some(
					(stream) =>
...
var childProcessDispatch = (eventTypeOrSessionId, payloadOrType, data) => {
	...
	if (
		eventTypeOrSessionId === "child_stdout" ||
		eventTypeOrSessionId === "child_stderr"
	) {
		...
		if (typeof Buffer !== "undefined" && Buffer.isBuffer(directData)) {
			...
			bytes =
				typeof Buffer !== "undefined"
					? Buffer.from(
							directData.buffer,
							directData.byteOffset,
							directData.byteLength,
						)
					: directData;
				...
				typeof payload?.dataBase64 === "string"
					...
					: typeof directData === "string"
						...
						: directData?.__agentOSType === "bytes" &&
								typeof directData?.base64 === "string"
							? directData.base64
							: "";
			bytes =
				typeof Buffer !== "undefined"
...
function scheduleOutputFlush(stream) {
	...
	queueMicrotask(() => {
		...
		if (
			stream._bufferedChunks.length > 0 &&
			hasOutputListeners(stream, "data")
		) {
			const chunks = stream._bufferedChunks.splice(
				0,
				stream._bufferedChunks.length,
			);
...
function readBufferedOutputChunk(stream, size) {
	...
	if (Number.isInteger(size) && size > 0 && chunk.length > size) {
		...
		const tail =
			typeof chunk === "string" ? chunk.slice(size) : chunk.subarray(size);
		stream._bufferedChunks.unshift(tail);
		return head;
	}
	if (stream._ended && stream._bufferedChunks.length === 0)
...
function checkStreamMaxListeners(stream, event) {
	...
	if (stream._maxListeners > 0 && !stream._maxListenersWarned.has(event)) {
		const total =
			(stream._listeners[event]?.length ?? 0) +
			(stream._onceListeners[event]?.length ?? 0);
...
var ChildProcess = class {
	...
	constructor() {
		...
		this.stdout = {
			...
			setEncoding(encoding) {
				this._readableEncoding =
					encoding == null || encoding === "buffer" ? null : String(encoding);
				return this;
			},
			setMaxListeners(n) {
	...
	_hasIpcMessageListeners() {
		return (
			(this._listeners.message?.length ?? 0) > 0 ||
			(this._onceListeners.message?.length ?? 0) > 0
		);
	}
	...
	_flushQueuedIpcMessages() {
		...
		queueMicrotask(() => {
			while (
				this._ipcQueuedMessages.length > 0 &&
				this._hasIpcMessageListeners()
			) {
	...
	send(message, sendHandleOrOptions, optionsOrCallback, maybeCallback) {
		...
		try {
			...
			if (!this._ipcReady) {
				if (
					this._ipcOutboundQueue.length >= 1024 ||
					this._ipcOutboundBytes + frame.length > 8 * 1024 * 1024
				) {
					const error = new Error(
						"ERR_RESOURCE_BUDGET_EXCEEDED: pre-ready child_process IPC queue exceeds 1024 messages or 8388608 bytes; wait for the child spawn/IPC channel before sending more data",
					);
					error.code = "ERR_RESOURCE_BUDGET_EXCEEDED";
...
function execSync(command, options) {
	...
	if (typeof _childProcessSpawnSync === "undefined") {
		throw new Error(
			"child_process.execSync requires CommandExecutor to be configured",
		);
	}
	const effectiveCwd =
		opts.cwd ?? (typeof process !== "undefined" ? process.cwd() : "/");
		...
		JSON.stringify({
			...
			timeout:
				Number.isInteger(opts.timeout) && opts.timeout > 0
					...
					: null,
			killSignal:
				normalizeChildProcessSignal(opts.killSignal).signalCode ?? "SIGTERM",
		}),
	]);
	const result =
		typeof jsonResult === "string" ? JSON.parse(jsonResult) : jsonResult;
	result.stdout = normalizeSyncOutput(result.stdout, opts.encoding);
	result.stderr = normalizeSyncOutput(result.stderr, opts.encoding);
	const execSyncStdio = Array.isArray(opts.stdio)
		? opts.stdio
		: opts.stdio === "inherit"
	...
	if (result.timedOut) {
		...
		err.code = "ETIMEDOUT";
		err.status =
			result.signal == null && typeof result.code === "number"
				? result.code
				: null;
...
function childProcessBridgeStdio(stdio) {
	return normalizeChildProcessStdio(stdio).map((mode) =>
		typeof mode === "string" ? mode : "pipe",
	);
}
function normalizeSyncOutput(value, encoding) {
...
function spawn(command, args, options) {
	...
	if (typeof _childProcessSpawnStart !== "undefined") {
		...
		try {
			const effectiveCwd =
				opts.cwd ?? (typeof process !== "undefined" ? process.cwd() : "/");
			spawnResult = normalizeChildProcessBridgePayload(
				_childProcessSpawnStart.applySync(void 0, [
					command,
					...
					JSON.stringify({
						...
						pty:
							opts.agentosPty && typeof opts.agentosPty === "object"
								? {
										cols:
											Number.isInteger(opts.agentosPty.cols) &&
											opts.agentosPty.cols > 0
												? opts.agentosPty.cols
												: 80,
										rows:
											Number.isInteger(opts.agentosPty.rows) &&
											opts.agentosPty.rows > 0
												? opts.agentosPty.rows
												: 24,
									}
								: opts.agentosPty === true
									? { cols: 80, rows: 24 }
									: null,
			...
			if (
				...
				spawnError.code == null &&
				/ERR_NATIVE_BINARY_NOT_SUPPORTED\b/i.test(
		...
		const sessionId =
			typeof spawnResult === "object" && spawnResult !== null
				? spawnResult.childId
				: spawnResult;
		...
		child.kill = (signal) => {
			...
			_childProcessKill.applySync(void 0, [
				sessionId,
				normalizedSignal.bridgeSignal,
			]);
			child.killed = true;
...
function spawnSync(command, args, options) {
	...
	if (typeof _childProcessSpawnSync === "undefined") {
		return {
			...
			output: [
				null,
				"",
				"child_process.spawnSync requires CommandExecutor to be configured",
			],
			stdout: "",
			stderr:
				"child_process.spawnSync requires CommandExecutor to be configured",
	...
	try {
		...
		const timeout =
			Number.isInteger(opts.timeout) && opts.timeout > 0 ? opts.timeout : null;
		const killSignal =
			normalizeChildProcessSignal(opts.killSignal).signalCode ?? "SIGTERM";
		const jsonResult = _childProcessSpawnSync.applySyncPromise(void 0, [
			command,
		...
		const result =
			typeof jsonResult === "string" ? JSON.parse(jsonResult) : jsonResult;
		const spawnSyncStdio = Array.isArray(opts.stdio)
			? opts.stdio
			: opts.stdio === "inherit"
				? ["inherit", "inherit", "inherit"]
				: [];
		let stdoutValue = normalizeSyncOutput(result.stdout, opts.encoding);
		let stderrValue = normalizeSyncOutput(result.stderr, opts.encoding);
		...
		if (redirectSyncOutputToInheritedFd(spawnSyncStdio[1], stdoutValue)) {
			stdoutValue =
				useBufferOutput && typeof Buffer !== "undefined" ? Buffer.from("") : "";
		}
		...
		if (result.timedOut) {
			...
			return {
				...
				stderr: stderrValue,
				status:
					typeof result.code === "number" && result.signal == null
						? result.code
						: null,
		...
		const stdoutValue =
			useBufferOutput && typeof Buffer !== "undefined" ? Buffer.from("") : "";
		const stderrValue =
			useBufferOutput && typeof Buffer !== "undefined"
				? Buffer.from(errMsg)
				: errMsg;
		return {
			pid: _nextChildPid++,
...
function execFileSync(file, args, options) {
	...
	if (
		result.error &&
		String(result.error.code) === "ERR_CHILD_PROCESS_STDIO_MAXBUFFER"
	) {
		throw result.error;
	}
	if (result.status !== 0) {
	...
	return typeof result.stdout === "string"
		? result.stdout
		: result.stdout.toString(opts.encoding);
}
function fork(modulePath, args, options) {
	...
	for (let index = 0; index < execArgv.length; index += 1) {
		...
		if (argument === "--require" || argument === "-r") {
			if (index + 1 < execArgv.length)
				preloadModules.push(String(execArgv[++index]));
	...
	const env = {
		...
		...(opts.env || {}),
		AGENTOS_NODE_IPC: "1",
		AGENTOS_NODE_IPC_SERIALIZATION:
			opts.serialization === "advanced" ? "advanced" : "json",
		AGENTOS_NODE_EXEC_ARGV: JSON.stringify(execArgv.map(String)),
		AGENTOS_NODE_PRELOAD_MODULES: JSON.stringify(preloadModules),
	};
	const child = spawn(
		opts.execPath ||
			(typeof process !== "undefined" ? process.execPath : "node"),
		[modulePath, ...argsArray],
		{
			...opts,
...
export {
	_childInheritedFds,
	_nextChildPid,
	CHILD_PROCESS_IPC_FRAME_PREFIX,
	ChildProcess,
	checkStreamMaxListeners,
	child_process_default,
	child_process_exports,
	childProcess,
	childProcessDispatch,
	childProcessInstances,
	createOutputAsyncIterator,
	decodeChildProcessIpcFramePayload,
	decodeOutputChunk,
	deferCloseIfChildInheritedFd,
	...
	fork,
	hasOutputListeners,
	normalizeChildProcessBridgePayload,
	normalizeChildProcessSessionId,
	redirectSyncOutputToInheritedFd,
	releaseChildInheritedFd,
	retainChildInheritedFd,
	routeChildProcessEvent,
	scheduleOutputFlush,
	spawn,
	spawnSync,
