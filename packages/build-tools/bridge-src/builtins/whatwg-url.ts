...
import { resolveRelativeNonFileUrl } from "./url-resolution.js";

var inspectCustomSymbol = /* @__PURE__ */ Symbol.for(
...
var kLinkedSearchParams = /* @__PURE__ */ Symbol(
...
function decodeFormUrlencodedComponent(value) {
	...
	for (let index = 0; index < source.length; index += 1) {
		if (source.charCodeAt(index) === 37 && index + 2 < source.length) {
			...
			let nextIndex = index;
			while (
				nextIndex + 2 < source.length &&
				source.charCodeAt(nextIndex) === 37
			) {
				...
				bytes.push(
					(decodeAsciiHexCodeUnit(high) << 4) + decodeAsciiHexCodeUnit(low),
				);
				nextIndex += 3;
			}
			if (bytes.length > 0) {
...
function normalizeSearchParamsInit(init) {
	...
	if (typeof init === "object" || typeof init === "function") {
		...
		for (const key of Reflect.ownKeys(init)) {
			...
			pairs.push([toNodeUSVString(key), toNodeUSVString(init[key])]);
		}
		return pairs;
	}
	return toNodeUSVString(init);
		...
		: class URLSearchParams {
				...
				toString() {
					return this._pairs
						.map(
							([key, value]) =>
								`${serializeFormUrlencodedComponent(key)}=${serializeFormUrlencodedComponent(value)}`,
						)
						.join("&");
				}
			};
function createStandaloneSearchParams(init) {
	...
	return init === void 0
		...
		: new NativeURLSearchParams(init);
}
...
var URLSearchParams = class _URLSearchParams {
	...
	append(name, value) {
		...
		if (arguments.length < 2) {
			throw createMissingArgsError(
				'The "name" and "value" arguments must be specified',
			);
		}
		getUrlSearchParamsImpl(this).append(
			toNodeUSVString(name),
			toNodeUSVString(value),
		);
	}
	delete(name) {
		assertUrlSearchParamsReceiver(this);
	...
	set(name, value) {
		...
		getUrlSearchParamsImpl(this).set(
			toNodeUSVString(name),
			toNodeUSVString(value),
		);
	}
	...
	entries() {
		...
		return new URLSearchParamsIterator(
			Array.from(getUrlSearchParamsImpl(this)),
		);
	}
	keys() {
		assertUrlSearchParamsReceiver(this);
		return new URLSearchParamsIterator(
			Array.from(getUrlSearchParamsImpl(this).keys()),
		);
	}
	values() {
		assertUrlSearchParamsReceiver(this);
		return new URLSearchParamsIterator(
			Array.from(getUrlSearchParamsImpl(this).values()),
		);
	}
		...
		if (typeof callback !== "function") {
			throw createNodeTypeError(
				'The "callback" argument must be of type function. Received ' +
					(callback === void 0 ? "undefined" : typeof callback),
	...
	[inspectCustomSymbol](depth, options, inspect) {
		...
		const formatValue =
			typeof inspect === "function"
				? (value) => inspect(value, options)
				: (value) => JSON.stringify(value);
		const items = Array.from(getUrlSearchParamsImpl(this)).map(
...
Object.defineProperties(URLSearchParams.prototype, {
	size: {
		get: Object.getOwnPropertyDescriptor(URLSearchParams.prototype, "size")
			?.get,
		configurable: true,
		enumerable: true,
	},
...
function canUseNativeUrlImplementation(candidate) {
	if (
		...
		candidate.__secureExecBootstrapStub === true
	) {
		return false;
	}
	try {
		return (
			String(
				new candidate("./child.mjs", "file:///root/base/entry.mjs").href,
			) === "file:///root/base/child.mjs"
		);
	} catch {
		return false;
	}
...
function normalizeRelativeFileUrlInput(input, base) {
	...
	try {
		...
		const baseDirectory = basePathname.endsWith("/")
			? basePathname
			: builtinPathStdlibModule.posix.dirname(basePathname);
		const resolvedPath = builtinPathStdlibModule.posix.resolve(
			baseDirectory,
			relativePath,
		);
		...
		const normalizedPath = needsTrailingSlash
			? ensureTrailingSlashForFilePath(resolvedPath)
			: resolvedPath;
		return {
...
var nativeUrlCandidate =
	typeof urlStdlibModuleNs?.URL === "function"
		? urlStdlibModuleNs.URL
		: typeof urlStdlibModuleNs?.default?.URL === "function"
			? urlStdlibModuleNs.default.URL
			: globalThis.URL;
var NativeURL = canUseNativeUrlImplementation(nativeUrlCandidate)
	? nativeUrlCandidate
	: class URL {
			constructor(url, base) {
				...
				if (!hasScheme) {
					...
					if (baseUrl.protocol === "file:") {
						...
						const baseDirectory = basePathname.endsWith("/")
							? basePathname
							: builtinPathStdlibModule.posix.dirname(basePathname);
						let resolvedPath = builtinPathStdlibModule.posix.resolve(
							baseDirectory,
							relativePath,
						);
						if (
						...
						full = resolveRelativeNonFileUrl(
							raw,
							baseUrl,
				...
				this.search = full.includes("?")
					? "?" + full.split("?")[1].split("#")[0]
					: "";
...
var URL2 = class _URL {
	...
	constructor(input, base) {
		...
		try {
			this.#impl =
				normalizedArgs.base !== void 0
					? new NativeURL(normalizedArgs.input, normalizedArgs.base)
					: new NativeURL(normalizedArgs.input);
		} catch {
	...
	[inspectCustomSymbol](depth, options, inspect) {
		const inspectName =
			this.constructor === _URL ? "URL" : this.constructor.name;
		if (depth < 0) {
			return `${inspectName} {}`;
		}
		const formatValue =
		...
		if (options?.showHidden) {
			...
			lines.push(
				`  [Symbol(context)]: ${formatUrlContext(this, inspect, options)}`,
			);
		}
		lines.push("}");
		return lines.join("\n");
...
Object.defineProperties(URL2.prototype, {
	...
	[toStringTagSymbol]: {
		get: Object.getOwnPropertyDescriptor(URL2.prototype, toStringTagSymbol)
			?.get,
		configurable: true,
		enumerable: false,
	},
...
export {
	...
	kBlobUrlStore,
	kBlobUrlCounter,
	SEARCH_PARAM_METHOD_NAMES,
	SEARCH_PARAM_PAIR_METHOD_NAMES,
	URL_SCHEME_TYPES,
	searchParamsBrand,
	searchParamsState,
	searchParamsIteratorBrand,
	searchParamsIteratorState,
	createNodeTypeError,
	createInvalidUrlError,
	createUrlReceiverTypeError,
	createMissingArgsError,
	createIterableTypeError,
	createTupleTypeError,
	createSymbolStringError,
	toNodeString,
	toWellFormedString,
	toNodeUSVString,
	assertUrlSearchParamsReceiver,
	assertUrlSearchParamsIteratorReceiver,
	getUrlSearchParamsImpl,
	countSearchParams,
	isAsciiHexCodeUnit,
	decodeAsciiHexCodeUnit,
	appendUtf8CodePoint,
	decodeFormUrlencodedComponent,
	serializeFormUrlencodedComponent,
	compareCodeUnitStrings,
	normalizeSearchParamsInit,
	NativeURLSearchParams,
	createStandaloneSearchParams,
	...
	URLSearchParams,
	canUseNativeUrlImplementation,
	ensureTrailingSlashForFilePath,
	normalizeRelativeFileUrlInput,
	nativeUrlCandidate,
	NativeURL,
	URL2,
