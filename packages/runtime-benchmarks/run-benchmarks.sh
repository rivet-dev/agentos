#!/usr/bin/env bash
#
# Run AgentOS runtime benchmarks against an optimized sidecar, saving JSON and
# logs to packages/runtime-benchmarks/results/.
...
pnpm --dir packages/runtime-core build >&2

echo "=== Building release sidecar ===" >&2
cargo build --release -p agentos-native-sidecar >&2
if [[ -n "${CARGO_TARGET_DIR:-}" ]]; then
	export AGENTOS_SIDECAR_BIN="${AGENTOS_SIDECAR_BIN:-$CARGO_TARGET_DIR/release/agentos-native-sidecar}"
else
	export AGENTOS_SIDECAR_BIN="${AGENTOS_SIDECAR_BIN:-$REPO_ROOT/target/release/agentos-native-sidecar}"
fi
...
build_native_baseline() {
	...
	cargo build --release -p agentos-native-baseline >&2
	if [[ -n "${CARGO_TARGET_DIR:-}" ]]; then
		export NATIVE_BASELINE_BIN="${NATIVE_BASELINE_BIN:-$CARGO_TARGET_DIR/release/agentos-native-baseline}"
	else
		export NATIVE_BASELINE_BIN="${NATIVE_BASELINE_BIN:-$REPO_ROOT/target/release/agentos-native-baseline}"
	fi
...
build_native_baseline_wasm() {
	...
	cargo build --release --target wasm32-wasip1 -p agentos-native-baseline >&2
	if [[ -n "${CARGO_TARGET_DIR:-}" ]]; then
		export NATIVE_BASELINE_WASM="${NATIVE_BASELINE_WASM:-$CARGO_TARGET_DIR/wasm32-wasip1/release/agentos-native-baseline.wasm}"
	else
		export NATIVE_BASELINE_WASM="${NATIVE_BASELINE_WASM:-$REPO_ROOT/target/wasm32-wasip1/release/agentos-native-baseline.wasm}"
	fi
...
run_tsx() {
	...
	echo "" >&2
	echo "=== Running $name ===" >&2
	pnpm --dir packages/runtime-benchmarks exec tsx "$@" \
