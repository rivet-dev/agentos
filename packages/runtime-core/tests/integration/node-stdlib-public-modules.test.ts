...
describe("pinned Node public builtin ledger", () => {
	...
	it("matches the exact expected pass set for the selected stdlib flavor", async () => {
		...
		const expected = ledger.cases
			.filter(
				(entry: { legacy: string; real: string }) => entry[flavor] === "pass",
			)
			.map((entry: { id: string }) => entry.id);
		const expectedAccepted = ledger.cases
			.filter(
				(entry: { legacy: string; real: string }) =>
					entry[flavor] === "fail-accepted",
			)
			.map((entry: { id: string }) => entry.id);
...
const schemeOnly = new Set(['sea', 'test', 'test/reporters']);
const passed = [];
const failed = [];
for (const id of ids) {
  try { require(schemeOnly.has(id) ? 'node:' + id : id); passed.push(id); }
...
`;
		...
		const result = await context.kernel.exec(
			"node /tmp/node-stdlib-public-ledger.cjs",
		);
		expect(result.exitCode, result.stderr).toBe(0);
		const actual = JSON.parse(result.stdout);
		expect(actual.passed, JSON.stringify(actual.failed)).toEqual(expected);
		expect(actual.failed.map((entry: { id: string }) => entry.id)).toEqual(
			expectedAccepted,
		);
	}, 30_000);
});
