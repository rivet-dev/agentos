		...
		const beforeMounts = [
			{
				...
				read_only: true,
				plugin: { id: "host", config: { source: "/tmp/before" } },
			},
		];
		const afterMounts = [
			{
				guest_path: "/workspace",
				read_only: true,
				plugin: { id: "host", config: { source: "/tmp/after" } },
			},
		];
		try {
			expect(
				await process.compareVmConfig(
					...
					after,
					{
						beforeMounts,
						afterMounts,
						beforeRestartIdentity: ["sha256:before"],
						afterRestartIdentity: ["sha256:after"],
					},
				),
			).toBe(false);
			...
			expect(send).toHaveBeenCalledWith({
				...
				payload: {
					...
					before,
					after,
					before_mounts: beforeMounts,
					after_mounts: afterMounts,
					before_restart_identity: ["sha256:before"],
					after_restart_identity: ["sha256:after"],
				},
			});
