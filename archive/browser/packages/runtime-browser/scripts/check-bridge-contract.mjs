...
function checkInstalledConvention(name) {
	...
	if (
		...
		errors.push(
			`${name} is installed as ${installedConvention} in worker.ts but vm-host-interface.json lists ${contractMethods.get(name)}`,
		);
	}
}
...
for (const name of exposedGlobals) {
	...
	if (contractMethods.has(name)) {
		...
		errors.push(`${name} is installed from worker.ts but not in vm-host-interface.json or the browser-only allowlist`);
	}
}
...
for (const name of sorted(contractMethods.keys())) {
	if (
		...
		errors.push(
			`${name} is listed in vm-host-interface.json but is not installed, covered by a browser facade, or explicitly marked unsupported`,
		);
	}
}
...
for (const name of sorted(browserFacadeContractGlobals)) {
	if (!contractMethods.has(name)) {
		errors.push(`${name} is listed as browser facade-covered but is missing from vm-host-interface.json`);
	}
}

for (const name of sorted(browserUnsupportedContractGlobals)) {
	if (!contractMethods.has(name)) {
		errors.push(`${name} is listed as browser-unsupported but is missing from vm-host-interface.json`);
	}
}
...
for (const name of sorted(runtimeGlobalReferences)) {
	...
	if (contractMethods.has(name)) {
		...
		errors.push(`${name} is referenced from runtime.ts but not in vm-host-interface.json or the browser-only allowlist`);
	}
}
