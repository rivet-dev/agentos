---
name: preview-publish-agentos
description: Roll secure-exec changes through an agent-os preview end-to-end — bump the committed .github/refs/secure-exec sha, preview-publish agent-os (which auto-cuts the secure-exec preview). Use when the user wants a secure-exec change validated or handed off via an agent-os preview build.
---

# Preview-publish secure-exec → preview-publish agent-os
...
3. **Preview-publish agent-os**:

   ```bash
   just preview-publish <agent-os-branch>
...
needed. Preview-publish is for previews ONLY; never cut a release with it.
