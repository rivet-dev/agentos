...
const Row = ({ label, value, highlight }: { label: React.ReactNode; value: string; highlight?: boolean }) => (
	<div className='flex items-baseline justify-between gap-4 py-2.5'>
		<span className={`inline-flex min-w-0 items-baseline font-mono text-[13px] ${highlight ? 'font-medium text-cream' : 'font-normal text-cream/45'}`}>
			{label}
		</span>
		<span className={`whitespace-nowrap font-mono text-[15px] tabular-nums ${highlight ? 'font-medium text-sage' : 'font-normal text-cream/45'}`}>
			{value}
		</span>
	</div>
);
				...
				>
					...
					<div className='flex flex-wrap items-start justify-between gap-3'>
						<div className='flex flex-col gap-1'>
							<span className='font-mono text-[11px] font-medium uppercase tracking-[0.18em] text-sage'>Memory Per Instance</span>
					...
					<div className='mt-7 flex h-56 items-end gap-6 max-sm:h-44'>
						...
						<div className='flex h-full flex-1 flex-col items-center'>
							<div className='relative h-full w-full overflow-hidden rounded-lg border border-dashed border-cream/15 bg-cream/[0.05]'>
								<span className='absolute inset-x-0 top-2 text-center font-mono text-[9px] uppercase tracking-wide text-cream/35'>reserved</span>
							</div>
							<span className='mt-2 font-mono text-[13px] tabular-nums text-cream/45'>{mem.sandbox}</span>
							<span className='font-mono text-[9px] uppercase tracking-wide text-cream/30'>reserved / instance</span>
						</div>

						{/* Agent OS — animated fill */}
						<div className='flex h-full flex-1 flex-col items-center'>
							<div className='relative h-full w-full overflow-hidden rounded-lg border border-accent/30'>
								...
								>
									...
									<div className='absolute inset-0 [background-image:repeating-linear-gradient(0deg,rgba(244,241,231,0.16)_0,rgba(244,241,231,0.16)_1px,transparent_1px,transparent_8px)]' />
									<div className='absolute inset-x-0 top-0 h-px bg-cream/40' />
								</motion.div>
							</div>
							<span className='mt-2 font-mono text-[13px] font-medium tabular-nums text-sage'>
								<CountUpStat text={mem.agentOS} active={inView} />
							</span>
							<span className='font-mono text-[9px] uppercase tracking-wide text-accent/70'>used / instance</span>
						</div>
					</div>

					{/* Comparison ledger */}
					<div className='mb-6 mt-7 divide-y divide-cream/10 border-y border-cream/10'>
						<Row
							highlight
							label={
								<>
					...
					<p className='mt-auto font-mono text-[10px] leading-relaxed text-cream/35'>Sandboxes reserve idle RAM per agent; Agent OS isolates share the host.</p>
				</motion.div>
			</InkPanel>
		</Reveal>
	);
