...
export const ExecutionDensity = ({ workload, onWorkloadChange }: { workload: WorkloadKey; onWorkloadChange: (w: WorkloadKey) => void }) => {
	...
	const chipGlyph = (i: number) =>
		workload === 'agent' ? (
			...
			<SquareTerminal className='h-4 w-4 text-sage' aria-hidden='true' />
		);
				...
				>
					...
					<div className='flex flex-wrap items-start justify-between gap-3'>
						<div className='flex flex-col gap-1'>
							<span className='font-mono text-[11px] font-medium uppercase tracking-[0.18em] text-sage'>Cost Per Execution-Second</span>
						...
						<div className='rounded-xl border border-sage/25 bg-sage/[0.06] p-4'>
							<div className='mb-3 flex items-center justify-between gap-3'>
								<span className='inline-flex items-center gap-2 text-sm font-medium text-cream'>
									<Server className='h-4 w-4 text-sage' aria-hidden='true' /> One server &mdash; {tier.label}
								</span>
								<span className='font-mono text-[11px] tabular-nums text-sage'>{execs} executions</span>
							</div>
							<motion.div
							...
							>
								{Array.from({ length: shown }).map((_, i) => (
									<motion.span
										...
										className='flex h-8 w-8 items-center justify-center rounded-md bg-cream/[0.06] ring-1 ring-cream/15'
									>
										{chipGlyph(i)}
									</motion.span>
								))}
								{overflow > 0 && (
									<span className='flex h-8 items-center rounded-md bg-sage/10 px-2 font-mono text-[10px] text-sage'>+{overflow} more</span>
								)}
							</motion.div>
						</div>

						{/* Exactly one sandbox box */}
						<div className='rounded-xl border border-dashed border-cream/15 bg-cream/[0.04] p-4'>
							<div className='mb-3 inline-flex items-center gap-2 text-sm text-cream/55'>
								<Container className='h-4 w-4 text-cream/45' aria-hidden='true' /> One per sandbox
							</div>
							<div className='flex h-12 w-28 items-center justify-center rounded-lg border border-dashed border-cream/20 bg-cream/[0.03]'>
								<span className='font-mono text-[10px] text-cream/45'>~1 GB reserved</span>
							</div>
						</div>
					</div>

					{/* Equation strip — the resolve to cost */}
					<p className='mt-5 font-mono text-[11px] leading-relaxed text-cream/45'>
						${tier.costPerHour.toFixed(4)}/hr server &divide; <span className='text-sage'>{execs} executions</span> = {tier.value} per execution-second
					</p>

					{/* Comparison ledger */}
					<div className='mb-6 mt-4 divide-y divide-cream/10 border-y border-cream/10'>
						<div className='flex items-baseline justify-between gap-4 py-2.5'>
							<span className='inline-flex min-w-0 items-baseline font-mono text-[13px] font-medium text-cream'>
								Agent OS
								<BenchInfoTooltip>
									<strong>What&apos;s measured:</strong>{' '}
									<code className='rounded bg-cream/10 px-1 py-0.5 text-[10px]'>server price per second / concurrent executions per server</code>
									<br /><br />
							...
							<span className='whitespace-nowrap font-mono text-[15px] font-medium tabular-nums text-sage'>
								<CountUpStat text={tier.value} active={inView} />
							</span>
						</div>
						<div className='flex items-baseline justify-between gap-4 py-2.5'>
							<span className='font-mono text-[13px] text-cream/45'>Cheapest sandbox</span>
							<span className='whitespace-nowrap font-mono text-[15px] tabular-nums text-cream/45'>{wl.sandboxCost}</span>
						</div>
					</div>

					<p className='mt-auto font-mono text-[10px] leading-relaxed text-cream/35'>Assumes one agent per sandbox, needed for isolation.</p>
				</motion.div>
			</InkPanel>
		</Reveal>
	);
