...
agentOS is not supported on Cloudflare Workers because its native VM runtime requires a Node.js or container host. Run it on platforms such as Railway, Kubernetes, or Vercel instead.
