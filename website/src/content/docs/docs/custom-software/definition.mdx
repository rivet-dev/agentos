...
description: "The full software-package definition for custom agents, tools, and WASM commands in an agentOS VM."
---

import { Aside } from '@astrojs/starlight/components';

**Software** is anything you install into a VM: an agent, a host tool, or a set of WASM commands. Every package, including the built-ins, is declared with `defineSoftware()` and passed to the `software` option.
...
- **`"tool"`**: host CLI commands exposed inside the VM. Key fields: `bins`, `requires`, `packageDir`.
...
Exposes one or more host CLI commands inside the VM by mapping a command name to the npm package that provides its `bin`.
