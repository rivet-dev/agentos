...
async function filesystem() {
	...
	await Promise.all([
		vm.writeFile("/home/agentos/src/index.ts", "console.log('hi');"),
		vm.writeFile(
...
async function processes() {
	...
	const { pid } = await vm.spawn("node", ["/tmp/server.mjs"], {
...
async function networking() {
	...
	await vm.spawn("node", ["/tmp/app.mjs"]);
...
async function cronJobs() {
	...
	console.log(await vm.listCronJobs());
	// docs:end cron
}
