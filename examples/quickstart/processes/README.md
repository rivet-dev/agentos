...
Create a VM with `AgentOs.create()`, then drive it two ways. Use `vm.process.exec()` for one-shot shell commands — it runs pipelines, `grep`, `sed`, and the like, returning an `exitCode` and, with `output: { capture: "all" }`, captured `stdout` and `stderr`. For longer-running work, `vm.process.spawn()` starts a process and hands back a `pid`; subscribe with `vm.onProcessOutput(pid, handler)`. Wait for completion with `vm.process.wait(pid)`, and inspect what is running with `vm.process.list()`.
