...
pub async fn create(ctx: &HostCtx, port: u16, ttl_seconds: u64) -> Result<SignedPreviewUrlDto> {
...
pub async fn expire(ctx: &HostCtx, token: &str) -> Result<()> {
    run_stmt(
        ctx,
...
pub async fn resolve(ctx: &HostCtx, token: &str) -> Result<Option<u16>> {
    let rows = query_rows(
        ctx,
