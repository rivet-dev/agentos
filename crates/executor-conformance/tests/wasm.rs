...
use agentos_executor_conformance::wasm::{
    ...
    while !saw_exit {
        match poll_wasm_event_after_initial_signal_mask(&mut execution, Duration::from_secs(5)) {
            ...
            Some(WasmExecutionEvent::SignalState {
                ...
                assert_eq!(
                    registration.action,
                    agentos_executor_conformance::ExecutionSignalDispositionAction::User
                );
...
if [ -x \"$basedir/node\" ]; then\n\
  ...
  exec node  \"$basedir/../fake/dist/cli.js\" \"$@\"\n\
    ...
    match &error {
        agentos_executor_conformance::WasmExecutionError::NonWasmBinary {
            path,
            header,
            shell_shim,
        } => {
