...
fn configure_mock_agent_packages(
    ...
    let adapters: Vec<PathBuf> = fs::read_dir(cwd)
        .expect("read adapter cwd")
        .filter_map(Result::ok)
        .map(|entry| entry.path())
        .filter(|path| {
            path.file_name()
                .and_then(|name| name.to_str())
                .is_some_and(|name| name.ends_with("-adapter.mjs"))
        })
        .collect();
...
fn agent_type_for_adapter(adapter: &Path) -> String {
    adapter
        .file_name()
        .and_then(|name| name.to_str())
