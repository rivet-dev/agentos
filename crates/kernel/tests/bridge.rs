...
where
    B: HostBridge,
    <B as agentos_kernel::bridge::BridgeTypes>::Error: Debug,
{
    let contents = bridge
    ...
    let metadata = bridge
        ...
        .expect("stat");
    assert_eq!(metadata.kind, agentos_kernel::bridge::FileKind::File);
    ...
    assert_eq!(
        ...
        agentos_kernel::bridge::PermissionDecision::allow()
    );
    assert_eq!(
        bridge
    ...
    let js_context = bridge
        .create_javascript_context(CreateJavascriptContextRequest {
            vm_id: String::from("vm-1"),
            bootstrap_module: Some(String::from("@rivet-dev/agentos-runtime-core/bootstrap")),
        })
    ...
    let execution = bridge
        .start_execution(agentos_kernel::bridge::StartExecutionRequest {
            vm_id: String::from("vm-1"),
    ...
    bridge
        ...
        .expect("close stdin");
    bridge
        .kill_execution(agentos_kernel::bridge::KillExecutionRequest {
            vm_id: String::from("vm-1"),
...
fn host_bridge_traits_are_method_oriented_and_composable() {
    ...
    bridge.seed_directory(
        ...
        vec![DirectoryEntry {
            ...
            kind: agentos_kernel::bridge::FileKind::File,
        }],
    );
