...
fn virtual_processes_appear_in_process_listings_and_wait_like_children() {
    ...
    assert_eq!(child_info.driver, "host-function");
    assert_eq!(child_info.command, "agentos-host-function");

    kernel
        .exit_process("host-function-dispatch", child.pid(), 23)
    ...
    kernel
        .exit_process("host-function-dispatch", parent.pid(), 0)
        .expect("exit virtual parent");
    assert_eq!(
        kernel.waitpid(parent.pid()).expect("wait parent"),
        agentos_kernel::kernel::WaitPidResult {
            pid: parent.pid(),
            status: 0,
        }
    );
...
fn virtual_process_stdio_uses_standard_fd_helpers_and_owner_checks() {
    ...
    let (stdin_read_fd, stdin_write_fd) = kernel
        .open_pipe("host-function-dispatch", process.pid())
        .expect("open stdin pipe");
    kernel
        .fd_dup2("host-function-dispatch", process.pid(), stdin_read_fd, 0)
    ...
    kernel
        .fd_dup2("host-function-dispatch", process.pid(), stdout_write_fd, 1)
        .expect("wire stdout pipe");
    kernel
        .write_process_stdout("host-function-dispatch", process.pid(), b"stdout-data")
        .expect("write virtual stdout");
    assert_eq!(
        kernel
            .fd_read("host-function-dispatch", process.pid(), stdout_read_fd, 64)
    ...
    kernel
        .fd_dup2("host-function-dispatch", process.pid(), stderr_write_fd, 2)
        .expect("wire stderr pipe");
    kernel
        .write_process_stderr("host-function-dispatch", process.pid(), b"stderr-data")
        .expect("write virtual stderr");
    assert_eq!(
        kernel
            .fd_read("host-function-dispatch", process.pid(), stderr_read_fd, 64)
    ...
    kernel
        .exit_process("host-function-dispatch", process.pid(), 9)
...
fn virtual_process_exit_reclaims_owned_sockets() {
    ...
    let process = kernel
        ...
        .expect("create virtual process");
    let socket = kernel
        .socket_create("host-function-dispatch", process.pid(), SocketSpec::tcp())
    ...
    kernel
        .socket_listen("host-function-dispatch", process.pid(), socket, 1)
    ...
    kernel
        .exit_process("host-function-dispatch", process.pid(), 0)
        .expect("exit virtual process");
