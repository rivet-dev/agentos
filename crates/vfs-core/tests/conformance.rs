use agentos_vfs_core::engine::engines::{ChunkedFs, ChunkedFsOptions, ObjectFs};
use agentos_vfs_core::engine::mem::{InMemoryMetadataStore, MemoryBlockStore, MemoryObjectBackend};
...
impl MetadataStore for PausingResolveStore {
    async fn resolve(&self, path: &str) -> VfsResult<agentos_vfs_core::engine::InodeMeta> {
    ...
    async fn resolve_parent(
        &self,
        path: &str,
    ) -> VfsResult<(agentos_vfs_core::engine::InodeMeta, String)> {
        self.inner.resolve_parent(path).await
    }

    async fn lstat(&self, path: &str) -> VfsResult<agentos_vfs_core::engine::InodeMeta> {
        self.inner.lstat(path).await
    }

    async fn list_dir(&self, ino: u64) -> VfsResult<Vec<agentos_vfs_core::engine::DentryStat>> {
    ...
    ) -> VfsResult<agentos_vfs_core::engine::InodeMeta> {
    ...
    async fn get_chunks(
        ...
        ino: u64,
        range: ChunkRange,
    ) -> VfsResult<Vec<agentos_vfs_core::engine::ChunkRef>> {
