...
impl AgentOs {
    ...
    /// path, which wedges on large/virtual dirs). The sidecar returns the typed children in the
    /// structured `dir_entries` field. `.`/`..` are filtered. Goes through the kernel, so mounts are
    /// listed correctly.
    pub async fn read_dir_with_types(&self, path: &str) -> Result<Vec<VirtualDirEntry>> {
        Self::assert_safe_absolute_path(path)?;
        let result = self
            .guest_fs_call(Self::fs_request(GuestFilesystemOperation::ReadDirWithTypes, path))
            .await?;
        let mut entries = Vec::new();
        for entry in result.dir_entries.unwrap_or_default() {
