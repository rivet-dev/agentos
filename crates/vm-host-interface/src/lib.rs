...
//! Trusted host bridge contracts used by agentOS clients and sidecars.
...
pub use local::{LocalVmHost, LocalVmHostError};

/// Shared associated types for bridge implementations.
pub trait VmHostTypes {
    type Error;
}
...
pub trait HostFilesystem: VmHostTypes {
...
pub trait HostPermissions: VmHostTypes {
...
pub trait HostPersistence: VmHostTypes {
...
pub trait HostClock: VmHostTypes {
...
pub trait HostRandom: VmHostTypes {
...
pub trait HostEvents: VmHostTypes {
...
pub trait HostExecution: VmHostTypes {
...
pub fn bridge_contract() -> &'static BridgeContract {
    BRIDGE_CONTRACT.get_or_init(|| {
        serde_json::from_str(include_str!("../vm-host-interface.json"))
            .expect("vm-host-interface.json must be valid")
    })
}
