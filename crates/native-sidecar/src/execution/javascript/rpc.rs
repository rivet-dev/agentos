...
pub(crate) fn deferred_kernel_wait_request_for_process(
    ...
    //
    // The ownership lookup is authoritative and is preferred over a numeric
    // `fd >= MAPPED_HOST_FD_START` range test: an fd in that range that this
    // process does NOT own should still fall through to the kernel and be
    // rejected there, rather than being silently treated as host-backed.
    if process.mapped_host_fd(fd).is_some() {
        return Ok(None);
    }
    let stat = match kernel.fd_stat(EXECUTION_DRIVER_NAME, process.kernel_pid, fd) {
        Ok(stat) => stat,
        ...
        // the normal write path re-checks ownership and raises the correct
        // errno there, so the loud failure is preserved, not swallowed.
