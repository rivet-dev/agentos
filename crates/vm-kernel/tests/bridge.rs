...
use agentos_vm_kernel::bridge::{
    ...
    VmHost, WriteExecutionStdinRequest, WriteFileRequest,
};
...
where
    B: VmHost,
    <B as agentos_vm_kernel::bridge::VmHostTypes>::Error: Debug,
{
    let contents = bridge
    ...
    let metadata = bridge
        ...
        .expect("stat");
    assert_eq!(metadata.kind, agentos_vm_kernel::bridge::FileKind::File);
    ...
    assert_eq!(
        ...
        agentos_vm_kernel::bridge::PermissionDecision::allow()
    );
    assert_eq!(
        bridge
    ...
    let js_context = bridge
        .create_javascript_context(CreateJavascriptContextRequest {
            vm_id: String::from("vm-1"),
            bootstrap_module: Some(String::from("@rivet-dev/agentos-core/bootstrap")),
        })
    ...
    let execution = bridge
        .start_execution(agentos_vm_kernel::bridge::StartExecutionRequest {
            vm_id: String::from("vm-1"),
    ...
    bridge
        ...
        .expect("close stdin");
    bridge
        .kill_execution(agentos_vm_kernel::bridge::KillExecutionRequest {
            vm_id: String::from("vm-1"),
...
fn host_bridge_traits_are_method_oriented_and_composable() {
    ...
    bridge.seed_directory(
        ...
        vec![DirectoryEntry {
            ...
            kind: agentos_vm_kernel::bridge::FileKind::File,
        }],
    );
