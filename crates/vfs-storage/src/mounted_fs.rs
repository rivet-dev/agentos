use agentos_driver_tokio::{BlockingJobError, DriverHandle};
...
impl<F> MountedEngineFileSystem<F> {
    pub fn with_runtime_context(inner: F, runtime: DriverHandle) -> Self {
        Self {
            inner: Arc::new(inner),
            runtime,
    ...
    fn run<T>(
        ...
        future: impl std::future::Future<Output = agentos_vfs_core::engine::VfsResult<T>>
            + Send
            + 'static,
    ) -> PosixVfsResult<T>
    where
        T: Send + 'static,
    {
        if agentos_driver_tokio::is_driver_worker_thread() {
        ...
        let handle = self.runtime.tokio_handle().clone();
...
where
    F: agentos_vfs_core::engine::VirtualFileSystem + 'static,
{
        ...
        .map(|entries| {
            entries
                ...
                .map(|entry| VirtualDirEntry {
                    name: entry.name,
                    is_directory: entry.kind == agentos_vfs_core::engine::InodeType::Directory,
                    is_symbolic_link: entry.kind == agentos_vfs_core::engine::InodeType::Symlink,
                })
                .collect()
        })
    }
...
fn convert_error(error: agentos_vfs_core::engine::VfsError) -> PosixVfsError {
    PosixVfsError::new(error.code(), error.message().to_owned())
}

fn convert_stat(stat: agentos_vfs_core::engine::VirtualStat, device_id: u64) -> VirtualStat {
    VirtualStat {
        mode: stat.mode,
        size: stat.size,
        blocks: stat.blocks,
...
fn timespec_ms(time: agentos_vfs_core::engine::Timespec) -> u64 {
    if time.sec < 0 {
        return 0;
    }
...
mod tests {
    ...
    use agentos_vfs_core::posix::S_IFREG;
    ...
    fn mounted_engine_filesystem_bridges_sync_posix_calls() {
        ...
        let mut mounted = MountedEngineFileSystem::with_runtime_context(fs, runtime.handle());
    ...
    fn mounted_engine_filesystem_rejects_waits_on_agentos_runtime_workers() {
        ...
        let task = context
            .spawn(agentos_driver_tokio::TaskClass::Plugin, async move {
                let fs = ChunkedFs::with_options(
                    InMemoryMetadataStore::new(),
                    MemoryBlockStore::new(),
                    ChunkedFsOptions::default(),
