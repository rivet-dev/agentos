...
use agentos_vfs_core::engine::mem::MemoryBlockStore;
use agentos_vfs_core::engine::{
    BlockKey, BlockStore, VirtualFileSystem, S_IFBLK, S_IFCHR, S_IFIFO,
};
use agentos_vfs_core::posix::MountedFileSystem;
use agentos_vfs_core::posix::{MemoryFileSystem, MountOptions, MountTable};
use agentos_vfs_storage::{FileBlockStore, MountedEngineFileSystem, SqliteMetadataStore};
...
fn chunked_local_mount_table_preserves_sparse_pwrite_allocation() {
    ...
    agentos_vfs_core::posix::VirtualFileSystem::mkdir(&mut table, "/mnt", false).unwrap();
    table
        .mount_boxed(
            "/mnt",
            Box::new(mounted),
    ...
    agentos_vfs_core::posix::VirtualFileSystem::write_file(&mut table, "/mnt/sparse", Vec::new())
        .unwrap();
    agentos_vfs_core::posix::VirtualFileSystem::pwrite(
        &mut table,
        "/mnt/sparse",
        vec![b'x'; 50 * 1024],
        1_600 * 1024,
    ...
    let stat = agentos_vfs_core::posix::VirtualFileSystem::stat(&mut table, "/mnt/sparse").unwrap();
    assert_eq!(stat.size, 1_650 * 1024);
    assert_eq!(stat.blocks, 100);
}
