...
use agentos_executor_v8_runtime::runtime_protocol::{
    RuntimeCommand, RuntimeEvent, WarmSessionHint,
};
...
impl V8RuntimeHost {
    ...
    pub fn spawn(runtime: &DriverHandle) -> io::Result<Self> {
        Ok(V8RuntimeHost {
    ...
    pub fn warm_snapshot_async(runtime: &DriverHandle, userland_code: String) {
        if userland_code.is_empty() {
            return;
        }
...
impl V8SessionHandle {
    ...
    pub fn set_module_reader(
        &self,
        reader: Box<dyn agentos_executor_v8_runtime::execution::GuestModuleReader>,
    ) -> io::Result<()> {
...
impl crate::backend::ExecutionWakeTarget for V8SessionHandle {
    fn publish_readiness(
        ...
        V8SessionHandle::publish_readiness(
            ...
            agentos_driver_tokio::readiness::ReadyFlags::from_bits(flags.bits()),
        )
...
pub fn ensure_runtime_initialized(runtime: &DriverHandle) -> io::Result<()> {
    shared_embedded_runtime_client(runtime).map(|_| ())
}

pub fn pre_warm_agent_snapshot(runtime: &DriverHandle, userland_code: &str) -> io::Result<()> {
    if userland_code.is_empty() {
        return Ok(());
    }
...
fn to_runtime_command(frame: &BinaryFrame) -> io::Result<RuntimeCommand> {
    ...
    let runtime_frame =
        agentos_executor_v8_runtime::ipc_binary::read_frame(&mut Cursor::new(bytes))?;
...
fn from_runtime_execution_error(
    error: agentos_executor_v8_runtime::ipc_binary::ExecutionErrorBin,
