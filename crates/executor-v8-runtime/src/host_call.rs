...
use agentos_driver_tokio::accounting::{Reservation, ResourceClass};
...
pub struct SyncBridgeCallResponse {
    pub status: u8,
    pub payload: Vec<u8>,
    pub reservation: Option<agentos_driver_tokio::accounting::SharedReservation>,
}
...
struct BridgeCallTarget {
    ...
    response_resources: Arc<agentos_driver_tokio::accounting::ResourceLedger>,
...
fn transfer_response_reservation(
    ...
    payload_bytes: usize,
) -> agentos_driver_tokio::accounting::SharedReservation {
    ...
    agentos_driver_tokio::accounting::SharedReservation::new(reservation)
}
...
pub struct BridgeCallContext {
    ...
    runtime: Option<agentos_driver_tokio::DriverHandle>,
...
impl BridgeCallContext {
    ...
    pub(crate) fn runtime_context(&self) -> Option<&agentos_driver_tokio::DriverHandle> {
        self.runtime.as_ref()
    }

    pub(crate) fn timer_task_owner(&self) -> Option<agentos_driver_tokio::TaskOwner> {
        self.session_generation
            .map(|generation| agentos_driver_tokio::TaskOwner::Vm { generation })
    }
            ...
            let runtime = match self.runtime.as_ref() {
                ...
                None => {
                    return Err(self.cleanup_pending_call_after_error(
                        call_id,
                        String::from(
                    "ERR_AGENTOS_RUNTIME_NOT_INJECTED: direct bridge calls require a session DriverHandle",
                        ),
                    ));
                }
            };
...
mod tests {
    ...
    fn test_runtime_context() -> DriverHandle {
    ...
    fn limited_bridge_runtime(
        ...
        DriverHandle,
        Arc<agentos_driver_tokio::accounting::ResourceLedger>,
    ) {
    ...
    fn assert_ledger_settles_to_zero(resources: &agentos_driver_tokio::accounting::ResourceLedger) {
    ...
    fn bridge_registry_accounts_and_releases_settled_call_reservations() {
        ...
        assert!(
            metrics.resources
                [agentos_driver_tokio::metrics::ResourceMetricClass::BridgeCalls.index()]
            .high_water
                >= 1
        );
        assert!(
            metrics.buffers[agentos_driver_tokio::metrics::BufferMetricClass::Bridge.index()]
                .high_water
                >= 4
        );
    }
        ...
        let error = registry
            .settle(
                ...
                BridgeResponse {
                    ...
                    status: 0,
                    payload: vec![0xA7; 2],
                    reservation: Some(agentos_driver_tokio::accounting::SharedReservation::new(
