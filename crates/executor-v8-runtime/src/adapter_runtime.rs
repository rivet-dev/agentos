...
use agentos_executor_v8_runtime::embedded_runtime::{
    spawn_embedded_runtime_ipc, EmbeddedRuntimeHandle,
};
...
impl V8Runtime {
    ...
    pub fn spawn(runtime_context: &DriverHandle) -> io::Result<Self> {
...
/// `vm-host-interface.json` so bridge installation and dispatch drift together.
pub fn map_bridge_method(method: &str) -> (&str, bool) {
    if let Some(target) = agentos_vm_host_interface::bridge_contract()
        .dispatch
        .get(method)
    {
