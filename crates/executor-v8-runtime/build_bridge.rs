...
const BUILD_SCRIPT_CANDIDATES: &str =
    "packages/build-tools/scripts/build-v8-bridge.mjs or packages/core/scripts/build-v8-bridge.mjs";
...
fn emit_rerun_inputs(repo_root: &Path, script_path: &Path, package_root: &Path) {
    let inputs = [
        repo_root.join("crates/executor-v8-runtime/build_bridge.rs"),
    ...
    let shim_dir = repo_root.join("crates/executor-v8-runtime/assets/undici-shims");
