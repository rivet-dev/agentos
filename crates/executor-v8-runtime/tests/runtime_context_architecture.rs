...
fn production_v8_runtime_never_discovers_the_process_runtime() {
    ...
    let forbidden = "TokioDriver::process_handle";
...
fn process_runtime_is_the_only_vm_executor_admission_owner() {
    ...
    let runtime = fs::read_to_string(manifest_root.join("../driver-tokio/src/lib.rs"))
    ...
    assert!(
        ...
        "one process-owned admission must be cloned unchanged into every DriverHandle scope"
    );

    let admission = fs::read_to_string(manifest_root.join("../driver-tokio/src/executor.rs"))
