...
fn stage_typescript_assets(manifest_dir: &Path, out_dir: &Path) {
    ...
    if !assets.iter().any(|asset| asset == "typescript.js") {
        ...
        println!(
            "cargo:warning=agentos-executor-v8-runtime: building without the bundled TypeScript compiler; guest TypeScript checking will be unavailable in this build."
        );
    }
...
fn stage_pyodide_assets(manifest_dir: &Path, out_dir: &Path) {
    ...
    if pyodide_unavailable {
        ...
        println!(
            "cargo:warning=agentos-executor-v8-runtime: building without bundled Pyodide assets; \
