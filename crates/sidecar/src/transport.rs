...
use agentos_vm::extension::{ExtensionRequestClass, ExtensionServices};
use agentos_vm::extension_services::{
...
use agentos_vm::service::CompletedExtensionRequest;
use agentos_vm::service::{CompletedRequest, PreparedMembershipCommit, PreparedRequest};
use agentos_vm::vm::{
...
use agentos_vm::{
    EventSinkTransport, Extension, SidecarRequestTransport, VmError, VmManager, VmManagerConfig,
};
...
struct DetachedExtensionCompletion {
    ...
    result: Result<CompletedExtensionRequest, VmError>,
}
...
enum DetachedRequestResult {
    Generic(Result<CompletedRequest, VmError>),
    Create(Result<CompletedCreateVm<LocalBridge>, VmError>),
    DisposeDrained {
        plan: DisposeVmPlan<LocalBridge>,
        disposal: VmDisposal,
    },
    DisposeExecuted {
        result: Result<CompletedDisposeVm, VmError>,
        disposal: VmDisposal,
    },
}
...
impl ProtocolFrameWriter {
    ...
    fn new(
        ...
        let ordinary_metric = agentos_driver_tokio::metrics::ChannelMetricClass::StdioEgress;
        let control_metric = agentos_driver_tokio::metrics::ChannelMetricClass::StdioEgress;
        Ok(Self {
            output,
            codec,
...
pub fn run_with_runtime_config(
    control_fd: OwnedFd,
    runtime: agentos_driver_tokio::DriverConfig,
...
pub fn run_with_extensions_and_executors(
    ...
    run_with_configured_executors(extensions, Some(control_fd), None, executors)
}
...
pub fn run_combined_with_extensions_and_executors(
    ...
    run_with_configured_executors(extensions, None, None, executors)
}

fn run_with_optional_control_and_runtime(
    extensions: Vec<Box<dyn Extension>>,
    control_fd: Option<OwnedFd>,
    runtime: Option<agentos_driver_tokio::DriverConfig>,
) -> Result<(), Box<dyn Error>> {
    run_with_configured_executors(extensions, control_fd, runtime, crate::executor_registry())
}

pub fn run_with_configured_executors(
    extensions: Vec<Box<dyn Extension>>,
    control_fd: Option<OwnedFd>,
    runtime: Option<agentos_driver_tokio::DriverConfig>,
    executors: agentos_vm::ExecutorRegistry,
) -> Result<(), Box<dyn Error>> {
    let mut config = VmManagerConfig {
    ...
    if let Err(error) = agentos_vm::executor::v8_host::ensure_runtime_initialized(&runtime_context)
    {
    ...
    // VmManager state Send or wrapping it in a global mutex.
    let local = tokio::task::LocalSet::new();
    runtime.block_on(local.run_until(run_async(
        extensions,
        executors,
        config,
        runtime_context,
...
async fn run_async(
    ...
    let extension_routes = Arc::new(sidecar.extension_routes().clone());
    ...
    let (limit_warning_tx, limit_warning_rx) = channel::<
...
struct ProtocolEngine {
    ...
    limit_warning_rx: Receiver<agentos_resource_accounting::queue_tracker::LimitWarning>,
...
async fn run_protocol_engine(engine: ProtocolEngine) -> Result<(), Box<dyn Error>> {
    ...
    let (connection_ids, session_ids) = sidecar.membership_snapshot();
    let mut active_sessions = session_ids
        .into_iter()
        .map(|(connection_id, session_id)| SessionScope {
            connection_id,
            session_id,
        })
        .collect::<BTreeSet<_>>();
    let mut active_connections = connection_ids.into_iter().collect::<BTreeSet<_>>();
    ...
    'protocol: loop {
        ...
        for _ in 0..pending_admission_turn {
            ...
            match target.admit_vm_event_nowait() {
                Ok(VmEventAdmissionResult::Admitted(mut target)) => {
                    if let Err(error) = target.prepare_admitted(&mut sidecar) {
                        tracing::error!(%error, "ERR_AGENTOS_PROCESS_EVENT_PREPARE: claimed host event could not be prepared");
                        continue;
                    }
                    ...
                    Some(warning) => {
                        ...
                        if let Some(connection_id) = active_connections.iter().next() {
                            ...
                            let frame = protocol_try!('protocol,
                                agentos_vm::service::structured_event_frame(
                                    connection_id,
                                    "limit_warning",
                                    detail,
                                ),
                ...
                for session in active_sessions.iter().cloned().collect::<Vec<_>>() {
                    ...
                    for target in turn.host_services {
                        match prepare_owned_process_event_service(
                            &mut sidecar,
                            &ownership_coordinator,
                            OwnedProcessEventService::Host(target),
                        )
                        .admit_vm_event_nowait()
                        {
                            ...
                            Err(error) => tracing::debug!(
                                %error,
                                "claimed host event ended before VM service admission"
                            ),
                        }
                    }
    ...
    match frame {
        ProtocolFrame::RequestFrame(request) => {
                ...
                == ExtensionRequestClass::Progress
            {
                ...
                let metadata = request_operation_metadata(&request, sidecar.extension_routes());
                let key = RequestOperationKey::new(
                    ownership_connection_id(&request.ownership),
                    request.request_id,
                );
            ...
            if let Some(prepared) = prepared {
                ...
                if matches!(request.payload, RequestPayload::CreateVmRequest(_)) {
                    ...
                    let agentos_vm::protocol::RequestPayload::CreateVm(payload) =
                ...
                if matches!(request.payload, RequestPayload::DisposeVmRequest(_)) {
                    ...
                    let agentos_vm::protocol::RequestPayload::DisposeVm(payload) =
                        compat_request.payload.clone()
                    else {
                        unreachable!("wire DisposeVm converted to a different compatibility route")
                    };
                    let cancellation_reason = match payload.reason {
                        agentos_vm::protocol::DisposeReason::Requested => {
                            OperationCancellationReason::Explicit
                        }
                        agentos_vm::protocol::DisposeReason::ConnectionClosed => {
                            OperationCancellationReason::ConnectionClosed
                        }
                        agentos_vm::protocol::DisposeReason::HostShutdown => {
                            OperationCancellationReason::Shutdown
                        }
                    };
                ...
                let error = VmError::InvalidState(String::from(
...
fn finish_request(
    ...
    let (dispatch, failed, update_membership) = match result {
        ...
        DetachedRequestResult::Create(result) => match result
            ...
            .and_then(agentos_vm::service::wire_dispatch_result)
        {
        ...
        DetachedRequestResult::DisposeExecuted { result, disposal } => {
            let central_result = match result {
                ...
                Err(error) => {
                    if let OwnershipScope::VmOwnership(scope) = &request.ownership {
                        ...
                        if let Err(cleanup_error) = sidecar.clear_vm_permissions(&scope.vm_id) {
                            eprintln!(
            ...
            let coordinator_result = disposal.complete().map_err(|error| {
                VmError::InvalidState(format!("ERR_AGENTOS_VM_COORDINATOR_DISPOSAL: {error}"))
            });
...
fn coordinator_admission_error(error: OwnershipCoordinatorError) -> VmError {
    VmError::RequestAdmission {
        code: error.code(),
...
fn update_ownership_membership(
    ...
    let result = match response {
        ...
        ResponsePayload::VmCreatedResponse(response) => match &request.ownership {
            ...
            ownership => Err(
                agentos_vm::ownership_coordinator::OwnershipCoordinatorError::OwnershipMismatch {
...
fn ensure_connection_membership(
    coordinator: &OwnershipCoordinator,
    connection_id: &str,
) -> Result<(), agentos_vm::ownership_coordinator::OwnershipCoordinatorError> {
    coordinator
        .register_connection(connection_id.to_owned())
        .map(|_| ())
        .or_else(|error| match error {
            agentos_vm::ownership_coordinator::OwnershipCoordinatorError::Duplicate {
                scope: "connection",
                ..
            } => Ok(()),
            error => Err(error),
...
#[cfg(test)]
#[path = "transport/request_concurrency_tests.rs"]
mod request_concurrency_tests;

#[cfg(test)]
mod tests {
    use super::*;
    use agentos_vm::wire::{AuthenticateRequest, ExtEnvelope};
    use std::io::Cursor;
    ...
    fn test_frame_writer_with_inflight(
        ...
        let protocol = agentos_driver_tokio::DriverProtocolConfig {
            ...
            max_progress_frames: max_in_flight,
            max_progress_bytes: max_in_flight.saturating_mul(maximum_encoded_bytes),
    ...
    impl Extension for GatedExtension {
        ...
        fn handle_request<'a>(
            &'a self,
            _ctx: agentos_vm::ExtensionContext,
            payload: Vec<u8>,
        ) -> agentos_vm::ExtensionFuture<'a, agentos_vm::ExtensionResponse> {
            Box::pin(async move {
                self.started.fetch_add(1, Ordering::AcqRel);
                self.started_notify.notify_waiters();
                self.release.notified().await;
                Ok(agentos_vm::ExtensionResponse::new(payload))
            })
        }
    ...
    async fn drain_claimed_process_event_services_one_at_a_time(
        ...
        for _ in 0..expected {
            ...
            let mut admitted = admitted.expect(
                "one pre-admitted internal event must run and release capacity for the next",
            );
            admitted
                .prepare_admitted(sidecar)
                .expect("prepare admitted host event");
        ...
        tokio::task::LocalSet::new()
            .run_until(async {
                ...
                for request_id in [10, 11] {
                    ...
                    let prepared = PreparedRequest::for_test_future(compat_request, async move {
                        if request_id == 10 {
                            release.notified().await;
                        }
                        Ok(agentos_vm::core::DispatchResult {
                            response: agentos_vm::core::reject(
                                &response_request,
                ...
                assert!(matches!(
                    ...
                    DetachedRequestResult::Generic(Err(VmError::Execution(message)))
    ...
    async fn staged_dispose_cancels_and_drains_a_gated_vm_operation_before_detach() {
        tokio::task::LocalSet::new()
            .run_until(async {
                ...
                .expect("test sidecar");
                agentos_vm::test_support::register_session(&mut sidecar, connection_id, session_id);
                let create_payload = agentos_vm::protocol::CreateVmRequest::legacy_test_config(
                    agentos_vm::protocol::GuestRuntimeKind::JavaScript,
                    Default::default(),
                    Default::default(),
                    None,
                );
                let create_request = agentos_vm::protocol::RequestFrame::new(
                    1,
                    agentos_vm::protocol::OwnershipScope::session(connection_id, session_id),
                    agentos_vm::protocol::RequestPayload::CreateVm(create_payload.clone()),
                );
                ...
                let agentos_vm::protocol::ResponsePayload::VmCreated(created) =
                ...
                let prepared_operation = PreparedRequest::for_test_future(compat_operation, {
                ...
                assert!(!agentos_vm::test_support::contains_vm(&sidecar, &vm_id));
                assert_eq!(operations.snapshot().in_flight_requests, 0);
        ...
        tokio::task::LocalSet::new()
            .run_until(async {
                ...
                let (gated, mut gated_reply) =
                    agentos_vm::extension_services::prepared_test_service_command(
                        "test_gated_service",
                        async move {
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn blocked_host_event_service_does_not_delay_an_independent_request() {
        tokio::task::LocalSet::new()
            .run_until(async {
                let config = VmManagerConfig::default();
                let runtime = agentos_driver_tokio::TokioDriver::process(&config.runtime)
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn detached_runtime_fault_finishes_without_internal_service_admission() {
        tokio::task::LocalSet::new()
            .run_until(async {
                let config = VmManagerConfig::default();
                let driver =
                    agentos_driver_tokio::TokioDriver::process(&config.runtime).expect("driver");
                let mut sidecar = VmManager::with_config_extensions_and_runtime(
                    LocalBridge::default(),
                    config.clone(),
                    Vec::new(),
                ...
                .expect("manager");
                let connection = "generic-events-connection";
                let session = "generic-events-session";
                agentos_vm::test_support::register_session(&mut sidecar, connection, session);
                let create = agentos_vm::protocol::CreateVmRequest::legacy_test_config(
                    agentos_vm::protocol::GuestRuntimeKind::JavaScript,
                    Default::default(),
                    Default::default(),
                    Some(agentos_vm::protocol::PermissionsPolicy::allow_all()),
                );
                let request = agentos_vm::protocol::RequestFrame::new(
                    900,
                    agentos_vm::protocol::OwnershipScope::session(connection, session),
                    agentos_vm::protocol::RequestPayload::CreateVm(create.clone()),
                );
                let prepared = sidecar
                    .prepare_create_vm(&request, create)
                    .expect("prepare VM");
                let completed = prepared.execute().await.expect("create VM");
                let created = sidecar.complete_create_vm(completed).expect("publish VM");
                let agentos_vm::protocol::ResponsePayload::VmCreated(created) =
                    created.response.payload
                else {
                    panic!("expected VM")
                ...
                let vm_id = created.vm_id;
                let detached = agentos_vm::test_support::insert_process(
                    &mut sidecar,
                    &vm_id,
                    "faulted-detached",
                    true,
                );
                detached.runtime_fault();
                let ownership =
                    agentos_vm::protocol::OwnershipScope::vm(connection, session, &vm_id);
                let turn = sidecar
                    .pump_process_events_nowait(&ownership, 1)
                    .expect("pump detached fault");
                assert!(
                    turn.host_services.is_empty(),
                    "runtime faults are terminal events, not host work"
                );
                assert!(
                    turn.emitted_any,
                    "detached fault must emit its terminal event"
                );
            })
            .await;
    }
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn stale_generation_completion_is_rejected_before_settlement() {
        tokio::task::LocalSet::new()
            .run_until(async {
                let config = VmManagerConfig::default();
                let driver =
                ...
                let vm_id = created.vm_id;
                let process =
                    agentos_vm::test_support::insert_process(&mut sidecar, &vm_id, "root", false);
                let reply = process.stale_completion();
                let ownership =
                    agentos_vm::protocol::OwnershipScope::vm(connection, session, &vm_id);
                ...
                let turn = sidecar
                    .pump_process_events_nowait(&ownership, 1)
                    .expect("claim stale completion");
                assert_eq!(turn.host_services.len(), 1);
                let target = turn.host_services.into_iter().next().expect("completion");
                let command = prepare_owned_process_event_service(
                    &mut sidecar,
                    &coordinator,
                    OwnedProcessEventService::Host(target),
                );
                ...
                let error = reply
                    ...
                    .expect_err("stale generation must not complete a new process's call");
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn root_and_child_host_calls_progress_under_ordinary_admission_saturation() {
        tokio::task::LocalSet::new()
            .run_until(async {
                let config = VmManagerConfig::default();
                let driver =
                ...
                let root =
                    agentos_vm::test_support::insert_process(&mut sidecar, &vm_id, "root", false);
                let child =
                    agentos_vm::test_support::insert_child(&mut sidecar, &vm_id, "root", "child");
                let detached = agentos_vm::test_support::insert_process(
                    &mut sidecar,
                    &vm_id,
                    "detached",
                ...
                let mut limits = config.runtime.clone();
                limits.protocol.max_in_flight_requests = 1;
                limits.protocol.max_process_events = 2;
                let coordinator = OwnershipCoordinator::from_runtime_config(&limits);
                coordinator
                    .register_connection(connection)
                    .expect("connection")
                    .open_session(session)
                ...
                let _ordinary = coordinator
                    .admit(
                        ...
                        agentos_vm::request_operations::OperationCancellation::new(),
                    )
                    .await
                    .expect("hold ordinary capacity");
                // Generic host calls exercise the same owned service path for every
                // executor. Each must preserve its root/child locator and reply once.
                let replies = [
                    root.rpc(900, "fs.readFile", vec![serde_json::json!("/missing")]),
                    child.rpc(901, "fs.readFile", vec![serde_json::json!("/missing")]),
                    detached.rpc(902, "fs.readFile", vec![serde_json::json!("/missing")]),
                ];
                let turn = sidecar
                    .pump_process_events_nowait(&ownership, 3)
                    .expect("claim generic calls");
                assert_eq!(turn.host_services.len(), 3);
                ...
                for target in turn.host_services {
                    ...
                    let command = match command.admit_vm_event_nowait().expect("event admission") {
                        VmEventAdmissionResult::Admitted(command) => command,
                        VmEventAdmissionResult::Deferred(command) => {
                            deferred += 1;
                            command
                        }
                    };
                ...
                assert_eq!(
                    deferred, 1,
                    "internal event admission is independently bounded"
                );
                ...
                for reply in replies {
                    let result = tokio::time::timeout(Duration::from_secs(1), reply)
                        .await
                        .expect("direct reply deadline")
                        .expect("direct reply channel");
                    assert!(
                        result.is_err(),
                        "missing VFS path must fail through the direct waiter"
                    );
                }
                assert!(root.is_empty() && child.is_empty() && detached.is_empty());
                assert!(sidecar
                    .pump_process_events_nowait(&ownership, 3)
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn host_event_service_panic_is_typed_and_releases_vm_admission() {
        tokio::task::LocalSet::new()
            .run_until(async {
                let config = VmManagerConfig::default();
                let runtime = agentos_driver_tokio::TokioDriver::process(&config.runtime)
                ...
                let (panicking, panic_reply) =
                    agentos_vm::extension_services::prepared_test_vm_service_command(
                        "service_python_process_event",
                        &coordinator,
                        &ownership,
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn vm_disposal_cancels_and_drains_blocked_host_event_service() {
        tokio::task::LocalSet::new()
            .run_until(async {
                let config = VmManagerConfig::default();
                let coordinator = OwnershipCoordinator::from_runtime_config(&config.runtime);
    ...
    async fn supervised_extension_wait_does_not_block_an_independent_request() {
        ...
        local
            .run_until(async {
                ...
                .expect("test sidecar");
                let protocol = agentos_driver_tokio::DriverProtocolConfig::default();
                ...
                let ownership_coordinator =
                    OwnershipCoordinator::from_runtime_config(&VmManagerConfig::default().runtime);
                ownership_coordinator
    ...
    async fn router_reports_count_and_byte_admission_limits_before_terminal_capacity() {
        ...
        local
            .run_until(async {
                ...
                let count_protocol = agentos_driver_tokio::DriverProtocolConfig {
                ...
                let byte_protocol = agentos_driver_tokio::DriverProtocolConfig {
    ...
    fn stdio_work_queues_are_bounded() {
        let capacity = agentos_driver_tokio::DEFAULT_PROTOCOL_MAX_INGRESS_FRAMES;
    ...
    async fn async_callback_waiter_does_not_park_router_and_cleans_up_on_drop() {
        ...
        let request = agentos_vm::protocol::SidecarRequestFrame::new(
            -8,
            ownership.clone(),
            agentos_vm::protocol::SidecarRequestPayload::Ext(agentos_vm::protocol::ExtEnvelope {
                namespace: String::from("dev.agentos.test.callback"),
        ...
        let dropped = agentos_vm::protocol::SidecarRequestFrame::new(
            -9,
            ownership,
            agentos_vm::protocol::SidecarRequestPayload::Ext(agentos_vm::protocol::ExtEnvelope {
                namespace: String::from("dev.agentos.test.callback"),
    ...
    fn queue_test_sidecar_request(request_id: RequestId) -> ProtocolFrame {
        ProtocolFrame::SidecarRequestFrame(agentos_vm::wire::SidecarRequestFrame {
            schema: wire::protocol_schema(),
            request_id,
    ...
    fn protocol_output_logical_control_reserves_are_independent() {
        ...
        let heartbeat = agentos_vm::service::structured_event_frame(
            HEARTBEAT_CONNECTION_ID,
            "heartbeat",
            std::collections::HashMap::new(),
        )
...
struct PendingLiveEvent {
    event: agentos_vm::wire::EventFrame,
    _reservation: ProtocolReservation,
}
    ...
    fn emit_event(&self, event: agentos_vm::wire::EventFrame) -> Result<(), VmError> {
        ...
        self.sender
            ...
            .map_err(|error| match error {
                tokio::sync::mpsc::error::TrySendError::Full(_) => VmError::Bridge(
                    String::from(
                ...
                tokio::sync::mpsc::error::TrySendError::Closed(_) => VmError::Bridge(
...
impl FrameSidecarRequestTransport {
    ...
    fn register_async_waiter(
        &self,
        request_id: RequestId,
    ) -> Result<tokio::sync::oneshot::Receiver<PendingSidecarResponseResult>, VmError> {
        let mut pending = self.pending.lock().map_err(|_| {
            VmError::Bridge(String::from("sidecar callback waiter map lock poisoned"))
        })?;
        if pending.contains_key(&request_id) {
    ...
    fn fail_all(&self, message: &str) -> Result<(), VmError> {
        let pending = {
            let mut pending = self.pending.lock().map_err(|_| {
                VmError::Bridge(String::from("sidecar callback waiter map lock poisoned"))
            })?;
        ...
        for (request_id, target) in pending {
            ...
            let error = Err(VmError::Io(message.to_owned()));
            match sender {
    ...
    fn send_request_async<'a>(
        ...
        Box<
            dyn std::future::Future<
                    Output = Result<agentos_vm::protocol::SidecarResponseFrame, VmError>,
                > + Send
                + 'a,
        >,
    > {
        Box::pin(async move {
            ...
            match tokio::time::timeout(response_timeout, receiver).await {
                ...
                Ok(Err(_)) => Err(VmError::Io(String::from(
                    "sidecar response waiter disconnected",
                ))),
                Err(_) => Err(VmError::Io(format!(
                    "timed out waiting for sidecar response after {}s",
                    timeout.as_secs()
                ))),
            }
