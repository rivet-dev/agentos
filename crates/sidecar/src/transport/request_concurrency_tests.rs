...
use super::*;
use agentos_vm::wire::ExtEnvelope;
...
struct Gate {
    ...
    release: Semaphore,
    cancel: agentos_vm::request_operations::OperationCancellation,
}
...
impl Default for Gate {
    fn default() -> Self {
        Self {
            ...
            cancel: agentos_vm::request_operations::OperationCancellation::new(),
        }
    }
}
    ...
    fn handle_request<'a>(
        ...
        Box::pin(async move {
            let mut command = String::from_utf8(payload).map_err(|error| {
                VmError::InvalidState(format!("invalid test extension command: {error}"))
            })?;
            if let Some(keyed) = command.strip_prefix("key:") {
                let (_, inner) = keyed.split_once(':').ok_or_else(|| {
                    VmError::InvalidState(String::from("invalid keyed test command"))
                })?;
                ...
                .or_else(|| command.strip_prefix("progress-events:"))
            {
                let events = (0..3)
                    .map(|index| {
                        ...
                        agentos_vm::service::structured_event_frame("conn", "request-batch", detail)
                    })
                    .collect::<Result<Vec<_>, _>>()?;
                return agentos_vm::ExtensionResponse::with_wire_events(
            ...
            if let Some(name) = command.strip_prefix("cancel:") {
                ...
                return Ok(agentos_vm::ExtensionResponse::new(
            ...
            if let Some(name) = command.strip_prefix("block:") {
                ...
                return Ok(agentos_vm::ExtensionResponse::new(outcome.into_bytes()));
            }
            ...
            Ok(agentos_vm::ExtensionResponse::new(command.into_bytes()))
        })
    }
}
...
impl ProtocolLoopHarness {
    fn build(
        ...
        let operations =
            OperationTable::from_protocol_config(&agentos_driver_tokio::DriverProtocolConfig {
                max_in_flight_requests: max_requests,
                max_in_flight_request_bytes: max_request_bytes,
                max_progress_frames: max_requests,
                max_progress_bytes: max_request_bytes,
                ..protocol.clone()
            });
        ...
        .run_until(async {
            ...
            tokio::time::timeout(TEST_TIMEOUT, async {
                while vm.snapshot().lifecycle
                    != agentos_vm::ownership_coordinator::VmLifecyclePhase::Pending
                {
...
async fn request_concurrency_real_loop_vm_a_disposal_does_not_delay_vm_b_query() {
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let held_cancellation = agentos_vm::request_operations::OperationCancellation::new();
            ...
            tokio::time::timeout(TEST_TIMEOUT, async {
                while vm_a_coordinator.snapshot().phase
                    != agentos_vm::ownership_coordinator::CoordinatorPhase::Closing
                {
                    tokio::task::yield_now().await;
                }
            })
...
async fn request_concurrency_real_loop_attached_and_detached_children_advance_while_ordinary_is_gated(
    ...
    assert_child_progress_while_ordinary_is_gated(false).await;
}

#[tokio::test(flavor = "current_thread")]
async fn request_concurrency_real_loop_signaled_child_preserves_signal_status() {
    assert_child_progress_while_ordinary_is_gated(true).await;
}

async fn assert_child_progress_while_ordinary_is_gated(signaled: bool) {
    tokio::task::LocalSet::new()
        .run_until(async {
            let state = Arc::new(GatedExtensionState::default());
            ...
            agentos_vm::test_support::register_session(
                &mut engine.sidecar,
                connection_id,
                session_id,
            );
            ...
            let prepared = engine
                .sidecar
                .prepare_create_vm(&create_request, create)
                .expect("prepare VM");
            let completed = prepared.execute().await.expect("create child-progress VM");
            let dispatch = engine
                .sidecar
            ...
            let process_event_notify = engine.sidecar.process_event_notify();
            agentos_vm::test_support::insert_process(
                &mut engine.sidecar,
                &vm_id,
                "child-progress-root",
                false,
            );
            let attached_producer = agentos_vm::test_support::insert_child(
                &mut engine.sidecar,
                &vm_id,
                "child-progress-root",
                "attached-child",
            );
            let attached_completion = agentos_vm::test_support::capture_child(
                &mut engine.sidecar,
                &vm_id,
                "child-progress-root",
                "attached-child",
            );
            let detached_producer = agentos_vm::test_support::insert_process(
                &mut engine.sidecar,
                &vm_id,
                "detached-child",
                true,
            ...
            attached_producer.stdout(b"attached-progress");
            if signaled {
                attached_producer.signal_exit(15);
            } else {
                attached_producer.exit(0);
            }
            detached_producer.stdout(b"detached-progress");
            process_event_notify.notify_one();

            let attached = tokio::time::timeout(TEST_TIMEOUT, attached_completion)
                .await
            ...
            if signaled {
                assert_eq!(
                    attached.get("signal").and_then(serde_json::Value::as_str),
                    Some("SIGTERM")
                );
            } else {
                assert_eq!(
...
async fn request_concurrency_real_loop_child_service_saturation_rearms_exactly_once() {
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let producer = agentos_vm::test_support::insert_process(
                &mut engine.sidecar,
                &vm_id,
            ...
            for request_id in [711, 712] {
                let _reply = producer.rpc(
                    request_id,
                    "fs.readFile",
                    vec![serde_json::json!("/missing")],
                );
            }
...
async fn targeted_public_waiter_cannot_consume_internal_process_pump_wake() {
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let ownership =
                agentos_vm::protocol::OwnershipScope::vm(connection_id, session_id, vm_id.clone());
            ...
            let _reply = producer.rpc(721, "fs.readFile", vec![serde_json::json!("/missing")]);
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            assert_eq!(
                ...
                agentos_vm::ownership_coordinator::VmLifecyclePhase::Idle,
            );
            assert_eq!(
                vm_b.snapshot().lifecycle,
                agentos_vm::ownership_coordinator::VmLifecyclePhase::Idle,
