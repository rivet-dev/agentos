...
fn scan_workspace() -> Vec<ScannedConst> {
    ...
    if bridge.is_file() {
        scan_file(&bridge, "crates/execution/assets/v8-bridge.source.js", true, &mut found);
    }
...
fn load_inventory() -> Vec<InventoryEntry> {
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("tests/fixtures/limits-inventory.json");
    ...
    array
        ...
        .map(|entry| {
            ...
            let wired = entry.get("wired").and_then(|v| v.as_str()).map(str::to_string);
            InventoryEntry {
                name,
                path,
                class,
...
fn limit_constants_are_classified() {
    ...
    for entry in &inventory {
        if entry.class == "policy" {
            let wired_ok = entry.wired.as_deref().map(|w| !w.is_empty()).unwrap_or(false);
            if !wired_ok {
                failures.push(format!(
...
fn match_rule_unit_assertions() {
    ...
    // Exclusions.
    assert!(!name_qualifies("AGENT_OS_WASM_MAX_FUEL_ENV"));
