...
use crate::metrics::{DriverMetrics, ExecutorMetricClass};
...
impl VmExecutorAdmission {
    pub(crate) fn new(maximum: usize, metrics: DriverMetrics) -> Self {
        Self {
...
mod tests {
    ...
    fn requested_ceiling_uses_the_process_counter_and_never_raises_process_limit() {
        let admission = VmExecutorAdmission::new(3, DriverMetrics::new());
        let first = admission
    ...
    fn near_limit_warning_is_coalesced_and_rearmed_below_eighty_percent() {
        let admission = VmExecutorAdmission::new(5, DriverMetrics::new());
        let mut permits = (0..4)
