...
use crate::metrics::{DriverMetrics, FairnessLevel};
...
///
/// There is exactly one broker per [`crate::TokioDriver`]. A ready handle
...
impl FairWorkBroker {
    pub fn new(config: FairnessConfig, metrics: DriverMetrics) -> Result<Self, FairnessError> {
        Ok(Self {
...
mod tests {
    ...
    fn broker() -> FairWorkBroker {
        FairWorkBroker::new(config(), DriverMetrics::new()).expect("fair work broker")
    }
    ...
    async fn vm_generation_churn_reclaims_membership_and_compacts_tombstones() {
        ...
        let broker = FairWorkBroker::new(bounded, DriverMetrics::new()).expect("bounded broker");
