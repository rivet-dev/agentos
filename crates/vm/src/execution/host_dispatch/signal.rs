use super::*;
use agentos_vm_kernel::process_table::{SigmaskHow, SignalAction, SignalDisposition, SignalSet};
...
fn signal_delivery_value(delivery: agentos_vm_kernel::process_table::SignalDelivery) -> Value {
    json!({
        "signal": delivery.signal,
        "token": delivery.token,
        "flags": delivery.action.flags,
