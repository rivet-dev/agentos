...
fn enforce_dns_result_limit(maximum: usize, observed: usize) -> Result<(), VmError> {
    if observed > maximum {
        return Err(VmError::Host(HostServiceError::limit(
    ...
    BridgeError<B>: fmt::Debug + Send + Sync + 'static,
{
    let future: std::pin::Pin<
        Box<dyn std::future::Future<Output = Result<Value, VmError>> + Send>,
    > = match operation {
...
fn parse_dns_record_type(rrtype: &str) -> Result<RecordType, VmError> {
    match rrtype {
        "A" => Ok(RecordType::A),
...
fn dns_resolution_to_node_value(
    ...
    match requested_type {
        ...
        "SOA" => resolution
            ...
            .ok_or_else(|| VmError::Execution(String::from("failed to resolve DNS SOA record"))),
...
fn dns_resolution_safe_ip_set(
    records: &[Record],
    hostname: &str,
) -> Result<BTreeSet<IpAddr>, VmError> {
    let ips = records
        .iter()
