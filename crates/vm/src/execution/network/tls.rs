...
) -> Result<crate::state::LoopbackTlsEndpoint, VmError> {
...
impl crate::state::LoopbackTlsEndpoint {
    pub(in crate::execution) fn shutdown_write(&self) -> Result<(), VmError> {
        let mut state = self.pair.state.lock().map_err(|_| {
            VmError::InvalidState(String::from("loopback TLS transport lock poisoned"))
        })?;
    ...
    fn close_endpoint(&self) -> Result<(), VmError> {
        let mut state = self.pair.state.lock().map_err(|_| {
            VmError::InvalidState(String::from("loopback TLS transport lock poisoned"))
        })?;
        let peer_waker = if self.is_lower_socket {
...
pub(in crate::execution) fn parse_tls_client_hello_from_bytes(
    ...
    let Some(accepted) = acceptor.accept().map_err(|(error, _)| {
        VmError::Execution(format!("failed to parse TLS client hello: {error}"))
    })?
    else {
        return Ok(None);
    };
...
pub(in crate::execution) fn peek_loopback_tls_client_hello(
    ...
    let state = pair
        .state
        .lock()
        .map_err(|_| VmError::InvalidState(String::from("loopback TLS transport lock poisoned")))?;
...
fn tls_material_entries(material: &TlsMaterial) -> Result<Vec<Vec<u8>>, VmError> {
    match material {
...
fn tls_data_value(value: &TlsDataValue) -> Result<Vec<u8>, VmError> {
    match value {
        TlsDataValue::Buffer { data } => base64::engine::general_purpose::STANDARD
            .decode(data)
            .map_err(|error| {
                VmError::InvalidState(format!("TLS material contains invalid base64: {error}"))
            }),
...
fn tls_private_key_from_material(
    material: &TlsMaterial,
) -> Result<PrivateKeyDer<'static>, VmError> {
    for entry in tls_material_entries(material)? {
...
pub(in crate::execution) fn build_server_tls_config(
    options: &TlsBridgeOptions,
) -> Result<ServerConfig, VmError> {
    let certificates = tls_certificates_from_material(options.cert.as_ref().ok_or_else(|| {
        VmError::InvalidState(String::from("TLS server upgrade requires a certificate"))
    })?)?;
    let key = tls_private_key_from_material(options.key.as_ref().ok_or_else(|| {
        VmError::InvalidState(String::from("TLS server upgrade requires a private key"))
    })?)?;
...
pub(in crate::execution) fn reserve_socket_read_buffer(
    ...
    byte_quantum: usize,
) -> Result<(Vec<u8>, Reservation), VmError> {
...
) -> Result<(Vec<u8>, Reservation, Reservation), VmError> {
...
pub(crate) fn reserve_tls_write_payload(
    resources: &ResourceLedger,
    contents: &[u8],
) -> Result<TlsWritePayload, VmError> {
...
pub(in crate::execution) fn native_tls_role(
    ...
    let server_name = ServerName::try_from(server_name)
        .map_err(|_| VmError::InvalidState(String::from("invalid TLS servername")))?;
