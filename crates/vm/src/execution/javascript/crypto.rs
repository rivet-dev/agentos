...
pub(crate) fn service_javascript_crypto_sync_rpc(
    ...
    match request.method.as_str() {
        ...
        "crypto.scrypt" => {
            ...
            .map_err(|error| {
                VmError::InvalidState(format!("crypto.scrypt options are invalid: {error}"))
            })?;
            let mut output = vec![0u8; key_len];
            scrypt(&password, &salt, &params, &mut output)
                .map_err(|error| VmError::Execution(format!("crypto.scrypt failed: {error}")))?;
            Ok(Value::String(
                base64::engine::general_purpose::STANDARD.encode(output),
            ))
        }
...
fn service_javascript_crypto_hash_create_sync_rpc(
    ...
    let context = openssl::hash::Hasher::new(algorithm.message_digest()).map_err(|error| {
        VmError::InvalidState(format!("failed to create crypto hash session: {error}"))
    })?;
...
fn service_javascript_crypto_hash_update_sync_rpc(
    ...
    let session = process
        .hash_sessions
        .get_mut(&session_id)
        .ok_or_else(|| VmError::InvalidState(format!("Hash session {session_id} not found")))?;
    session.context.update(&data).map_err(|error| {
        VmError::InvalidState(format!("failed to update crypto hash session: {error}"))
    })?;
    Ok(Value::Null)
}
...
fn service_javascript_crypto_hash_final_sync_rpc(
    ...
    let digest = session.context.finish().map_err(|error| {
        VmError::InvalidState(format!("failed to finish crypto hash session: {error}"))
    })?;
    Ok(Value::String(
...
impl JavascriptCryptoDigestAlgorithm {
    ...
    fn hmac(self, key: &[u8], data: &[u8]) -> Result<Vec<u8>, VmError> {
        match self {
            Self::Md5 => {
                let mut mac = Hmac::<Md5>::new_from_slice(key)
                    .map_err(|error| VmError::InvalidState(format!("invalid HMAC key: {error}")))?;
                mac.update(data);
...
fn service_javascript_crypto_cipheriv_sync_rpc(request: &HostRpcRequest) -> Result<Value, VmError> {
...
fn service_javascript_crypto_cipheriv_update_sync_rpc(
    ...
    let session = process
        .cipher_sessions
        .get_mut(&session_id)
        .ok_or_else(|| VmError::InvalidState(format!("Cipher session {session_id} not found")))?;
...
fn service_javascript_crypto_cipheriv_final_sync_rpc(
    ...
    Ok(Value::String(serde_json::to_string(&response).map_err(
        |error| VmError::InvalidState(format!("serialize cipher final response: {error}")),
    )?))
}

fn service_javascript_crypto_sign_sync_rpc(request: &HostRpcRequest) -> Result<Value, VmError> {
...
fn service_javascript_crypto_verify_sync_rpc(request: &HostRpcRequest) -> Result<Value, VmError> {
    let algorithm = request.args.first().and_then(Value::as_str);
...
fn service_javascript_crypto_create_key_object_sync_rpc(
    ...
    Ok(Value::String(
        ...
        .map_err(|error| VmError::InvalidState(format!("serialize crypto key object: {error}")))?,
    ))
}
...
fn service_javascript_crypto_generate_key_pair_sync_rpc(
    ...
    Ok(Value::String(serde_json::to_string(&response).map_err(
        |error| VmError::InvalidState(format!("serialize generated key pair: {error}")),
    )?))
}
...
fn service_javascript_crypto_generate_key_sync_rpc(
    ...
    Ok(Value::String(serde_json::to_string(&serialized).map_err(
        |error| VmError::InvalidState(format!("serialize generated key: {error}")),
    )?))
}
...
fn service_javascript_crypto_generate_prime_sync_rpc(
    ...
    Ok(Value::String(serde_json::to_string(&payload).map_err(
        |error| VmError::InvalidState(format!("serialize generated prime: {error}")),
    )?))
}
...
fn service_javascript_crypto_diffie_hellman_sync_rpc(
    ...
    let private_key = javascript_crypto_parse_key_material_value(
        parsed.get("privateKey").ok_or_else(|| {
            VmError::InvalidState(String::from("crypto.diffieHellman missing privateKey"))
        })?,
        Some("private"),
    ...
    let public_key = javascript_crypto_parse_key_material_value(
        parsed.get("publicKey").ok_or_else(|| {
            VmError::InvalidState(String::from("crypto.diffieHellman missing publicKey"))
        })?,
        Some("public"),
    ...
    Ok(Value::String(
        ...
        .map_err(|error| VmError::InvalidState(format!("serialize derived secret: {error}")))?,
    ))
}
...
fn service_javascript_crypto_diffie_hellman_group_sync_rpc(
    ...
    Ok(Value::String(serde_json::to_string(&response).map_err(
        |error| VmError::InvalidState(format!("serialize diffieHellmanGroup response: {error}")),
    )?))
}
...
fn service_javascript_crypto_diffie_hellman_session_call_sync_rpc(
    ...
    let session = process
        ...
        .get_mut(&session_id)
        .ok_or_else(|| {
            VmError::InvalidState(format!("Diffie-Hellman session {session_id} not found"))
        })?;
    ...
    Ok(Value::String(
        ...
        .map_err(|error| {
            VmError::InvalidState(format!("serialize diffie session result: {error}"))
        })?,
    ))
}
...
fn service_javascript_crypto_subtle_sync_rpc(request: &HostRpcRequest) -> Result<Value, VmError> {
    let raw = javascript_sync_rpc_arg_str(&request.args, 0, "crypto.subtle request")?;
    let parsed: Value = serde_json::from_str(raw).map_err(|error| {
        VmError::InvalidState(format!("crypto.subtle request must be valid JSON: {error}"))
    })?;
    let op = parsed
        .get("op")
        .and_then(Value::as_str)
        .ok_or_else(|| VmError::InvalidState(String::from("crypto.subtle request missing op")))?;
    match op {
        "digest" => {
            let algorithm = parsed
                ...
                .and_then(Value::as_str)
                .ok_or_else(|| {
                    VmError::InvalidState(String::from("crypto.subtle.digest missing algorithm"))
                })?;
            let data = parsed.get("data").and_then(Value::as_str).ok_or_else(|| {
                VmError::InvalidState(String::from("crypto.subtle.digest missing data"))
            })?;
            let bytes = base64::engine::general_purpose::STANDARD
                .decode(data)
                .map_err(|error| {
                    VmError::InvalidState(format!("crypto.subtle.digest data base64: {error}"))
                })?;
            ...
            Ok(Value::String(
                ...
                .map_err(|error| {
                    VmError::InvalidState(format!("serialize crypto.subtle digest: {error}"))
                })?,
            ))
        }
        "generateKey" => {
            let algorithm = parsed.get("algorithm").ok_or_else(|| {
                VmError::InvalidState(String::from("crypto.subtle.generateKey missing algorithm"))
            })?;
            let name =
            ...
            Ok(Value::String(
                serde_json::to_string(&json!({ "key": key })).map_err(|error| {
                    VmError::InvalidState(format!("serialize crypto.subtle generated key: {error}"))
                })?,
            ))
        }
        "importKey" => {
            let format = parsed
                ...
                .and_then(Value::as_str)
                .ok_or_else(|| {
                    VmError::InvalidState(String::from("crypto.subtle.importKey missing format"))
                })?;
            if format != "raw" {
                return Err(VmError::InvalidState(format!(
            ...
            let key_data = parsed
                ...
                .and_then(Value::as_str)
                .ok_or_else(|| {
                    VmError::InvalidState(String::from("crypto.subtle.importKey missing keyData"))
                })?;
            ...
            let algorithm = parsed.get("algorithm").ok_or_else(|| {
                VmError::InvalidState(String::from("crypto.subtle.importKey missing algorithm"))
            })?;
            let key = javascript_crypto_serialize_subtle_secret_key(
                &raw,
                javascript_crypto_normalize_subtle_secret_algorithm(algorithm.clone(), &raw)?,
            ...
            Ok(Value::String(
                serde_json::to_string(&json!({ "key": key })).map_err(|error| {
                    VmError::InvalidState(format!("serialize crypto.subtle imported key: {error}"))
                })?,
            ))
        }
        "exportKey" => {
            let format = parsed
                ...
                .and_then(Value::as_str)
                .ok_or_else(|| {
                    VmError::InvalidState(String::from("crypto.subtle.exportKey missing format"))
                })?;
            if format != "raw" {
                return Err(VmError::InvalidState(format!(
            ...
            let raw = javascript_crypto_subtle_key_raw(
                parsed.get("key").ok_or_else(|| {
                    VmError::InvalidState(String::from("crypto.subtle.exportKey missing key"))
                })?,
            ...
            Ok(Value::String(
                ...
                .map_err(|error| {
                    VmError::InvalidState(format!("serialize crypto.subtle export: {error}"))
                })?,
            ))
        }
...
fn service_javascript_crypto_subtle_hmac_sync_rpc(
    ...
    let algorithm = parsed
        .get("algorithm")
        .ok_or_else(|| VmError::InvalidState(format!("crypto.subtle.{op} missing algorithm")))?;
    ...
    let hash = algorithm.get("hash").ok_or_else(|| {
        VmError::InvalidState(format!("crypto.subtle.{op} HMAC algorithm missing hash"))
    })?;
    let hash_name =
    ...
    let key = javascript_crypto_subtle_key_raw(
        parsed
            .get("key")
            .ok_or_else(|| VmError::InvalidState(format!("crypto.subtle.{op} missing key")))?,
    ...
    if op == "verify" {
        ...
        return Ok(Value::String(
            ...
            .map_err(|error| {
                VmError::InvalidState(format!("serialize crypto.subtle verify: {error}"))
            })?,
        ));
    }
    Ok(Value::String(
        ...
        .map_err(|error| VmError::InvalidState(format!("serialize crypto.subtle sign: {error}")))?,
    ))
}
...
fn javascript_crypto_subtle_base64_field(
    ...
    let value = parsed
        .get(field)
        .and_then(Value::as_str)
        .ok_or_else(|| VmError::InvalidState(format!("crypto.subtle.{op} missing {field}")))?;
    base64::engine::general_purpose::STANDARD
        .decode(value)
        .map_err(|error| {
            VmError::InvalidState(format!("crypto.subtle.{op} {field} base64: {error}"))
        })
}
...
fn javascript_crypto_subtle_algorithm_name<'a>(
    ...
    algorithm
        .get("name")
        .and_then(Value::as_str)
        .ok_or_else(|| VmError::InvalidState(format!("{label} algorithm missing name")))
}
...
fn javascript_crypto_normalize_subtle_secret_algorithm(
    ...
    let name = object
        ...
        .and_then(Value::as_str)
        .ok_or_else(|| {
            VmError::InvalidState(String::from("crypto.subtle secret algorithm missing name"))
        })?
        .to_string();
...
fn javascript_crypto_subtle_key_raw(key: &Value, label: &str) -> Result<Vec<u8>, VmError> {
    let raw = key
        .get("_raw")
        .and_then(Value::as_str)
        .ok_or_else(|| VmError::InvalidState(format!("{label} must be a raw secret CryptoKey")))?;
    base64::engine::general_purpose::STANDARD
        .decode(raw)
        .map_err(|error| VmError::InvalidState(format!("{label} raw base64: {error}")))
}
...
fn service_javascript_crypto_subtle_aes_crypt_sync_rpc(
    ...
    let iv = algorithm
        .get("iv")
        .and_then(Value::as_str)
        .ok_or_else(|| VmError::InvalidState(format!("crypto.subtle.{op} {name} missing iv")))?;
    let iv = base64::engine::general_purpose::STANDARD
        .decode(iv)
        .map_err(|error| VmError::InvalidState(format!("crypto.subtle.{op} iv base64: {error}")))?;
    let data = parsed
        .get("data")
        .and_then(Value::as_str)
        .ok_or_else(|| VmError::InvalidState(format!("crypto.subtle.{op} missing data")))?;
    let mut data = base64::engine::general_purpose::STANDARD
        .decode(data)
        .map_err(|error| {
            VmError::InvalidState(format!("crypto.subtle.{op} data base64: {error}"))
        })?;
    if name == "AES-CBC" {
        if iv.len() != 16 {
            return Err(VmError::InvalidState(format!(
        ...
        return Ok(Value::String(
            ...
            .map_err(|error| {
                VmError::InvalidState(format!("serialize crypto.subtle {op}: {error}"))
            })?,
        ));
    }
    ...
    Ok(Value::String(
        ...
        .map_err(|error| VmError::InvalidState(format!("serialize crypto.subtle {op}: {error}")))?,
    ))
}

fn javascript_crypto_subtle_aes_gcm_tag_len(algorithm: &Value) -> Result<usize, VmError> {
    ...
    usize::try_from(tag_bits / 8).map_err(|_| {
        VmError::InvalidState(String::from("crypto.subtle AES-GCM tagLength too large"))
    })
}
...
fn service_javascript_crypto_cipheriv_inner(
    ...
    Ok(Value::String(serde_json::to_string(&response).map_err(
        |error| VmError::InvalidState(format!("serialize {label} response: {error}")),
    )?))
}
...
fn javascript_crypto_parse_direct_key_input(
    ...
    expected: Option<&str>,
    label: &str,
) -> Result<JavascriptDirectKeyInput, VmError> {
...
fn javascript_crypto_parse_key_from_pem(
    ...
    match expected {
        Some("private") => PKey::private_key_from_pem(pem)
            ...
            .map_err(|error| {
                VmError::InvalidState(format!("{label} private key is invalid: {error}"))
            }),
        Some("public") => PKey::public_key_from_pem(pem)
            .map(JavascriptCryptoKeyMaterial::Public)
            .map_err(|error| {
                VmError::InvalidState(format!("{label} public key is invalid: {error}"))
            }),
        _ => PKey::private_key_from_pem(pem)
            .map(JavascriptCryptoKeyMaterial::Private)
            .or_else(|_| PKey::public_key_from_pem(pem).map(JavascriptCryptoKeyMaterial::Public))
            .map_err(|error| VmError::InvalidState(format!("{label} PEM key is invalid: {error}"))),
    }
}
...
fn javascript_crypto_parse_key_from_bytes(
    ...
    match (format.unwrap_or("der"), kind.or(expected)) {
        ("der", Some("pkcs8")) | ("der", Some("private")) => PKey::private_key_from_der(der)
            .map(JavascriptCryptoKeyMaterial::Private)
            .map_err(|error| {
                VmError::InvalidState(format!("{label} private key DER is invalid: {error}"))
            }),
        ("der", Some("spki")) | ("der", Some("public")) => PKey::public_key_from_der(der)
            .map(JavascriptCryptoKeyMaterial::Public)
            .map_err(|error| {
                VmError::InvalidState(format!("{label} public key DER is invalid: {error}"))
            }),
        _ => Err(VmError::InvalidState(format!(
...
fn javascript_crypto_parse_serialized_key_object(
    ...
    let serialized: JavascriptSerializedSandboxKeyObject = serde_json::from_value(value.clone())
        .map_err(|error| VmError::InvalidState(format!("{label} keyObject is invalid: {error}")))?;
    match serialized.kind.as_str() {
        ...
        "private" => {
            let pem = serialized.pem.ok_or_else(|| {
                VmError::InvalidState(format!("{label} private keyObject is missing pem"))
            })?;
            javascript_crypto_parse_key_from_pem(pem.as_bytes(), Some("private"), label)
        }
        "public" => {
            let pem = serialized.pem.ok_or_else(|| {
                VmError::InvalidState(format!("{label} public keyObject is missing pem"))
            })?;
...
fn javascript_crypto_new_signer<'a>(
    ...
    Signer::new(
        javascript_crypto_message_digest_from_name(algorithm.ok_or_else(|| {
            VmError::InvalidState(String::from("crypto.sign requires a digest algorithm"))
        })?)?,
        key,
    )
    .map_err(javascript_crypto_openssl_error)
...
fn javascript_crypto_new_verifier<'a>(
    algorithm: Option<&'a str>,
    key: &'a PKey<Public>,
) -> Result<Verifier<'a>, VmError> {
    if matches!(key.id(), PKeyId::ED25519 | PKeyId::ED448) || algorithm.is_none() {
        return Verifier::new_without_digest(key).map_err(javascript_crypto_openssl_error);
    }
    Verifier::new(
        javascript_crypto_message_digest_from_name(algorithm.ok_or_else(|| {
            VmError::InvalidState(String::from("crypto.verify requires a digest algorithm"))
        })?)?,
        key,
    )
    .map_err(javascript_crypto_openssl_error)
...
fn javascript_crypto_message_digest_from_name(name: &str) -> Result<MessageDigest, VmError> {
...
fn javascript_crypto_padding_from_value(value: &Value) -> Result<Option<Padding>, VmError> {
...
fn javascript_crypto_decode_bridge_buffer(value: &Value, label: &str) -> Result<Vec<u8>, VmError> {
...
fn javascript_crypto_serialize_sandbox_key_object(
    ...
    let serialized = match key {
        JavascriptCryptoKeyMaterial::Private(key) => JavascriptSerializedSandboxKeyObject {
            ...
            pem: Some(
                ...
                .map_err(|error| {
                    VmError::InvalidState(format!("private key PEM is not utf8: {error}"))
                })?,
            ),
            raw: None,
        ...
        JavascriptCryptoKeyMaterial::Public(key) => JavascriptSerializedSandboxKeyObject {
            ...
            pem: Some(
                ...
                .map_err(|error| {
                    VmError::InvalidState(format!("public key PEM is not utf8: {error}"))
                })?,
            ),
            raw: None,
            asymmetric_key_type: javascript_crypto_pkey_type_name(key.id()),
    ...
    serde_json::to_value(serialized)
        .map_err(|error| VmError::InvalidState(format!("serialize key object: {error}")))
}
...
fn javascript_crypto_rsa_output_size(key: &JavascriptCryptoKeyMaterial) -> Result<usize, VmError> {
    match key {
        ...
        JavascriptCryptoKeyMaterial::Public(key) => key
            ...
            .map(|rsa| rsa.size() as usize)
            .map_err(javascript_crypto_openssl_error),
        JavascriptCryptoKeyMaterial::Secret(_) => Err(VmError::InvalidState(String::from(
...
fn javascript_crypto_u32_from_bridge_value(value: &Value, label: &str) -> Result<u32, VmError> {
...
fn javascript_crypto_curve_nid(name: &str) -> Result<Nid, VmError> {
    match name {
...
fn javascript_crypto_named_dh_group(name: &str) -> Result<Dh<Params>, VmError> {
    match name {
...
fn javascript_crypto_clone_dh_params(params: &Dh<Params>) -> Result<Dh<Params>, VmError> {
    Dh::from_pqg(
        params
            .prime_p()
            .to_owned()
...
fn javascript_crypto_build_dh_params(args: &[Value]) -> Result<Dh<Params>, VmError> {
    ...
    match method {
        ...
        "computeSecret" => {
            ...
            let peer = javascript_crypto_bignum_from_bridge_value(
                args.first().ok_or_else(|| {
                    VmError::InvalidState(String::from("computeSecret requires peer public key"))
                })?,
        ...
        "setPrivateKey" => {
            let private_key = javascript_crypto_bignum_from_bridge_value(
                args.first().ok_or_else(|| {
                    VmError::InvalidState(String::from("setPrivateKey requires private key"))
                })?,
        ...
        "setPublicKey" => {
            let public_key = javascript_crypto_bignum_from_bridge_value(
                args.first().ok_or_else(|| {
                    VmError::InvalidState(String::from("setPublicKey requires public key"))
                })?,
    ...
    if let Some(encoding) = encoding {
        ...
        return Ok(match format {
            ...
            _ => json!({
                ...
                .map_err(|error| VmError::InvalidState(format!("public key PEM utf8: {error}")))?,
            }),
        });
    }
    ...
    if let Some(encoding) = encoding {
        ...
        return Ok(match format {
            ...
            _ => json!({
                ...
                .map_err(|error| VmError::InvalidState(format!("private key PEM utf8: {error}")))?,
            }),
        });
    }
...
fn javascript_crypto_cipher_error(error: AesCipherError) -> VmError {
    VmError::InvalidState(error.0)
}
...
fn javascript_crypto_decode_cipher_option_b64(
    ...
    base64::engine::general_purpose::STANDARD
        ...
        .map(Some)
        .map_err(|error| {
            VmError::InvalidState(format!("cipher {field} contains invalid base64: {error}"))
        })
}
...
fn javascript_crypto_build_cipher_session(
    ...
    decrypt: bool,
    options: Option<&Value>,
) -> Result<StreamCipherSession, VmError> {
    let pad = options
...
fn javascript_crypto_requested_aead_tag_len(
    ...
    usize::try_from(requested).map_err(|_| {
        VmError::InvalidState(String::from("cipher authTagLength must fit within usize"))
    })
}
...
fn javascript_crypto_openssl_error(error: openssl::error::ErrorStack) -> VmError {
    VmError::Execution(format!("crypto operation failed: {error}"))
}
