...
/// never retains `VmManager` or a `VmState` borrow across an await.
    ...
    BridgeError<B>: fmt::Debug + Send + Sync + 'static,
{
    ...
    let prepared = (|| {
        let command = request.command.clone().ok_or_else(|| {
            VmError::InvalidState(String::from("python subprocessRun requires a command"))
        })?;
        let (internal_bootstrap_env, cwd, max_buffer) =
            vm.try_command("prepare owned Python subprocess", |vm| {
                ...
                let process = VmManager::<B>::active_process_by_path(root, &path)
                ...
                Ok((
                    ...
                    cwd,
                    VmManager::<B>::child_process_sync_max_buffer(process, request.max_buffer)?,
                ))
            })?;
        let spawn = VmManager::<B>::build_owned_descendant_javascript_child_process_spawn(
            vm.clone(),
            vm_id.clone(),
            process_id.clone(),
            child_path.clone(),
    ...
    Box::pin(async move {
        ...
        vm.try_command("close owned Python subprocess stdin", |vm| {
            ...
            let root = active_processes
                ...
                .ok_or_else(|| missing_process_error(&vm_id, &process_id))?;
            let path = child_path.iter().map(String::as_str).collect::<Vec<_>>();
            let process = VmManager::<B>::active_process_by_path_mut(root, &path)
                .ok_or_else(|| javascript_child_process_gone_error(&process_id, &path))?;
            let child = process
                .child_processes
    ...
    async fn cancelled_python_subprocess_registration_removes_spawned_child() {
        tokio::task::LocalSet::new()
            .run_until(async {
                ...
                let runtime = agentos_driver_tokio::SidecarRuntime::process(&config.runtime)
