...
fn python_socket_host(request: &PythonVfsRpcRequest) -> Result<String, VmError> {
    request
        .hostname
        .clone()
        .ok_or_else(|| VmError::InvalidState(String::from("python socket op requires a host")))
}

fn python_socket_port(request: &PythonVfsRpcRequest) -> Result<u16, VmError> {
    request
        .port
        .ok_or_else(|| VmError::InvalidState(String::from("python socket op requires a port")))
}
...
) -> Result<PythonSocketPayload, VmError> {
...
fn decode_python_socket_payload(
    ...
    let bytes = base64::engine::general_purpose::STANDARD
        .decode(body)
        .map_err(|error| {
            VmError::InvalidState(format!("invalid base64 python socket payload: {error}"))
        })?;
...
pub(in crate::execution) fn python_socket_id(
    ...
    request.socket_id.ok_or_else(|| {
        VmError::InvalidState(String::from("python socket op requires socketId"))
    })
}

fn python_socket_missing_error(socket_id: u64) -> VmError {
    VmError::Execution(format!("EBADF: unknown python socket {socket_id}"))
}

fn python_socket_backend_missing_error(socket_id: u64) -> VmError {
...
fn consume_python_tcp_pending_read(
    ...
    max: usize,
    resources: &ResourceLedger,
) -> Result<Option<PythonSocketImmediate>, VmError> {
...
fn python_tcp_event_response(
    ...
    max: usize,
    resources: &ResourceLedger,
) -> Result<PythonSocketResponse, VmError> {
    match event {
        ...
        Some(JavascriptTcpSocketEvent::Error { code, message }) => {
            ...
            Err(VmError::Execution(format!("{code}: {message}")))
        }
...
fn encode_python_socket_bytes(
    bytes: &[u8],
    resources: &ResourceLedger,
) -> Result<(String, Reservation), VmError> {
...
fn python_socket_completion_dropped_error() -> VmError {
    VmError::Execution(String::from(
...
fn respond_python_socket_async(
    ...
    request_id: u64,
    response: Result<PythonVfsRpcResponsePayload, VmError>,
) {
...
fn python_socket_kind_error(op: &str, expected: &str) -> VmError {
    ...
    BridgeError<B>: fmt::Debug + Send + Sync + 'static,
{
                ...
                runtime
                    .spawn(agentos_driver_tokio::TaskClass::Socket, async move {
                        let response = match response.await {
                            ...
                            Ok(Err(error)) => Err(VmError::Execution(format!(
                                "{}: {}",
    ...
    fn python_socket_async_context(
        &self,
        vm_id: &str,
    ) -> Result<(agentos_driver_tokio::DriverHandle, PythonVfsRpcResponder), VmError> {
        let vm = self.vms.get(vm_id).ok_or_else(|| missing_vm_error(vm_id))?;
...
mod python_socket_accounting_tests {
    use super::{
        ...
        PythonSocketConnectResponder, PythonVfsRpcResponsePayload, VmError,
    };
    ...
    struct CapturingConnectResponder {
        response: RefCell<Option<(u64, Result<PythonVfsRpcResponsePayload, VmError>)>>,
    }
