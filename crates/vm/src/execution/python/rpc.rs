...
/// the protocol router's `VmManager`.
    ...
    BridgeError<B>: fmt::Debug + Send + Sync + 'static,
{
    match request.method {
        ...
        PythonVfsRpcMethod::SubprocessRun => Err(VmError::InvalidState(String::from(
    ...
    BridgeError<B>: fmt::Debug + Send + Sync + 'static,
{
    ...
    let prepared = (|| {
        ...
        let url_text = request.url.clone().ok_or_else(|| {
            VmError::InvalidState(String::from("python httpRequest requires a url"))
        })?;
        let url = Url::parse(&url_text)
            .map_err(|error| VmError::Execution(format!("ERR_INVALID_URL: {error}")))?;
        let host = url.host_str().ok_or_else(|| {
            VmError::Execution(String::from("ERR_INVALID_URL: missing host"))
        })?;
        let port = url.port_or_known_default().ok_or_else(|| {
            VmError::Execution(String::from("ERR_INVALID_URL: missing port"))
        })?;
    ...
    let response = (|| {
        let vm = vm.try_read("service Python DNS RPC", |state| {
            ...
            let hostname = request.hostname.as_deref().ok_or_else(|| {
                VmError::InvalidState(String::from("python dnsLookup requires a hostname"))
            })?;
    ...
    BridgeError<B>: fmt::Debug + Send + Sync + 'static,
{
    ...
    fn handle_python_http_rpc_request(
        ...
        let response = (|| {
            ...
            // rehost_function DNS server cannot make the second (TLS/TCP) lookup land
