...
use agentos_driver_tokio::DriverConfig;
...
pub struct OwnershipCoordinatorLimits {
...
impl OwnershipCoordinatorLimits {
    pub fn from_runtime_config(config: &DriverConfig) -> Self {
        Self {
...
#[cfg(any(test, feature = "test-support"))]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct EntityCoordinatorSnapshot {
...
#[cfg(any(test, feature = "test-support"))]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct VmCoordinatorSnapshot {
    pub phase: CoordinatorPhase,
    pub active_operations: usize,
    pub lifecycle: VmLifecyclePhase,
}

#[derive(Clone, Debug, PartialEq, Eq)]
pub enum OwnershipCoordinatorError {
    Limit {
        scope: &'static str,
        current: usize,
        limit: usize,
...
#[derive(Debug)]
pub enum InternalVmEventAdmission {
...
impl OwnershipCoordinator {
    pub fn new(limits: OwnershipCoordinatorLimits) -> Self {
        limits.validate();
        Self {
    ...
    pub fn begin_vm_disposal_with_operations(
        &self,
        ownership: &OwnershipScope,
        reason: OperationCancellationReason,
    ...
    pub fn begin_connection_disposal(
        &self,
        connection_id: &str,
        reason: OperationCancellationReason,
...
impl ConnectionCoordinator {
    ...
    #[cfg(any(test, feature = "test-support"))]
    pub fn snapshot(&self) -> EntityCoordinatorSnapshot {
...
impl SessionCoordinator {
    ...
    pub fn vm(&self, vm_id: &str) -> Result<VmCoordinator, OwnershipCoordinatorError> {
        let root = upgrade_root(&self.root, "session", &self.label())?;
        let state = lock(&root.state, "ownership membership");
        let session = matching_session(&state, self)?;
        let vm = session
...
impl VmCoordinator {
    #[cfg(any(test, feature = "test-support"))]
    pub fn snapshot(&self) -> VmCoordinatorSnapshot {
        let phase = self
            .root
            .upgrade()
            .and_then(|root| {
...
#[derive(Debug)]
pub struct CoordinatorOperationPermit {
...
impl VmDisposal {
    ...
    pub fn complete(mut self) -> Result<(), OwnershipCoordinatorError> {
        if let Some(drain) = &self.operation_drain {
