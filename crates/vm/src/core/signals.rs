...
use agentos_vm_host_interface::ExecutionSignal;
...
pub fn parse_process_signal_state_request(
    args: &[Value],
) -> Result<(u32, SignalHandlerRegistration), crate::core::SidecarCoreError> {
    ...
    let mask: Vec<u32> = serde_json::from_str(mask_json).map_err(|error| {
        crate::core::SidecarCoreError::typed(
            "EINVAL",
    ...
    let action = match action.trim().to_ascii_lowercase().as_str() {
        ...
        other => {
            return Err(crate::core::SidecarCoreError::typed(
                "EINVAL",
...
fn validate_process_signal_number(
    signal: u32,
    label: &str,
) -> Result<(), crate::core::SidecarCoreError> {
    if is_valid_posix_signal_number(signal) {
        Ok(())
    } else {
        Err(crate::core::SidecarCoreError::typed(
            "EINVAL",
...
fn signal_state_u32_arg(
    ...
    index: usize,
    label: &str,
) -> Result<u32, crate::core::SidecarCoreError> {
    let value = args.get(index).ok_or_else(|| {
        crate::core::SidecarCoreError::typed("EINVAL", format!("{label} missing"))
    })?;
    if let Some(value) = value.as_u64() {
        return u32::try_from(value).map_err(|_| {
            crate::core::SidecarCoreError::typed("EINVAL", format!("{label} must fit in u32"))
        });
    }
    ...
    if let Some(value) = value.as_str() {
        return value.parse::<u32>().map_err(|error| {
            crate::core::SidecarCoreError::typed("EINVAL", format!("{label}: {error}"))
        });
    }
    Err(crate::core::SidecarCoreError::typed(
        "EINVAL",
...
fn signal_state_str_arg<'a>(
    ...
    index: usize,
    label: &str,
) -> Result<&'a str, crate::core::SidecarCoreError> {
    args.get(index).and_then(Value::as_str).ok_or_else(|| {
        crate::core::SidecarCoreError::typed("EINVAL", format!("{label} must be a string"))
    })
}
