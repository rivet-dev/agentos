...
use agentos_vm_kernel::dns::DnsLookupPolicy;
use agentos_vm_kernel::kernel::{KernelError, KernelVm};
use agentos_vm_kernel::poll::{
    PollEvents, PollTargetEntry, POLLERR, POLLHUP, POLLIN, POLLNVAL, POLLOUT,
};
use agentos_vm_kernel::socket_table::{InetSocketAddress, SocketId, SocketShutdown, SocketSpec};
    ...
    F: VirtualFileSystem + 'static,
{
    ...
    let response = match operation {
        ...
        other if crate::core::guest_pty::is_pty_operation(other) => {
            crate::core::guest_pty::dispatch_pty_operation(
                kernel,
                pid,
                requester_driver,
                other,
