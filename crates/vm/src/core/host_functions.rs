use super::limits::HostFunctionLimits;
...
impl fmt::Display for HostFunctionRegistrationError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ResourceLimit { used, requested, limit, config_path } => write!(f,
                "ERR_AGENTOS_RESOURCE_LIMIT: {config_path} used={used} requested={requested} limit={limit}; raise {config_path} to increase the limit"),
        }
    }
}
...
pub fn validate_host_functions_registration(
    ...
    validate_host_functions_registration_with_limits(payload, &HostFunctionLimits::default())
}

pub fn validate_host_functions_registration_with_limits(
    payload: &RegisterHostCallbacksRequest,
    limits: &HostFunctionLimits,
) -> Result<(), HostFunctionRegistrationError> {
    ...
    check_limit(
        ...
        limits.max_functions_per_collection,
    ...
    for (host_function_name, host_function) in &payload.callbacks {
        ...
        if let Some(timeout_ms) = host_function.timeout_ms {
            check_limit(
                ...
                usize::try_from(limits.max_timeout_ms).unwrap_or(usize::MAX),
        ...
        check_limit(
            ...
            limits.max_examples_per_function,
        ...
        for (index, example) in host_function.examples.iter().enumerate() {
            ...
            validate_json_byte_length(
                ...
                &example_input,
                limits.max_example_input_bytes,
...
pub fn ensure_host_function_registry_capacity(
    ...
    ensure_host_function_registry_capacity_with_limits(
        host_functions,
        payload,
        &HostFunctionLimits::default(),
    )
}

pub fn ensure_host_function_registry_capacity_with_limits(
    host_functions: &BTreeMap<String, RegisterHostCallbacksRequest>,
    payload: &RegisterHostCallbacksRequest,
    limits: &HostFunctionLimits,
) -> Result<(), HostFunctionRegistrationError> {
    check_limit(
        host_functions.len(),
        1,
        limits.max_registered_collections,
        "limits.hostFunctions.maxRegisteredCollections",
    )?;
    let registered = host_functions
        .values()
    ...
    check_limit(
        registered,
        payload.callbacks.len(),
        limits.max_registered_functions_per_vm,
...
fn check_limit(
    ...
    let total = used.checked_add(requested);
    if total.is_none_or(|total| total > limit) {
        return Err(HostFunctionRegistrationError::ResourceLimit {
            used,
            requested,
            limit,
            config_path,
    ...
    let total = total.expect("checked limit usage");
    if total.saturating_mul(5) >= limit.saturating_mul(4) {
        tracing::warn!(
            used = total,
            limit,
            configuration_path = config_path,
            "host-function resource approaches its configured limit"
        );
    }
    Ok(())
}
...
fn validate_json_byte_length(
    ...
    let length = serde_json::to_vec(value)
        ...
        .len();
    check_limit(0, length, limit, config_path)
}
