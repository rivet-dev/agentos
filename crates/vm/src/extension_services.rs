...
use crate::execution::{
    internal_event_reply, service_owned_child_bridge_event, OwnedChildBridgeEventService,
    OwnedHostEventService,
};
...
type ExtensionServiceCompletionMutation = Box<
    dyn FnOnce(&mut VmManager<LocalBridge>) -> Option<PreparedExtensionServiceCommand> + 'static,
>;
type ExtensionServiceCommandFuture =
    Pin<Box<dyn Future<Output = ExtensionServiceCompletionMutation> + 'static>>;

/// Command work that no longer borrows the protocol router's `VmManager`.
///
...
pub struct PreparedExtensionServiceCommand {
...
pub struct CompletedExtensionServiceCommand {
    ...
    failure_reply: Box<dyn FnOnce(VmError) + 'static>,
...
pub enum VmEventAdmissionResult {
...
impl PreparedExtensionServiceCommand {
    ...
    pub fn admit_vm_event_nowait(mut self) -> Result<VmEventAdmissionResult, VmError> {
        ...
        if !matches!(
            ...
            (self.panic_reply)(VmError::InvalidState(message.clone()));
            return Err(VmError::InvalidState(message));
        }
    ...
    pub fn cancel_before_schedule(self, reason: OperationCancellationReason) {
        ...
        panic_reply(VmError::InvalidState(format!(
            "ERR_AGENTOS_EXTENSION_SERVICE_CANCELLED: extension service {operation} cancelled before task admission: {reason:?}"
        )));
        drop(admission);
    }

    /// Called only after VM admission succeeds, on the coordinator thread.
    pub fn prepare_admitted(
        &mut self,
        sidecar: &mut VmManager<LocalBridge>,
    ) -> Result<(), VmError> {
        if self.deferred_prepare.is_none() {
            return Ok(());
        }
        let admitted = self
            .admission
            .as_ref()
            .is_some_and(|admission| admission.pre_admitted.is_some());
        if !admitted {
            return Err(VmError::InvalidState("ERR_AGENTOS_PROCESS_EVENT_NOT_ADMITTED: host event preparation requires an active VM permit".into()));
        }
        if let Some(reason) = self
            .admission
            .as_ref()
            .and_then(|admission| admission.cancellation.reason())
        {
            let error = VmError::InvalidState(format!("ERR_AGENTOS_EXTENSION_SERVICE_CANCELLED: host event cancelled before preparation: {reason:?}"));
            let fail = std::mem::replace(
                &mut self.panic_reply,
                Box::new(
                    |error| tracing::error!(%error, "host event already rejected during preparation"),
                ),
            );
            fail(VmError::InvalidState(error.to_string()));
            return Err(error);
        }
        if let Some(prepare) = self.deferred_prepare.take() {
            match catch_unwind(AssertUnwindSafe(|| prepare(sidecar))) {
                Ok(future) => self.future = future,
                Err(_) => {
                    let error = VmError::InvalidState("ERR_AGENTOS_PROCESS_EVENT_PREPARE_PANIC: admitted host event preparation panicked".into());
                    let fail = std::mem::replace(
                        &mut self.panic_reply,
                        Box::new(
                            |error| tracing::error!(%error, "host event already rejected during preparation"),
    ...
    pub async fn execute_supervised(self) -> Option<CompletedExtensionServiceCommand> {
        let PreparedExtensionServiceCommand {
            operation,
            future,
            panic_reply,
        ...
        if deferred_prepare.is_some() {
            panic_reply(VmError::InvalidState("ERR_AGENTOS_PROCESS_EVENT_NOT_PREPARED: admitted host event was scheduled before coordinator preparation".into()));
            return None;
        }
                ...
                match admission.pre_admitted.take() {
                    ...
                    None => {
                        ...
                        match admitted {
                            ...
                            Err(error) => {
                                panic_reply(VmError::InvalidState(error.to_string()));
                                return None;
                            }
                        }
                    }
        ...
        match joined {
            ...
            Err(error) => {
                let failure = VmError::Execution(format!(
...
impl<T: 'static> SharedReply<T> {
    ...
    fn send(&self, result: Result<T, VmError>, operation: &str) {
...
where
    T: 'static,
    F: FnOnce() -> Fut + 'static,
    Fut: Future<Output = Result<T, VmError>> + 'static,
{
    ...
    PreparedExtensionServiceCommand {
        ...
        future: Box::pin(async move {
            ...
            Box::new(move |_sidecar: &mut VmManager<LocalBridge>| {
...
where
    T: 'static,
    F: FnOnce(&mut VmManager<LocalBridge>) -> Result<T, VmError> + 'static,
{
    let reply = SharedReply::new(reply);
    let completion_reply = reply.clone();
    let panic_reply = reply.clone();
    PreparedExtensionServiceCommand {
        operation,
        future: Box::pin(async move {
            Box::new(move |sidecar: &mut VmManager<LocalBridge>| {
...
pub fn prepared_test_service_command<Fut>(
    operation: &'static str,
    future: Fut,
) -> (
    PreparedExtensionServiceCommand,
    oneshot::Receiver<Result<(), VmError>>,
)
where
...
#[cfg(any(test, feature = "test-support"))]
pub fn prepared_test_vm_service_command<Fut>(
    operation: &'static str,
    coordinator: &OwnershipCoordinator,
    ownership: &OwnershipScope,
    future: Fut,
...
fn prepare_owned_host_event_service(
    _sidecar: &mut VmManager<LocalBridge>,
    coordinator: &OwnershipCoordinator,
    target: OwnedHostEventService,
) -> PreparedExtensionServiceCommand {
    let ownership = target.ownership.clone();
    let completion_reply = internal_event_reply(&target.event);
    let panic_reply = completion_reply.clone();
    let operation = "service_host_process_event";
    let prepared = PreparedExtensionServiceCommand {
        operation,
        future: Box::pin(std::future::pending()),
        deferred_prepare: Some(Box::new(move |sidecar| {
            let service = sidecar.prepare_owned_host_event_service(target);
            Box::pin(async move {
                let result = service.await;
                Box::new(move |_sidecar: &mut VmManager<LocalBridge>| {
                    if let Err(error) = result {
                        tracing::error!(operation, %error, "owned host event service failed");
                        if let Some(reply) = completion_reply {
                            if let Err(reply_error) =
                                reply.fail(crate::executor::backend::HostServiceError::new(
                                    "ERR_AGENTOS_PROCESS_EVENT_SERVICE",
                                    error.to_string(),
                                ))
                            {
                                tracing::error!(%reply_error, "failed to settle host event service failure");
                            }
                        }
                    }
                    None
        ...
        panic_reply: Box::new(move |error| {
            tracing::error!(operation, %error, "owned host event service cancelled or panicked");
            if let Some(reply) = &panic_reply {
                if let Err(reply_error) =
                    reply.fail(crate::executor::backend::HostServiceError::new(
                        "ERR_AGENTOS_PROCESS_EVENT_SERVICE",
                        error.to_string(),
                ...
                {
                    tracing::error!(%reply_error, "failed to settle cancelled host event service");
                }
            }
        }),
        admission: None,
...
pub enum OwnedProcessEventService {
    Host(OwnedHostEventService),
    ChildBridge(OwnedChildBridgeEventService),
}

pub fn prepare_owned_process_event_service(
    sidecar: &mut VmManager<LocalBridge>,
    coordinator: &OwnershipCoordinator,
    ...
    match target {
        OwnedProcessEventService::Host(target) => {
            prepare_owned_host_event_service(sidecar, coordinator, target)
        }
...
pub enum ExtensionServiceCommand {
    VmDatabase {
        ownership: OwnershipScope,
...
#[derive(Clone)]
pub struct RoutedExtensionServices {
...
impl CompletedProcessEventStore {
    ...
    fn take(&self, target: &ProcessEventTarget) -> Result<Option<EventFrame>, VmError> {
    ...
    async fn reserve(&self) -> Result<OwnedSemaphorePermit, VmError> {
...
impl RoutedExtensionServices {
    ...
    pub fn new_with_process_event_broker_and_progress(
        commands: mpsc::Sender<ExtensionServiceCommand>,
        progress_commands: mpsc::Sender<ExtensionServiceCommand>,
        routed_process_event_notify: Arc<Notify>,
                ...
                let lease = if routed {
                    if timeout.is_zero() {
                        match tokio::time::timeout(Duration::ZERO, waiter.next_lease()).await {
                            Ok(result) => Some(
                                result.map_err(|error| VmError::InvalidState(error.to_string()))?,
                            ),
                            Err(_) => None,
                        }
                    } else {
...
fn unexpected_service_response(operation: &str, payload: ResponsePayload) -> VmError {
    match payload {
...
fn prepare_next_extension_vm_disposal(
        ...
        .begin_vm_disposal(&ownership, OperationCancellationReason::Explicit)
    {
        ...
        Err(error) => {
            state.reply.send(
                Err(VmError::InvalidState(error.to_string())),
...
pub fn prepare_extension_service_command(
    sidecar: &mut VmManager<LocalBridge>,
    coordinator: &OwnershipCoordinator,
...
mod internal_event_lifecycle_tests {
    ...
    fn prepared_internal_event(
        ...
        let prepared = PreparedExtensionServiceCommand {
            ...
            future: Box::pin(async move {
                drop(reservation);
                Box::new(|_sidecar: &mut VmManager<LocalBridge>| None)
                    as ExtensionServiceCompletionMutation
            }),
    ...
    #[test]
    fn host_event_preparation_requires_admission_and_runs_once() {
        let config = crate::VmManagerConfig::default();
        let driver =
            agentos_driver_tokio::TokioDriver::process(&config.runtime).expect("test driver");
        let mut manager = VmManager::with_config_extensions_and_runtime(
            LocalBridge::default(),
            config.clone(),
            Vec::new(),
            driver.handle(),
        ...
        coordinator
            ...
            .expect("session")
            .open_vm("vm-a")
            .expect("VM membership");
        let calls = Arc::new(AtomicUsize::new(0));
        let mut command = prepared_internal_event(
            &coordinator,
            &OwnershipScope::vm("connection-a", "session-a", "vm-a"),
            Arc::new(AtomicUsize::new(0)),
            Arc::new(AtomicUsize::new(0)),
        );
        let prepared_calls = calls.clone();
        command.deferred_prepare = Some(Box::new(move |_| {
            prepared_calls.fetch_add(1, Ordering::AcqRel);
            Box::pin(async {
                Box::new(|_: &mut VmManager<LocalBridge>| None)
                    as ExtensionServiceCompletionMutation
            })
        }));
        assert!(command.prepare_admitted(&mut manager).is_err());
        assert_eq!(calls.load(Ordering::Acquire), 0);
        let VmEventAdmissionResult::Admitted(mut command) =
            command.admit_vm_event_nowait().expect("admit host event")
        else {
            panic!("uncontended event must be admitted");
        };
        command
            .prepare_admitted(&mut manager)
            .expect("prepare admitted event");
        command
            .prepare_admitted(&mut manager)
