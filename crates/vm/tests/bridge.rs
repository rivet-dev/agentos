...
use agentos_vm::VmManagerHost;
use agentos_vm_host_interface::{
    ClockRequest, CommandPermissionRequest, CreateJavascriptContextRequest, DiagnosticRecord,
    EnvironmentAccess, EnvironmentPermissionRequest, FilesystemAccess, FilesystemPermissionRequest,
    ...
    PathRequest, PollExecutionEventRequest, ReadFileRequest, StructuredEventRecord, VmHostTypes,
    WriteFileRequest,
};
    ...
    <B as VmHostTypes>::Error: Debug,
{
    ...
    assert_eq!(
        ...
        agentos_vm_host_interface::PermissionDecision::allow()
    );
    assert_eq!(
        bridge
    ...
    bridge
        .emit_log(LogRecord {
            vm_id: String::from("vm-1"),
            level: LogLevel::Info,
            message: String::from("sidecar ready"),
        })
        .expect("emit log");
    bridge
