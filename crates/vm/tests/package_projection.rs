...
fn package_leaf_builder_honors_adjustable_limit() {
    ...
    let error = agentos_vm::package_projection::build_package_leaf_mounts_with_limit(
    ...
    assert!(matches!(
        error,
        agentos_vm::VmError::PackageMountLimit {
            used: 0,
            requested: 3,
            limit: 2
        }
    ));
    let mounts = agentos_vm::package_projection::build_package_leaf_mounts_with_limit(
        &[descriptor],
        "/opt/agentos",
        3,
    )
