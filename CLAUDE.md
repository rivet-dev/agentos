...
- Docs are **user-facing, not internals.** Write for a developer *using the SDK*, not for a contributor maintaining the runtime. Every page is framed around how the SDK is used: what you call, what you pass, what you get back, and what behavior to expect. Do not document internal implementation (kernel data structures, isolate/VM plumbing, refactor history); that belongs in code comments or `CLAUDE.md`. When a page must reference a runtime concept (filesystem model, networking, isolation, WASM VM), explain it through the developer-visible API and behavior, show a quick code example, then go deeper as reference rather than narrating internals.
- **Exception: `Reference → Architecture` and `Reference → Security Model` are explicitly excluded from this rule.** These are the *only* two sections that intentionally document internals: Architecture covers how the runtime is built (agent sessions, processes, filesystem, networking, POSIX syscalls, compiler toolchain) and Security Model covers the isolation / trust / threat model. They are deliberate deep-dives for readers who want the mechanics. Keep all internals content confined to Architecture and Security Model; everywhere else stays user-facing per the rule above.
- Docs are **bullet-heavy and code-heavy, not prose-heavy.** Default to scannable bullet lists in the `- **Foo**: Bar` form (bolded term, then the one-line explanation) instead of paragraphs. Lead with a code snippet wherever a concept has an API, and link to runnable example code (e.g. `examples/*`) whenever one exists. Avoid long paragraphs; if you find yourself writing more than two or three sentences in a row, convert it to bullets or a code block. Prose is the exception, used only to connect ideas that genuinely cannot be a list. (Applies to the user-facing docs; the Architecture/Security Model internals sections may use more prose where mechanics demand it.)
- Every substantial documentation code block must be backed by an example under `examples/docs/*` that **type-checks** against `@rivet-dev/agentos` (the interim types-only package), with the doc code copied **verbatim** from that example and a `*[See Full Example](…)*` link to it. The examples are **type-checked, not run** (the interim `@rivet-dev/agentos` runtime is a noop stub whose handlers throw), so the bar is `tsc --noEmit` passing, not end-to-end execution. Tiny illustrative snippets (a line or two showing a signature or option shape) are exempt; anything a reader could copy should come from a real, type-checked example. Keep the doc block and the example in sync. (This mirrors the secure-exec docs rule, adapted: type-check instead of run-against-sidecar.)
- Docs styling is owned by the shared **`@rivet-dev/docs-theme`** repo (`github.com/rivet-dev/docs-theme`), consumed via `github:rivet-dev/docs-theme#<tag>` and wired in via `...docsTheme(starlight, siteConfig)`. To change any docs styling (palette, header, sidebar, code blocks, fonts), edit that repo and follow its CLAUDE.md release workflow — never restyle docs in `website/src`. When iterating on theme styling locally, add a `pnpm.overrides` `link:` to the local docs-theme checkout (kept through local commits while iterating), but **remove it before pushing** — a pushed consumer must pin `github:rivet-dev/docs-theme#<tag>`, never a path. This site owns only content + `website/docs.config.mjs` (sidebar icons via each item's `attrs['data-icon']`).
...
- **Package names:** the SDK/integration packages are `@rivet-dev/agentos` (+ `@rivet-dev/agentos-core`, `@rivet-dev/agentos-sandbox`); software AND agent packages are both `@agentos-software/*` (software e.g. `@agentos-software/common`, `@agentos-software/coreutils`; agents e.g. `@agentos-software/pi`, `@agentos-software/codex`). `@agentos-software/common` is part of the default software set and must NOT be added explicitly in docs/examples. Docs/examples import EVERYTHING from `@rivet-dev/agentos` (including `setup`) and NEVER import from `rivetkit` or `@rivetkit/react` directly.
- **Casing:** the actor factory is `agentOS()` (capital S); the standalone core class is `AgentOs`; the product name is "agentOS".
- The `agentOS()` config is FLAT, e.g. `agentOS({ software: [...], additionalInstructions: "..." })` — not nested under `options`.
- `software` is an array of IMPORTED software packages (e.g. `import common from "@agentos-software/common"; software: [common]`), never an array of strings.
- Do NOT reference "secure-exec" in the docs; it is the low-level runtime users do not need to know about.
- Do NOT import explicit types that are not needed; rely on inference. The only sanctioned type import is the registry (`import type { registry }` for `createClient<typeof registry>`).
