...
- secure-exec dependency workflow. Manage the secure-exec dependency ONLY through `scripts/secure-exec-dep.mjs` (the `just secure-exec-*` / `just agentos-pkgs-*` recipes); never hand-edit the `path` / `version` / `catalog:` pins.
  ...
  - secure-exec is also committed at product version `0.0.1`, so agent-os's committed secure-exec crate dep versions track it at `0.0.1`; real secure-exec versions are pinned only transiently at publish by `release-swap`.
...
- **GATE — agent-os's OWN product version is always `0.0.1` in the committed tree.** Every `@rivet-dev/agentos*` `package.json` and the root `Cargo.toml [workspace.package]` version stays pinned at `0.0.1`; the real version is applied ONLY transiently in the CI publish checkout (the `bump-versions` step in `publish.yaml`), never committed. The release version lives solely in the `publish.yaml` `version` input and the resulting `v<version>` git tag — `resolveVersion` derives it from git tags, not from any committed `package.json`. `scripts/verify-fixed-versions.mjs` enforces this in CI — a branch with a non-`0.0.1` product version fails the gate. This mirrors the secure-exec file-dep gate above: dev/CI state is committed, publish state is transient. `cut-release.ts` is a pure trigger and must never mutate or commit version files.
...
- **The projected `/opt/agentos` filesystem is the SOURCE OF TRUTH for all package/agent resolution — always read it live, never cache a package list.** Packages can be installed dynamically at runtime (`linkSoftware` appends new leaf mounts to the live `/opt/agentos`), so every resolution/enumeration path MUST read the current guest filesystem, not an in-memory snapshot captured at `configureVm` time. Each package is a dir at `/opt/agentos/pkgs/<name>/` with a `current/agentos-package.json`; commands are symlinks under `/opt/agentos/bin/`. Agent identity is the unscoped `agentos-package.json` `name` (e.g. `pi`, `claude`) — NOT the scoped npm package name (`@agentos-software/pi`).
- **Agent resolution AND enumeration are OWNED BY THE SIDECAR.** `createSession(name)`/`resumeSession(name)` send only the agent `agentType` (no `adapterEntrypoint` on the wire); the sidecar reads the projected `/opt/agentos/pkgs/<name>/current/agentos-package.json` and resolves `entrypoint = /opt/agentos/bin/<agent.acpEntrypoint>` plus the manifest's `env` (applied as defaults; caller env wins) and `launchArgs` (prepended before caller args), then spawns. `listAgents()` is a sidecar ACP RPC (`AcpListAgentsRequest`) — the sidecar enumerates the projected packages by reading the live `/opt/agentos/pkgs` directory; the client parses NO manifests for discovery. There is no hardcoded `AGENT_CONFIGS`/`BUILTIN_AGENT_IDS` table and no `/root/node_modules/<adapter>` npm lookup.
