...
- **Build the registry software before running the core tests.** With file-linked deps the `@agentos-software/*` packages are unbuilt source, and any test that constructs an `AgentOs` hits `resolveDefaultSoftware()`, which asserts every default software package (e.g. `common`) is built (`<pkg>/dist/package/`). Run `just registry-build` in the sibling secure-exec checkout first (add `just registry-native` if a lane needs the native/WASM command binaries); otherwise `createSession`/prompt tests throw `software package … is not BUILT`.
