...
async function setupGuestFixture(vm: AgentOs, root: string, payloadBytes: number): Promise<void> {
...
async function createVm(sidecar: AgentOsSidecar): Promise<AgentOs> {
	return AgentOs.create({
		...
		permissions: { fs: "allow", childProcess: "allow", process: "allow" },
	});
}
...
async function runCaseWithOptionalLatency(args: {
	...
	iterations: number;
	warmup: number;
	sharedSidecar: AgentOsSidecar | null;
	syncRpcLatencyEnabled: boolean;
	fsSyncPhasesEnabled: boolean;
}): Promise<FsSyncCaseResult> {
...
async function main(): Promise<void> {
	...
	const sharedSidecar =
		syncRpcLatencyEnabled || fsSyncPhasesEnabled ? null : await AgentOs.createSidecar();
	try {
