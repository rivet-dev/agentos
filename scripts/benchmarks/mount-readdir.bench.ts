 ...
 *
 * Isolates mount-count effects by timing AgentOs.readdir over native host_dir
...
function hostDirMount(path: string, hostPath: string): NativeMountConfig {
	return {
		path,
...
async function runUnrelatedMountCase(
	...
	const mounts: NativeMountConfig[] = [hostDirMount("/target", target)];
...
async function runChildMountCase(
	...
	const mounts: NativeMountConfig[] = [
