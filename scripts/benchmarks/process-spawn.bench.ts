...
import { runNativePhaseLayer } from "./native.js";
...
async function runGuestLayer(sidecar: AgentOsSidecar): Promise<number[]> {
...
async function runGuestNodeExitPhases(sidecar: AgentOsSidecar): Promise<PhaseSamples> {
	const samples: PhaseSamples = { total: [], spawn: [], wait_reap: [] };
	const vm = await AgentOs.create({ sidecar: { kind: "explicit", handle: sidecar } });
	try {
		for (let i = 0; i < WARMUP + ITERATIONS; i++) {
...
async function runGuestFanoutPhases(sidecar: AgentOsSidecar): Promise<PhaseSamples> {
	const samples: PhaseSamples = {
		total: [],
		spawn_batch: [],
		wait_reap_batch: [],
