/* Tests the behavior of writing to the pty after the controller was closed;
 * this should return an error, which is the reason a `poll(3)` would
 * return `POLLERR`. */
...
int main(void)
{
	...
	if ( session == 0 )
	{
		...
		uint8_t buf = 42;
		ssize_t ret = write(pty, &buf, sizeof(buf));
