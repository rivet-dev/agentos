/* Test if TIOCSCTTY works without O_NOCTTY. */
...
int main(void)
{
	...
	if ( !session )
	{
		...
		if ( pty < 0 )
			err(1, "%s", name);
#ifdef TIOCSCTTY
		// This may assign a controlling tty, do nothing if already assigned, or
		// even fail if a controlling tty has already been set.
