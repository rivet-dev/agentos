/* Test what is the process group of a process in limbo (process group leader
   that has been awaited but still has a member)? */
...
int main(void)
{
	...
	pid_t pgid = getpgid(zombie);
	if ( pgid < 0 )
		err(1, "getpgid on zombie");
	if ( pgid != zombie )
		err(1, "getpgid(zombie) != zombie");
	close(pipes[1]);
