/* Lock a temporary file with F_OFD_SETLK for writing, reopen the file and try
   to lock it again for writing. */
...
int main(void)
{
	...
	{
		unlink(path);
		err(1, "fcntl: F_OFD_SETLK: F_WRLCK");
	}
	...
	struct flock relock = { .l_type = F_WRLCK };
