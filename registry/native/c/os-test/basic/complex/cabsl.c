/* Test whether a basic cabsl invocation works. */
...
{
	long double output = cabsl(input1);
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) cabsl(%.4Lf + i*%.4Lf) = %.17La, not %.17La, diff %.17La, ratio %.20Lg",
...
int main(void)
{
	test(1, CMPLXL(90.01, 13.37), 0xb.5fec0f535325c22p+3L, 0xb.5fec0f535325c23p+3L, 0xb.5fec0f535325c24p+3L, 0);
	test(2, CMPLXL(-12.34, 13.37), 0x9.18dec66f18099a1p+1L, 0x9.18dec66f18099a2p+1L, 0x9.18dec66f18099a3p+1L, 0);
	...
	test(7, CMPLXL(90.01, -12.34), 0xb.5b431fbbcfc915ap+3L, 0xb.5b431fbbcfc915bp+3L, 0xb.5b431fbbcfc915cp+3L, 0);
	test(8, CMPLXL(-12.34, -12.34), 0x8.b9c752b9b457044p+1L, 0x8.b9c752b9b457045p+1L, 0x8.b9c752b9b457046p+1L, 0);
	...
	test(12, CMPLXL(0.0, -12.34), 0xc.570a3d70a3d6fffp+0L, 0xc.570a3d70a3d7p+0L, 0xc.570a3d70a3d7001p+0L, 0);
	...
	test(32, CMPLXL(-12.34, 0.0), 0xc.570a3d70a3d6fffp+0L, 0xc.570a3d70a3d7p+0L, 0xc.570a3d70a3d7001p+0L, 0);
