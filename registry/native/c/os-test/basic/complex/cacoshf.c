/* Test whether a basic cacoshf invocation works. */
...
{
	float complex output = cacoshf(input1);
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       real == expected_real) )
		{
			if ( imprecise && isfinite(real) && isfinite(expected_real) )
				return;
			warnx("(%d.) cacoshf(%.4f + i*%.4f).real = %.6a, not %.6a, diff %.6a, ratio %.8g",
		       ...
		       imag == expected_imag) )
		{
			if ( imprecise && isfinite(imag) && isfinite(expected_imag) )
				return;
			warnx("(%d.) cacoshf(%.4f + i*%.4f).imag = %.6a, not %.6a, diff %.6a, ratio %.8g",
...
int main(void)
{
	test(1, CMPLXF(90.01, 13.37), CMPLXF(0x1.4d0d86p+2, 0x1.2e048ap-3), CMPLXF(0x1.4d0d88p+2, 0x1.2e048cp-3), CMPLXF(0x1.4d0d8ap+2, 0x1.2e048ep-3), 0);
	test(2, CMPLXF(-12.34, 13.37), CMPLXF(0x1.cc129p+1, 0x1.285f0ep+1), CMPLXF(0x1.cc1292p+1, 0x1.285f1p+1), CMPLXF(0x1.cc1294p+1, 0x1.285f12p+1), 0);
	...
	test(6, CMPLXF(0.0, 13.37), CMPLXF(0x1.a4cea2p+1, 0x1.921fb4p+0), CMPLXF(0x1.a4cea4p+1, 0x1.921fb6p+0), CMPLXF(0x1.a4cea6p+1, 0x1.921fb8p+0), 0);
	test(7, CMPLXF(90.01, -12.34), CMPLXF(0x1.4cf348p+2, -0x1.170cb2p-3), CMPLXF(0x1.4cf34ap+2, -0x1.170cbp-3), CMPLXF(0x1.4cf34cp+2, -0x1.170caep-3), 0);
	test(8, CMPLXF(-12.34, -12.34), CMPLXF(0x1.c6ba88p+1, -0x1.2d7ce4p+1), CMPLXF(0x1.c6ba8ap+1, -0x1.2d7ce2p+1), CMPLXF(0x1.c6ba8cp+1, -0x1.2d7cep+1), 0);
	...
	test(12, CMPLXF(0.0, -12.34), CMPLXF(0x1.9a93a4p+1, -0x1.921fb8p+0), CMPLXF(0x1.9a93a6p+1, -0x1.921fb6p+0), CMPLXF(0x1.9a93a8p+1, -0x1.921fb4p+0), 0);
	...
	test(18, CMPLXF(0.0, nanf("")), CMPLXF(nanf(""), 0x1.921fb4p+0), CMPLXF(nanf(""), 0x1.921fb6p+0), CMPLXF(nanf(""), 0x1.921fb8p+0), 0 | MF_ANYSIGN2);
	...
	test(31, CMPLXF(90.01, 0.0), CMPLXF(0x1.4c5ab6p+2, -0x1.0p-149), CMPLXF(0x1.4c5ab8p+2, 0x0.0p+0), CMPLXF(0x1.4c5abap+2, 0x1.0p-149), 0);
	test(32, CMPLXF(-12.34, 0.0), CMPLXF(0x1.9a280cp+1, 0x1.921fb4p+1), CMPLXF(0x1.9a280ep+1, 0x1.921fb6p+1), CMPLXF(0x1.9a281p+1, 0x1.921fb8p+1), 0);
	...
	test(36, CMPLXF(0.0, 0.0), CMPLXF(-0x1.0p-149, 0x1.921fb4p+0), CMPLXF(0x0.0p+0, 0x1.921fb6p+0), CMPLXF(0x1.0p-149, 0x1.921fb8p+0), 0);
	return imprecise;
}
