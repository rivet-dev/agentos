/* Test whether a basic ngettext invocation works. */
...
int main(void)
{
	...
	// Test 0.
	char* output0 = ngettext(input, input_plural, 0);
	if ( !output0 )
		errx(1, "ngettext 0 returned NULL");
	...
	// Test 1.
	char* output1 = ngettext(input, input_plural, 1);
	if ( !output1 )
		errx(1, "ngettext 1 returned NULL");
	if ( strcmp(output1, input) != 0 )
		errx(1, "1 got \"%s\" not \"%s\"", output1, input);
	if ( output1 != input )
		errx(1, "1 did not return input string");
	// Test 2.
	char* output2 = ngettext(input, input_plural, 2);
	if ( !output2 )
		errx(1, "ngettext 2 returned NULL");
	...
	// Test 9.
	char* output9 = ngettext(input, input_plural, 9);
	if ( !output9 )
		errx(1, "ngettext 9 returned NULL");
