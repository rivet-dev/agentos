/* Test whether a basic ngettext_l invocation works. */
...
int main(void)
{
	...
	// Test 0.
	char* output0 = ngettext_l(input, input_plural, 0, locale);
	if ( !output0 )
		errx(1, "ngettext_l 0 returned NULL");
	...
	// Test 1.
	char* output1 = ngettext_l(input, input_plural, 1, locale);
	if ( !output1 )
		errx(1, "ngettext_l 1 returned NULL");
	if ( strcmp(output1, input) != 0 )
		errx(1, "1 got \"%s\" not \"%s\"", output1, input);
	if ( output1 != input )
		errx(1, "1 did not return input string");
	// Test 2.
	char* output2 = ngettext_l(input, input_plural, 2, locale);
	if ( !output2 )
		errx(1, "ngettext_l 2 returned NULL");
	...
	// Test 9.
	char* output9 = ngettext_l(input, input_plural, 9, locale);
	if ( !output9 )
		errx(1, "ngettext_l 9 returned NULL");
