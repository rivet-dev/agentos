/* Test whether a basic dcngettext invocation works. */
...
int main(void)
{
	...
	// Test 0.
	char* output0 = dcngettext("os-test", input, input_plural, 0, LC_NUMERIC);
	if ( !output0 )
		errx(1, "dcngettext 0 returned NULL");
	...
	// Test 1.
	char* output1 = dcngettext("os-test", input, input_plural, 1, LC_NUMERIC);
	if ( !output1 )
		errx(1, "dcngettext 1 returned NULL");
	if ( strcmp(output1, input) != 0 )
		errx(1, "1 got \"%s\" not \"%s\"", output1, input);
	if ( output1 != input )
		errx(1, "1 did not return input string");
	// Test 2.
	char* output2 = dcngettext("os-test", input, input_plural, 2, LC_NUMERIC);
	if ( !output2 )
		errx(1, "dcngettext 2 returned NULL");
	...
	// Test 9.
	char* output9 = dcngettext("os-test", input, input_plural, 9, LC_NUMERIC);
	if ( !output9 )
		errx(1, "dcngettext 9 returned NULL");
