/* Test whether a basic dcngettext_l invocation works. */
...
int main(void)
{
	...
	// Test 0.
	char* output0 = dcngettext_l("os-test", input, input_plural, 0, LC_NUMERIC,
	                             locale);
	if ( !output0 )
		errx(1, "dcngettext_l 0 returned NULL");
	if ( strcmp(output0, input_plural) != 0 )
		errx(1, "0 got \"%s\" not \"%s\"", output0, input_plural, LC_NUMERIC,
	                             locale);
	if ( output0 != input_plural )
		errx(1, "0 did not return input string");
	// Test 1.
	char* output1 = dcngettext_l("os-test", input, input_plural, 1, LC_NUMERIC,
	                             locale);
	if ( !output1 )
		errx(1, "dcngettext_l 1 returned NULL");
	if ( strcmp(output1, input) != 0 )
		errx(1, "1 got \"%s\" not \"%s\"", output1, input);
	if ( output1 != input )
		errx(1, "1 did not return input string");
	// Test 2.
	char* output2 = dcngettext_l("os-test", input, input_plural, 2, LC_NUMERIC,
	                             locale);
	if ( !output2 )
		errx(1, "dcngettext_l 2 returned NULL");
	...
	// Test 9.
	char* output9 = dcngettext_l("os-test", input, input_plural, 9, LC_NUMERIC,
	                             locale);
	if ( !output9 )
		errx(1, "dcngettext_l 9 returned NULL");
