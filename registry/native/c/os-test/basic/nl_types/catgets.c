/* Test whether a basic catgets invocation works. */
...
int main(void)
{
	...
	// Test reading messages that exist from sets that exist.
	errno = 0;
	set = 1;
	msg = 1;
	expected = "One";
	str = catgets(cat, set, msg, def);
	if ( str == NULL )
		err(1, "catgets set %i msg %i returned NULL", set, msg);
	else if ( str == def && errno != 0 )
		err(1, "catgets set %i msg %i", set, msg);
	else if ( str == def && !errno )
		errx(1, "catgets did not find set %i msg %i", set, msg);
	if ( strcmp(str, expected) != 0 )
		errx(1, "catgets got \"%s\" instead of \"%s\"");
	...
	if ( str == NULL )
		err(1, "catgets set %i msg %i returned NULL", set, msg);
	else if ( str == def && errno == ENOMSG )
		;
	else if ( str == def && errno != 0 )
		err(1, "catgets set %i msg %i", set, msg);
	else if ( str == def && errno == 0 )
		errx(1, "catgets set %i msg %i errno != ENOMSG", set, msg);
	else
		errx(1, "catgets somehow found set %i msg %i", set, msg);
