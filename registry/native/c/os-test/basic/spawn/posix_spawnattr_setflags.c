/*[SPN]*/
/* Test whether a basic posix_spawnattr_setflags invocation works. */
...
int main(int argc, char* argv[])
{
	...
	// POSIX_SPAWN_SETSID is new in POSIX-1.2024. Test it if available to avoid
	// repeating the posix_spawnattr_setpgroup test,
	...
	// Test putting the child in its own session / process group.
	posix_spawnattr_t attr;
	if ( (errno = posix_spawnattr_init(&attr)) )
		err(1, "posix_spawnattr_init");
#ifdef POSIX_SPAWN_SETSID
	if ( (errno = posix_spawnattr_setflags(&attr, POSIX_SPAWN_SETSID)) )
#else
