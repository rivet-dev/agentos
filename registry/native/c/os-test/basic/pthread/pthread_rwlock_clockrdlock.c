/* Test whether a basic pthread_rwlock_clockrdlock invocation works. */
...
int main(void)
{
	...
	if ( (errno = pthread_rwlock_clockrdlock(&rwlock, CLOCK_MONOTONIC,
	                                         &long_timeout)) )
		err(1, "pthread_rwlock_timedlock");
	                                         ...
	                                         &short_timeout)) )
	{
		if ( errno != ETIMEDOUT && errno != EDEADLK )
			err(1, "pthread_rwlock_clockrdlock");
	}
	else
		errx(1, "pthread_rwlock_clockrdlock did not time out");
	return 0;
}
