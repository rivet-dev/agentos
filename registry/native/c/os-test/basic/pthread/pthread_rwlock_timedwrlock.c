/* Test whether a basic pthread_rwlock_timedwrlock invocation works. */
...
int main(void)
{
	...
	if ( (errno = pthread_rwlock_timedwrlock(&rwlock, &long_timeout)) )
		err(1, "pthread_rwlock_timedwrlock");
	struct timespec short_timeout;
	clock_gettime(CLOCK_REALTIME, &short_timeout);
	if ( (errno = pthread_rwlock_timedwrlock(&rwlock, &short_timeout)) )
	{
		if ( errno != ETIMEDOUT && errno != EDEADLK )
			err(1, "pthread_rwlock_timedwrlock");
	}
	else
		errx(1, "pthread_rwlock_timedwrlock did not time out");
	return 0;
}
