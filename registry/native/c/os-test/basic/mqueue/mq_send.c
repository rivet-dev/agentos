/*[MSG]*/
/* Test whether a basic mq_send invocation works. */
...
int main(void)
{
	...
	// Try sending too large a message.
	if ( mq_send(mq, "food", 4, 0) < 0 )
	{
		if ( errno != EMSGSIZE )
			err(1, "second mq_send");
	}
	else
		errx(1, "second mq_send did not EMSGSIZE");

	// Try successfully sending a normal message.
	if ( mq_send(mq, "foo", 3, 0) < 0 )
		err(1, "second mq_send");

	// Try sending non-blocking when the queue is full.
	...
	if ( mq_send(mq, "foo", 3, 0) < 0 )
	{
		...
		errx(1, "third mq_send did not EAGAIN");
