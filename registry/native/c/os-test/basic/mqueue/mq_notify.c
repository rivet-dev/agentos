/*[MSG]*/
/* Test whether a basic mq_notify invocation works. */
...
int main(void)
{
	...
	// Try signal notification.
	struct sigevent sevsig =
	{
		.sigev_notify = SIGEV_SIGNAL,
		.sigev_signo = SIGUSR1,
	};
	struct sigaction usr1_sa = { .sa_handler = on_sigusr1 };
	sigset_t usr1_set, usr1_unblocked;
	sigemptyset(&usr1_set);
	sigaddset(&usr1_set, SIGUSR1);
	sigprocmask(SIG_BLOCK, &usr1_set, &usr1_unblocked);
	if ( sigaction(SIGUSR1, &usr1_sa, NULL) < 0 )
	     err(1, "sigaction");
	sigdelset(&usr1_unblocked, SIGUSR1);
	if ( mq_notify(mq, &sevsig) < 0 )
		err(1, "first mq_notify");

	// Test failing if a notification is already installed.
	struct sigevent sevnone = { .sigev_notify = SIGEV_NONE };
	if ( mq_notify(mq, &sevnone) < 0 )
	{
		if ( errno != EBUSY )
			err(1, "second mq_notify");
	}
	else
		errx(1, "second mq_notify did not EBUSY");

	// Receive the signal.
	if ( mq_send(mq, "foo", 3, 0) < 0 )
		errx(1, "first mq_send");
	alarm(1);
	sigsuspend(&usr1_unblocked);
	if ( got_sigusr1 != SIGUSR1 )
		errx(1, "did not receive SIGUSR1");
	// Test reinstalling notification.
	if ( mq_notify(mq, &sevsig) < 0 )
		err(1, "third mq_notify");

	// Test uninstalling notification.
	if ( mq_notify(mq, NULL) < 0 )
		err(1, "fourth mq_notify");
