/*[XSI]*/
/* Test whether a basic dbm_delete invocation works. */
...
int main(void)
{
	...
	{
		if ( dbm_error(db) )
			err(1, "first dbm_fetch");
		errx(1, "first dbm_fetch did not find bar");
	}
	if ( lookup.dsize != 3 )
		errx(1, "first dbm_fetch bar had wrong size");
	if ( memcmp(lookup.dptr, newbardata.dptr, 3) != 0 )
		errx(1, "first dbm_fetch bar had wrong data");
	// Try delete bar.
	if ( dbm_delete(db, barkey) < 0 )
		errx(1, "dbm_delete");
	// Try get bar (deleted).
	lookup = dbm_fetch(db, barkey);
	if ( !lookup.dptr )
	{
		if ( dbm_error(db) )
			err(1, "second dbm_fetch");
	}
	else
		errx(1, "second dbm_fetch found absent foo");
	dbm_close(db);
	return 0;
}
