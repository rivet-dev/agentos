/*[XSI]*/
/* Test whether a basic dbm_store invocation works. */
...
int main(void)
{
	...
	if ( (ret = dbm_store(db, fookey, foodata, DBM_INSERT)) < 0 )
		...
		errx(1, "dbm_store found absent entry");
	else if ( ret != 0 )
		errx(1, "dbm_store returned weird");
	// Try insert foo again (will conflict).
	if ( (ret = dbm_store(db, fookey, foodata, DBM_INSERT)) < 0 )
		err(1, "dbm_store");
	else if ( ret == 1 )
		;
		...
		errx(1, "dbm_store did not find conflict");
	// Try replace foo with itself.
	if ( (ret = dbm_store(db, fookey, foodata, DBM_REPLACE)) < 0 )
		err(1, "dbm_store");
	else if ( ret == 1 )
		errx(1, "dbm_store found absent entry");
	else if ( ret != 0 )
