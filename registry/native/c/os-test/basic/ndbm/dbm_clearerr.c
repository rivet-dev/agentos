/*[XSI]*/
/* Test whether a basic dbm_clearerr invocation works. */
...
int main(void)
{
	...
	if ( !db )
		err(1, "dbm_open");
	// I can't think of a good way to force a database error condition, short of
	// possibly exhausting disk space or trying to mess with the underlying file
	// descriptors or resource limits. Let's just assume the error handling
	// fine.
	dbm_clearerr(db);
	if ( dbm_error(db) )
		errx(1, "dbm_error was set after dbm_clearerr");
	dbm_close(db);
	return 0;
}
