/* Test whether a basic lgammal invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) lgammal(%.4Lf) = %.17La, not %.17La, diff %.17La, ratio %.20Lg",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0x9.cd9509fb93a1776p+5L, 0x9.cd9509fb93a1777p+5L, 0x9.cd9509fb93a1778p+5L, 0);
	test(2, -12.34, 0, 0, -0x9.c974601eff84036p+1L, -0x9.c974601eff84035p+1L, -0x9.c974601eff84034p+1L, 0);
	...
	test(6, 0.0, ERANGE, FE_DIVBYZERO, strtold("inf", NULL), strtold("inf", NULL), strtold("inf", NULL), 0);
	return imprecise;
}
