/* Test whether a basic log10l invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) log10l(%.4Lf) = %.17La, not %.17La, diff %.17La, ratio %.20Lg",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0xf.a26331e30cd739ep-3L, 0xf.a26331e30cd739fp-3L, 0xf.a26331e30cd73ap-3L, 0);
	test(2, -12.34, EDOM, FE_INVALID, nanl(""), nanl(""), nanl(""), 0);
