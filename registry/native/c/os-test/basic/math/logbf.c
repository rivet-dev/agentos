/* Test whether a basic logbf invocation works. */
...
void test(int variant, float input1, int errnum, int fperr, float lower, float expected, float upper, int flags)
{
	...
	float output = logbf(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) logbf(%.4f) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) logbf(%.4f) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) logbf(%.4f) = %.6a, not %.6a, diff %.6a, ratio %.8g",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0x1.7ffffep+2, 0x1.8p+2, 0x1.800002p+2, 0);
	test(2, -12.34, 0, 0, 0x1.7ffffep+1, 0x1.8p+1, 0x1.800002p+1, 0);
	...
	test(6, 0.0, ERANGE, FE_DIVBYZERO, strtof("-inf", NULL), strtof("-inf", NULL), strtof("-inf", NULL), 0 | MF_MAYERR);
	return imprecise;
}
