/* Test whether a basic asinhl invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) asinhl(%.4Lf) = %.17La, not %.17La, diff %.17La, ratio %.20Lg",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0xa.62ddd8f2a556bfbp-1L, 0xa.62ddd8f2a556bfcp-1L, 0xa.62ddd8f2a556bfdp-1L, 0);
	test(2, -12.34, 0, 0, -0xc.d49d33e487b029bp-2L, -0xc.d49d33e487b029ap-2L, -0xc.d49d33e487b0299p-2L, 0);
