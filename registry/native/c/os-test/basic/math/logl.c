/* Test whether a basic logl invocation works. */
...
void test(int variant, long double input1, int errnum, int fperr, long double lower, long double expected, long double upper, int flags)
{
	...
	long double output = logl(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) logl(%.4Lf) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) logl(%.4Lf) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) logl(%.4Lf) = %.17La, not %.17La, diff %.17La, ratio %.20Lg",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0x8.fff59da8be1a523p-1L, 0x8.fff59da8be1a524p-1L, 0x8.fff59da8be1a525p-1L, 0);
