/* Test whether a basic acosf invocation works. */
...
void test(int variant, float input1, int errnum, int fperr, float lower, float expected, float upper, int flags)
{
	...
	float output = acosf(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) acosf(%.4f) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) acosf(%.4f) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) acosf(%.4f) = %.6a, not %.6a, diff %.6a, ratio %.8g",
...
int main(void)
{
	test(1, 0.9001, 0, 0, 0x1.cd9ddp-2, 0x1.cd9dd2p-2, 0x1.cd9dd4p-2, 0);
	test(2, -0.1234, 0, 0, 0x1.b1cb82p+0, 0x1.b1cb84p+0, 0x1.b1cb86p+0, 0);
	...
	test(6, 0.0, 0, 0, 0x1.921fb4p+0, 0x1.921fb6p+0, 0x1.921fb8p+0, 0);
	return imprecise;
}
