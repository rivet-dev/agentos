/* Test whether a basic sin invocation works. */
...
void test(int variant, double input1, int errnum, int fperr, double lower, double expected, double upper, int flags)
{
	...
	double output = sin(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) sin(%.4f) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) sin(%.4f) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) sin(%.4f) = %.14a, not %.14a, diff %.14a, ratio %.16g",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0x1.c768c8574930cp-1, 0x1.c768c8574930dp-1, 0x1.c768c8574930ep-1, 0);
	test(2, -12.34, 0, 0, 0x1.cba85cb1eb4b8p-3, 0x1.cba85cb1eb4b9p-3, 0x1.cba85cb1eb4bap-3, 0);
