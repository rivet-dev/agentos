/* Test whether a basic lgammaf invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) lgammaf(%.4f) = %.6a, not %.6a, diff %.6a, ratio %.8g",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0x1.39b2ap+8, 0x1.39b2a2p+8, 0x1.39b2a4p+8, 0);
	test(2, -12.34, 0, 0, -0x1.392e8ep+4, -0x1.392e8cp+4, -0x1.392e8ap+4, 0);
	test(3, nanf(""), 0, 0, nanf(""), nanf(""), nanf(""), 0);
	test(4, strtof("inf", NULL), 0, 0, strtof("inf", NULL), strtof("inf", NULL), strtof("inf", NULL), 0);
	test(5, strtof("-inf", NULL), 0, 0, strtof("inf", NULL), strtof("inf", NULL), strtof("inf", NULL), 0);
	test(6, 0.0, ERANGE, FE_DIVBYZERO, strtof("inf", NULL), strtof("inf", NULL), strtof("inf", NULL), 0);
	return imprecise;
}
