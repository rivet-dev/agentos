/* Test whether a basic nearbyintf invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) nearbyintf(%.4f) = %.6a, not %.6a, diff %.6a, ratio %.8g",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0x1.67fffep+6, 0x1.68p+6, 0x1.680002p+6, 0);
	test(2, -12.34, 0, 0, -0x1.800002p+3, -0x1.8p+3, -0x1.7ffffep+3, 0);
