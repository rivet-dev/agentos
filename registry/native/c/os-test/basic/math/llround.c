/* Test whether a basic llround invocation works. */
...
void test(int variant, double input1, int errnum, int fperr, long long expected, int flags)
{
	...
	{
		if ( output != expected )
			errx(1, "(%d.) llround(%.4f) = %lld, not %lld",
...
int main(void)
{
	...
	test(3, nan(""), EDOM, FE_INVALID, 0, 0 | MF_UNSPEC1);
	test(4, strtod("inf", NULL), EDOM, FE_INVALID, 0, 0 | MF_UNSPEC1);
	test(5, strtod("-inf", NULL), EDOM, FE_INVALID, 0, 0 | MF_UNSPEC1);
