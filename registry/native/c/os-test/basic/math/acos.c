/* Test whether a basic acos invocation works. */
...
void test(int variant, double input1, int errnum, int fperr, double lower, double expected, double upper, int flags)
{
	...
	double output = acos(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) acos(%.4f) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) acos(%.4f) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) acos(%.4f) = %.14a, not %.14a, diff %.14a, ratio %.16g",
...
int main(void)
{
	test(1, 0.9001, 0, 0, 0x1.cd9dd17ee3a59p-2, 0x1.cd9dd17ee3a5ap-2, 0x1.cd9dd17ee3a5bp-2, 0);
	test(2, -0.1234, 0, 0, 0x1.b1cb8458ab084p+0, 0x1.b1cb8458ab085p+0, 0x1.b1cb8458ab086p+0, 0);
	test(3, nan(""), 0, 0, nan(""), nan(""), nan(""), 0);
	test(4, strtod("inf", NULL), EDOM, FE_INVALID, nan(""), nan(""), nan(""), 0);
	test(5, strtod("-inf", NULL), EDOM, FE_INVALID, nan(""), nan(""), nan(""), 0);
	test(6, 0.0, 0, 0, 0x1.921fb54442d17p+0, 0x1.921fb54442d18p+0, 0x1.921fb54442d19p+0, 0);
	return imprecise;
}
