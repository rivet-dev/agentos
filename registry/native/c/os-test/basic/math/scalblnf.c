/* Test whether a basic scalblnf invocation works. */
...
void test(int variant, float input1, long input2, int errnum, int fperr, float lower, float expected, float upper, int flags)
{
	errno = 0;
	if ( feclearexcept(FE_ALL_EXCEPT) )
		errx(1, "feclearexcept");
	float output = scalblnf(input1, input2);
	if ( errnum == 0 && errno )
		err(1, "(%d.) scalblnf(%.4f, %ld) failed",
                    variant, input1, input2);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) scalblnf(%.4f, %ld) did not %s",
                     variant, input1, input2, strerrno(errnum));
	int excepts = fetestexcept(FE_INVALID | FE_DIVBYZERO | FE_OVERFLOW | FE_UNDERFLOW);
	if ( fperr == 0 && excepts )
		errx(1, "(%d.) scalblnf(%.4f, %ld) %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) scalblnf(%.4f, %ld) = %.6a, not %.6a, diff %.6a, ratio %.8g",
...
int main(void)
{
	test(1, 13, 13.37, 0, 0, 0x1.9ffffep+16, 0x1.ap+16, 0x1.a00002p+16, 0);
	test(2, -12.34, 13.37, 0, 0, -0x1.8ae14ap+16, -0x1.8ae148p+16, -0x1.8ae146p+16, 0);
