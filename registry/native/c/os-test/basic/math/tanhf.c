/* Test whether a basic tanhf invocation works. */
...
void test(int variant, float input1, int errnum, int fperr, float lower, float expected, float upper, int flags)
{
	...
	float output = tanhf(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) tanhf(%.4f) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) tanhf(%.4f) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) tanhf(%.4f) = %.6a, not %.6a, diff %.6a, ratio %.8g",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0x1.fffffep-1, 0x1.0p+0, 0x1.000002p+0, 0);
	test(2, -12.34, 0, 0, -0x1.000002p+0, -0x1.0p+0, -0x1.fffffep-1, 0);
	test(3, nanf(""), 0, 0, nanf(""), nanf(""), nanf(""), 0);
	test(4, strtof("inf", NULL), 0, 0, 0x1.fffffep-1, 0x1.0p+0, 0x1.000002p+0, 0);
