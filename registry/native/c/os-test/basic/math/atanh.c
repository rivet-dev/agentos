/* Test whether a basic atanh invocation works. */
...
void test(int variant, double input1, int errnum, int fperr, double lower, double expected, double upper, int flags)
{
	...
	double output = atanh(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) atanh(%.4f) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) atanh(%.4f) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) atanh(%.4f) = %.14a, not %.14a, diff %.14a, ratio %.16g",
...
int main(void)
{
	test(1, 0.9001, 0, 0, 0x1.7905e2ace17ecp+0, 0x1.7905e2ace17edp+0, 0x1.7905e2ace17eep+0, 0);
	test(2, -0.1234, 0, 0, -0x1.fc0921af779c8p-4, -0x1.fc0921af779c7p-4, -0x1.fc0921af779c6p-4, 0);
