/* Test whether a basic acosl invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) acosl(%.4Lf) = %.17La, not %.17La, diff %.17La, ratio %.20Lg",
...
int main(void)
{
	test(1, 0.9001, 0, 0, 0xe.6cee8bf71d2d2e5p-5L, 0xe.6cee8bf71d2d2e6p-5L, 0xe.6cee8bf71d2d2e7p-5L, 0);
	test(2, -0.1234, 0, 0, 0xd.8e5c22c5584272fp-3L, 0xd.8e5c22c5584273p-3L, 0xd.8e5c22c55842731p-3L, 0);
	...
	test(6, 0.0, 0, 0, 0xc.90fdaa22168c234p-3L, 0xc.90fdaa22168c235p-3L, 0xc.90fdaa22168c236p-3L, 0);
	return imprecise;
}
