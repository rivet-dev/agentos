/* Test whether a basic acoshf invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) acoshf(%.4f) = %.6a, not %.6a, diff %.6a, ratio %.8g",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0x1.4c5ab6p+2, 0x1.4c5ab8p+2, 0x1.4c5abap+2, 0);
	...
	test(6, 0.0, EDOM, FE_INVALID, nanf(""), nanf(""), nanf(""), 0);
	return imprecise;
}
