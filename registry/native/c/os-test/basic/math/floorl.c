/* Test whether a basic floorl invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) floorl(%.4Lf) = %.17La, not %.17La, diff %.17La, ratio %.20Lg",
...
int main(void)
{
	...
	test(2, -12.34, 0, 0, -0xd.000000000000001p+0L, -0xd.0p+0L, -0xc.fffffffffffffffp+0L, 0);
