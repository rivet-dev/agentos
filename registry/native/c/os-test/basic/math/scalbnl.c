/* Test whether a basic scalbnl invocation works. */
...
void test(int variant, long double input1, int input2, int errnum, int fperr, long double lower, long double expected, long double upper, int flags)
{
	errno = 0;
	if ( feclearexcept(FE_ALL_EXCEPT) )
		errx(1, "feclearexcept");
	long double output = scalbnl(input1, input2);
	if ( errnum == 0 && errno )
		err(1, "(%d.) scalbnl(%.4Lf, %d) failed",
                    variant, input1, input2);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) scalbnl(%.4Lf, %d) did not %s",
                     variant, input1, input2, strerrno(errnum));
	int excepts = fetestexcept(FE_INVALID | FE_DIVBYZERO | FE_OVERFLOW | FE_UNDERFLOW);
	if ( fperr == 0 && excepts )
		errx(1, "(%d.) scalbnl(%.4Lf, %d) %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) scalbnl(%.4Lf, %d) = %.17La, not %.17La, diff %.17La, ratio %.20Lg",
