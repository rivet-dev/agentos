/* Test whether a basic sinh invocation works. */
...
void test(int variant, double input1, int errnum, int fperr, double lower, double expected, double upper, int flags)
{
	...
	double output = sinh(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) sinh(%.4f) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) sinh(%.4f) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) sinh(%.4f) = %.14a, not %.14a, diff %.14a, ratio %.16g",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0x1.cfada9e9f4347p+128, 0x1.cfada9e9f4348p+128, 0x1.cfada9e9f4349p+128, 0);
	test(2, -12.34, 0, 0, -0x1.be9af9dcdaaf9p+16, -0x1.be9af9dcdaaf8p+16, -0x1.be9af9dcdaaf7p+16, 0);
