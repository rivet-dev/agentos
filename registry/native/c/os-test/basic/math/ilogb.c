/* Test whether a basic ilogb invocation works. */
...
void test(int variant, double input1, int errnum, int fperr, int expected, int flags)
{
	errno = 0;
	if ( feclearexcept(FE_ALL_EXCEPT) )
		errx(1, "feclearexcept");
	int output = ilogb(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) ilogb(%.4f) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) ilogb(%.4f) did not %s",
...
int main(void)
{
	...
	test(3, nan(""), EDOM, FE_INVALID, FP_ILOGBNAN, 0);
	test(4, strtod("inf", NULL), EDOM, FE_INVALID, INT_MAX, 0);
	test(5, strtod("-inf", NULL), EDOM, FE_INVALID, INT_MAX, 0);
