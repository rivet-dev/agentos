/* Test whether a basic scalblnl invocation works. */
...
void test(int variant, long double input1, long input2, int errnum, int fperr, long double lower, long double expected, long double upper, int flags)
{
	errno = 0;
	if ( feclearexcept(FE_ALL_EXCEPT) )
		errx(1, "feclearexcept");
	long double output = scalblnl(input1, input2);
	if ( errnum == 0 && errno )
		err(1, "(%d.) scalblnl(%.4Lf, %ld) failed",
                    variant, input1, input2);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) scalblnl(%.4Lf, %ld) did not %s",
                     variant, input1, input2, strerrno(errnum));
	int excepts = fetestexcept(FE_INVALID | FE_DIVBYZERO | FE_OVERFLOW | FE_UNDERFLOW);
	if ( fperr == 0 && excepts )
		errx(1, "(%d.) scalblnl(%.4Lf, %ld) %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) scalblnl(%.4Lf, %ld) = %.17La, not %.17La, diff %.17La, ratio %.20Lg",
...
int main(void)
{
	test(1, 13, 13.37, 0, 0, 0xc.fffffffffffffffp+13L, 0xd.0p+13L, 0xd.000000000000001p+13L, 0);
	test(2, -12.34, 13.37, 0, 0, -0xc.570a3d70a3d7001p+13L, -0xc.570a3d70a3d7p+13L, -0xc.570a3d70a3d6fffp+13L, 0);
