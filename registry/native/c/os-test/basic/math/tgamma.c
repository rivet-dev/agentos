/* Test whether a basic tgamma invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) tgamma(%.4f) = %.14a, not %.14a, diff %.14a, ratio %.16g",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0x1.7c18aa84560f6p+452, 0x1.7c18aa84560f7p+452, 0x1.7c18aa84560f8p+452, 0);
	test(2, -12.34, 0, 0, -0x1.b1cc96599b94dp-29, -0x1.b1cc96599b94cp-29, -0x1.b1cc96599b94bp-29, 0);
	test(3, nan(""), 0, 0, nan(""), nan(""), nan(""), 0);
	test(4, strtod("inf", NULL), 0, 0, strtod("inf", NULL), strtod("inf", NULL), strtod("inf", NULL), 0);
	test(5, strtod("-inf", NULL), EDOM, FE_INVALID, nan(""), nan(""), nan(""), 0);
