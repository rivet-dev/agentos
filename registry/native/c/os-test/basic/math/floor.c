/* Test whether a basic floor invocation works. */
...
void test(int variant, double input1, int errnum, int fperr, double lower, double expected, double upper, int flags)
{
	...
	double output = floor(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) floor(%.4f) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) floor(%.4f) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) floor(%.4f) = %.14a, not %.14a, diff %.14a, ratio %.16g",
...
int main(void)
{
	...
	test(2, -12.34, 0, 0, -0x1.a000000000001p+3, -0x1.ap+3, -0x1.9ffffffffffffp+3, 0);
