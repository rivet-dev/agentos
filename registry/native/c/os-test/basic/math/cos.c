/* Test whether a basic cos invocation works. */
...
void test(int variant, double input1, int errnum, int fperr, double lower, double expected, double upper, int flags)
{
	...
	double output = cos(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) cos(%.4f) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) cos(%.4f) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) cos(%.4f) = %.14a, not %.14a, diff %.14a, ratio %.16g",
...
int main(void)
{
	test(1, 90.01, 0, 0, -0x1.d3f574e6573ap-2, -0x1.d3f574e65739fp-2, -0x1.d3f574e65739ep-2, 0);
	test(2, -12.34, 0, 0, 0x1.f2f003281ed37p-1, 0x1.f2f003281ed38p-1, 0x1.f2f003281ed39p-1, 0);
