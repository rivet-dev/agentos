/* Test whether a basic cbrtf invocation works. */
...
void test(int variant, float input1, int errnum, int fperr, float lower, float expected, float upper, int flags)
{
	...
	float output = cbrtf(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) cbrtf(%.4f) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) cbrtf(%.4f) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) cbrtf(%.4f) = %.6a, not %.6a, diff %.6a, ratio %.8g",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0x1.1ed20cp+2, 0x1.1ed20ep+2, 0x1.1ed21p+2, 0);
	test(2, -12.34, 0, 0, -0x1.27c9fp+1, -0x1.27c9eep+1, -0x1.27c9ecp+1, 0);
