/* Test whether a basic atanl invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) atanl(%.4Lf) = %.17La, not %.17La, diff %.17La, ratio %.20Lg",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0xc.7a3d21128d128d6p-3L, 0xc.7a3d21128d128d7p-3L, 0xc.7a3d21128d128d8p-3L, 0);
	test(2, -12.34, 0, 0, -0xb.eb636ea52065e7dp-3L, -0xb.eb636ea52065e7cp-3L, -0xb.eb636ea52065e7bp-3L, 0);
	test(3, nanl(""), 0, 0, nanl(""), nanl(""), nanl(""), 0);
	test(4, strtold("inf", NULL), 0, 0, 0xc.90fdaa22168c234p-3L, 0xc.90fdaa22168c235p-3L, 0xc.90fdaa22168c236p-3L, 0);
	test(5, strtold("-inf", NULL), 0, 0, -0xc.90fdaa22168c236p-3L, -0xc.90fdaa22168c235p-3L, -0xc.90fdaa22168c234p-3L, 0);
