/* Test whether a basic exp2 invocation works. */
...
void test(int variant, double input1, int errnum, int fperr, double lower, double expected, double upper, int flags)
{
	...
	double output = exp2(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) exp2(%.4f) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) exp2(%.4f) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) exp2(%.4f) = %.14a, not %.14a, diff %.14a, ratio %.16g",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0x1.01c7d6c404f1bp+90, 0x1.01c7d6c404f1cp+90, 0x1.01c7d6c404f1dp+90, 0);
	test(2, -12.34, 0, 0, 0x1.94804b79e2606p-13, 0x1.94804b79e2607p-13, 0x1.94804b79e2608p-13, 0);
	test(3, nan(""), 0, 0, nan(""), nan(""), nan(""), 0);
	test(4, strtod("inf", NULL), 0, 0, strtod("inf", NULL), strtod("inf", NULL), strtod("inf", NULL), 0);
	test(5, strtod("-inf", NULL), 0, 0, -0x0.0000000000001p-1022, 0x0.0p+0, 0x0.0000000000001p-1022, 0);
	test(6, 0.0, 0, 0, 0x1.fffffffffffffp-1, 0x1.0p+0, 0x1.0000000000001p+0, 0);
	return imprecise;
}
