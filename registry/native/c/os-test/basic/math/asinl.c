/* Test whether a basic asinl invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) asinl(%.4Lf) = %.17La, not %.17La, diff %.17La, ratio %.20Lg",
...
int main(void)
{
	test(1, 0.9001, 0, 0, 0x8.f5c207244f40d7ap-3L, 0x8.f5c207244f40d7bp-3L, 0x8.f5c207244f40d7cp-3L, 0);
	test(2, -0.1234, 0, 0, -0xf.d5e78a341b64fbbp-7L, -0xf.d5e78a341b64fbap-7L, -0xf.d5e78a341b64fb9p-7L, 0);
