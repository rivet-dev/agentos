/* Test whether a basic fmodf invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) fmodf(%.4f, %.4f) = %.6a, not %.6a, diff %.6a, ratio %.8g",
...
int main(void)
{
	test(1, 90.01, 13.37, 0, 0, 0x1.3947b2p+3, 0x1.3947b4p+3, 0x1.3947b6p+3, 0);
