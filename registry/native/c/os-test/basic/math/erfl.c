/* Test whether a basic erfl invocation works. */
...
void test(int variant, long double input1, int errnum, int fperr, long double lower, long double expected, long double upper, int flags)
{
	...
	long double output = erfl(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) erfl(%.4Lf) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) erfl(%.4Lf) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) erfl(%.4Lf) = %.17La, not %.17La, diff %.17La, ratio %.20Lg",
...
int main(void)
{
	test(1, 1.01, 0, 0, 0xd.8c8929b35c13c2dp-4L, 0xd.8c8929b35c13c2ep-4L, 0xd.8c8929b35c13c2fp-4L, 0);
	test(2, -12.34, 0, 0, -0x8.000000000000001p-3L, -0x8.0p-3L, -0xf.fffffffffffffffp-4L, 0);
