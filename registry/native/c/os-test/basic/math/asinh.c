/* Test whether a basic asinh invocation works. */
...
void test(int variant, double input1, int errnum, int fperr, double lower, double expected, double upper, int flags)
{
	...
	double output = asinh(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) asinh(%.4f) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) asinh(%.4f) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) asinh(%.4f) = %.14a, not %.14a, diff %.14a, ratio %.16g",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0x1.4c5bbb1e54aacp+2, 0x1.4c5bbb1e54aadp+2, 0x1.4c5bbb1e54aaep+2, 0);
	test(2, -12.34, 0, 0, -0x1.9a93a67c90f61p+1, -0x1.9a93a67c90f6p+1, -0x1.9a93a67c90f5fp+1, 0);
