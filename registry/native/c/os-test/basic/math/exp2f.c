/* Test whether a basic exp2f invocation works. */
...
void test(int variant, float input1, int errnum, int fperr, float lower, float expected, float upper, int flags)
{
	...
	float output = exp2f(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) exp2f(%.4f) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) exp2f(%.4f) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) exp2f(%.4f) = %.6a, not %.6a, diff %.6a, ratio %.8g",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0x1.01c7eep+90, 0x1.01c7fp+90, 0x1.01c7f2p+90, 0);
	test(2, -12.34, 0, 0, 0x1.948046p-13, 0x1.948048p-13, 0x1.94804ap-13, 0);
	test(3, nanf(""), 0, 0, nanf(""), nanf(""), nanf(""), 0);
	test(4, strtof("inf", NULL), 0, 0, strtof("inf", NULL), strtof("inf", NULL), strtof("inf", NULL), 0);
	test(5, strtof("-inf", NULL), 0, 0, -0x1.0p-149, 0x0.0p+0, 0x1.0p-149, 0);
