/* Test whether a basic tgammaf invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) tgammaf(%.4f) = %.6a, not %.6a, diff %.6a, ratio %.8g",
...
int main(void)
{
	test(1, 9.001, 0, 0, 0x1.3bacep+15, 0x1.3bace2p+15, 0x1.3bace4p+15, 0);
	test(2, -12.34, 0, 0, -0x1.b1cc86p-29, -0x1.b1cc84p-29, -0x1.b1cc82p-29, 0);
