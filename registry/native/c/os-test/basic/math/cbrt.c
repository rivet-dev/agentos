/* Test whether a basic cbrt invocation works. */
...
void test(int variant, double input1, int errnum, int fperr, double lower, double expected, double upper, int flags)
{
	...
	double output = cbrt(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) cbrt(%.4f) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) cbrt(%.4f) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) cbrt(%.4f) = %.14a, not %.14a, diff %.14a, ratio %.16g",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0x1.1ed20dfd855b4p+2, 0x1.1ed20dfd855b5p+2, 0x1.1ed20dfd855b6p+2, 0);
	test(2, -12.34, 0, 0, -0x1.27c9ed3172f32p+1, -0x1.27c9ed3172f31p+1, -0x1.27c9ed3172f3p+1, 0);
