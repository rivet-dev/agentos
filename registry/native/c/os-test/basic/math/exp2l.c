/* Test whether a basic exp2l invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) exp2l(%.4Lf) = %.17La, not %.17La, diff %.17La, ratio %.20Lg",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0x8.0e3eb620278ddedp+87L, 0x8.0e3eb620278ddeep+87L, 0x8.0e3eb620278ddefp+87L, 0);
	test(2, -12.34, 0, 0, 0xc.a4025bcf130387ap-16L, 0xc.a4025bcf130387bp-16L, 0xc.a4025bcf130387cp-16L, 0);
	test(3, nanl(""), 0, 0, nanl(""), nanl(""), nanl(""), 0);
	test(4, strtold("inf", NULL), 0, 0, strtold("inf", NULL), strtold("inf", NULL), strtold("inf", NULL), 0);
	test(5, strtold("-inf", NULL), 0, 0, -0x0.000000000000001p-16385L, 0x0.0p+0L, 0x0.000000000000001p-16385L, 0);
