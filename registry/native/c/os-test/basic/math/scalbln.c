/* Test whether a basic scalbln invocation works. */
...
void test(int variant, double input1, long input2, int errnum, int fperr, double lower, double expected, double upper, int flags)
{
	errno = 0;
	if ( feclearexcept(FE_ALL_EXCEPT) )
		errx(1, "feclearexcept");
	double output = scalbln(input1, input2);
	if ( errnum == 0 && errno )
		err(1, "(%d.) scalbln(%.4f, %ld) failed",
                    variant, input1, input2);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) scalbln(%.4f, %ld) did not %s",
                     variant, input1, input2, strerrno(errnum));
	int excepts = fetestexcept(FE_INVALID | FE_DIVBYZERO | FE_OVERFLOW | FE_UNDERFLOW);
	if ( fperr == 0 && excepts )
		errx(1, "(%d.) scalbln(%.4f, %ld) %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) scalbln(%.4f, %ld) = %.14a, not %.14a, diff %.14a, ratio %.16g",
