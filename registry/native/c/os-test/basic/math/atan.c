/* Test whether a basic atan invocation works. */
...
void test(int variant, double input1, int errnum, int fperr, double lower, double expected, double upper, int flags)
{
	...
	double output = atan(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) atan(%.4f) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) atan(%.4f) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) atan(%.4f) = %.14a, not %.14a, diff %.14a, ratio %.16g",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0x1.8f47a42251a24p+0, 0x1.8f47a42251a25p+0, 0x1.8f47a42251a26p+0, 0);
	test(2, -12.34, 0, 0, -0x1.7d6c6dd4a40cdp+0, -0x1.7d6c6dd4a40ccp+0, -0x1.7d6c6dd4a40cbp+0, 0);
	test(3, nan(""), 0, 0, nan(""), nan(""), nan(""), 0);
	test(4, strtod("inf", NULL), 0, 0, 0x1.921fb54442d17p+0, 0x1.921fb54442d18p+0, 0x1.921fb54442d19p+0, 0);
	test(5, strtod("-inf", NULL), 0, 0, -0x1.921fb54442d19p+0, -0x1.921fb54442d18p+0, -0x1.921fb54442d17p+0, 0);
