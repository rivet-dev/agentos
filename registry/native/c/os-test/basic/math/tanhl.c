/* Test whether a basic tanhl invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) tanhl(%.4Lf) = %.17La, not %.17La, diff %.17La, ratio %.20Lg",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0xf.fffffffffffffffp-4L, 0x8.0p-3L, 0x8.000000000000001p-3L, 0);
	test(2, -12.34, 0, 0, -0xf.fffffffd5f150dap-4L, -0xf.fffffffd5f150d9p-4L, -0xf.fffffffd5f150d8p-4L, 0);
	test(3, nanl(""), 0, 0, nanl(""), nanl(""), nanl(""), 0);
	test(4, strtold("inf", NULL), 0, 0, 0xf.fffffffffffffffp-4L, 0x8.0p-3L, 0x8.000000000000001p-3L, 0);
