/* Test whether a basic coshl invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) coshl(%.4Lf) = %.17La, not %.17La, diff %.17La, ratio %.20Lg",
...
int main(void)
{
	...
	test(2, -12.34, 0, 0, 0xd.f4d7cee9207439p+13L, 0xd.f4d7cee92074391p+13L, 0xd.f4d7cee92074392p+13L, 0);
