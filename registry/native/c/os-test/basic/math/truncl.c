/* Test whether a basic truncl invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) truncl(%.4Lf) = %.17La, not %.17La, diff %.17La, ratio %.20Lg",
