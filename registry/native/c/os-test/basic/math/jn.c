/*[XSI]*/
/* Test whether a basic jn invocation works. */
...
void test(int variant, int input1, double input2, int errnum, int fperr, double lower, double expected, double upper, int flags)
{
	...
	double output = jn(input1, input2);
	if ( errnum == 0 && errno )
		err(1, "(%d.) jn(%d, %.4f) failed",
                    variant, input1, input2);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) jn(%d, %.4f) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) jn(%d, %.4f) = %.14a, not %.14a, diff %.14a, ratio %.16g",
...
int main(void)
{
	test(1, 5, 13.37, 0, 0, 0x1.79a634fb451fap-3, 0x1.79a634fb5557fp-3, 0x1.79a634fb65904p-3, 0);
	test(2, 5, -12.34, 0, 0, -0x1.3e35cf0936655p-12, -0x1.3e35cf0928ba9p-12, -0x1.3e35cf091b0fdp-12, 0);
	test(3, 5, nan(""), 0, 0, nan(""), nan(""), nan(""), 0);
	test(4, 5, strtod("inf", NULL), 0, 0, -0x0.0000000000001p-1022, 0x0.0p+0, 0x0.0000000000001p-1022, 0);
	test(5, 5, 0.0, 0, 0, -0x0.0000000000001p-1022, 0x0.0p+0, 0x0.0000000000001p-1022, 0);
	return imprecise;
}
