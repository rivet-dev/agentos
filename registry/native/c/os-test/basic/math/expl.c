/* Test whether a basic expl invocation works. */
...
void test(int variant, long double input1, int errnum, int fperr, long double lower, long double expected, long double upper, int flags)
{
	...
	long double output = expl(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) expl(%.4Lf) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) expl(%.4Lf) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) expl(%.4Lf) = %.17La, not %.17La, diff %.17La, ratio %.20Lg",
...
int main(void)
{
	...
	test(2, -12.34, 0, 0, 0x9.2be160e71b82096p-21L, 0x9.2be160e71b82097p-21L, 0x9.2be160e71b82098p-21L, 0);
