/* Test whether a basic erfcl invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) erfcl(%.4Lf) = %.17La, not %.17La, diff %.17La, ratio %.20Lg",
...
int main(void)
{
	test(1, 1.01, 0, 0, 0x9.cddb59328fb0f45p-6L, 0x9.cddb59328fb0f46p-6L, 0x9.cddb59328fb0f47p-6L, 0);
	test(2, -0.1234, 0, 0, 0x9.1bba09e66697d39p-3L, 0x9.1bba09e66697d3ap-3L, 0x9.1bba09e66697d3bp-3L, 0);
	test(3, nanl(""), 0, 0, nanl(""), nanl(""), nanl(""), 0);
	test(4, strtold("inf", NULL), 0, 0, -0x0.000000000000001p-16385L, 0x0.0p+0L, 0x0.000000000000001p-16385L, 0);
	test(5, strtold("-inf", NULL), 0, 0, 0xf.fffffffffffffffp-3L, 0x8.0p-2L, 0x8.000000000000001p-2L, 0);
