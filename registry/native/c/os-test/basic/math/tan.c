/* Test whether a basic tan invocation works. */
...
void test(int variant, double input1, int errnum, int fperr, double lower, double expected, double upper, int flags)
{
	...
	double output = tan(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) tan(%.4f) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) tan(%.4f) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) tan(%.4f) = %.14a, not %.14a, diff %.14a, ratio %.16g",
...
int main(void)
{
	test(1, 90.01, 0, 0, -0x1.f244f867bfbfcp+0, -0x1.f244f867bfbfbp+0, -0x1.f244f867bfbfap+0, 0);
	test(2, -12.34, 0, 0, 0x1.d7b11663a643ep-3, 0x1.d7b11663a643fp-3, 0x1.d7b11663a644p-3, 0);
