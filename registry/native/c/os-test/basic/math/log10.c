/* Test whether a basic log10 invocation works. */
...
void test(int variant, double input1, int errnum, int fperr, double lower, double expected, double upper, int flags)
{
	...
	double output = log10(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) log10(%.4f) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) log10(%.4f) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) log10(%.4f) = %.14a, not %.14a, diff %.14a, ratio %.16g",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0x1.f44c663c619adp+0, 0x1.f44c663c619aep+0, 0x1.f44c663c619afp+0, 0);
	test(2, -12.34, EDOM, FE_INVALID, nan(""), nan(""), nan(""), 0);
