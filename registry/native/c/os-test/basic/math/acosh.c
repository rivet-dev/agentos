/* Test whether a basic acosh invocation works. */
...
void test(int variant, double input1, int errnum, int fperr, double lower, double expected, double upper, int flags)
{
	...
	double output = acosh(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) acosh(%.4f) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) acosh(%.4f) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) acosh(%.4f) = %.14a, not %.14a, diff %.14a, ratio %.16g",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0x1.4c5ab844b2914p+2, 0x1.4c5ab844b2915p+2, 0x1.4c5ab844b2916p+2, 0);
	...
	test(6, 0.0, EDOM, FE_INVALID, nan(""), nan(""), nan(""), 0);
	return imprecise;
}
