/* Test whether a basic asinf invocation works. */
...
void test(int variant, float input1, int errnum, int fperr, float lower, float expected, float upper, int flags)
{
	...
	float output = asinf(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) asinf(%.4f) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) asinf(%.4f) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) asinf(%.4f) = %.6a, not %.6a, diff %.6a, ratio %.8g",
...
int main(void)
{
	test(1, 0.9001, 0, 0, 0x1.1eb83ep+0, 0x1.1eb84p+0, 0x1.1eb842p+0, 0);
	test(2, -0.1234, 0, 0, -0x1.fabcf4p-4, -0x1.fabcf2p-4, -0x1.fabcfp-4, 0);
