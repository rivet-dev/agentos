/* Test whether a basic llroundf invocation works. */
...
void test(int variant, float input1, int errnum, int fperr, long long expected, int flags)
{
	...
	{
		if ( output != expected )
			errx(1, "(%d.) llroundf(%.4f) = %lld, not %lld",
...
int main(void)
{
	...
	test(3, nanf(""), EDOM, FE_INVALID, 0, 0 | MF_UNSPEC1);
	test(4, strtof("inf", NULL), EDOM, FE_INVALID, 0, 0 | MF_UNSPEC1);
	test(5, strtof("-inf", NULL), EDOM, FE_INVALID, 0, 0 | MF_UNSPEC1);
