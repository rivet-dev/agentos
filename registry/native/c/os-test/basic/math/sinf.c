/* Test whether a basic sinf invocation works. */
...
void test(int variant, float input1, int errnum, int fperr, float lower, float expected, float upper, int flags)
{
	...
	float output = sinf(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) sinf(%.4f) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) sinf(%.4f) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) sinf(%.4f) = %.6a, not %.6a, diff %.6a, ratio %.8g",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0x1.c768a6p-1, 0x1.c768a8p-1, 0x1.c768aap-1, 0);
	test(2, -12.34, 0, 0, 0x1.cba846p-3, 0x1.cba848p-3, 0x1.cba84ap-3, 0);
