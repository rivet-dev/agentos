/* Test whether a basic lgamma invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) lgamma(%.4f) = %.14a, not %.14a, diff %.14a, ratio %.16g",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0x1.39b2a13f72742p+8, 0x1.39b2a13f72743p+8, 0x1.39b2a13f72744p+8, 0);
	test(2, -12.34, 0, 0, -0x1.392e8c03dff09p+4, -0x1.392e8c03dff08p+4, -0x1.392e8c03dff07p+4, 0);
	...
	test(6, 0.0, ERANGE, FE_DIVBYZERO, strtod("inf", NULL), strtod("inf", NULL), strtod("inf", NULL), 0);
	return imprecise;
}
