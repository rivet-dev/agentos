/* Test whether a basic fabsl invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) fabsl(%.4Lf) = %.17La, not %.17La, diff %.17La, ratio %.20Lg",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0xb.4051eb851eb87ffp+3L, 0xb.4051eb851eb88p+3L, 0xb.4051eb851eb8801p+3L, 0);
	test(2, -12.34, 0, 0, 0xc.570a3d70a3d6fffp+0L, 0xc.570a3d70a3d7p+0L, 0xc.570a3d70a3d7001p+0L, 0);
