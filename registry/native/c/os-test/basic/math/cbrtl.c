/* Test whether a basic cbrtl invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) cbrtl(%.4Lf) = %.17La, not %.17La, diff %.17La, ratio %.20Lg",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0x8.f6906fec2adaab2p-1L, 0x8.f6906fec2adaab3p-1L, 0x8.f6906fec2adaab4p-1L, 0);
	test(2, -12.34, 0, 0, -0x9.3e4f698b9798832p-2L, -0x9.3e4f698b9798831p-2L, -0x9.3e4f698b979883p-2L, 0);
