/* Test whether a basic ldexpl invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) ldexpl(%.4Lf, %d) = %.17La, not %.17La, diff %.17La, ratio %.20Lg",
