/* Test whether a basic sinhl invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) sinhl(%.4Lf) = %.17La, not %.17La, diff %.17La, ratio %.20Lg",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0xe.7d6d4f4fa1a3fddp+125L, 0xe.7d6d4f4fa1a3fdep+125L, 0xe.7d6d4f4fa1a3fdfp+125L, 0);
	test(2, -12.34, 0, 0, -0xd.f4d7cee6d57be0ep+13L, -0xd.f4d7cee6d57be0dp+13L, -0xd.f4d7cee6d57be0cp+13L, 0);
