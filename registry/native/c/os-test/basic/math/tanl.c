/* Test whether a basic tanl invocation works. */
...
void test(int variant, long double input1, int errnum, int fperr, long double lower, long double expected, long double upper, int flags)
{
	...
	long double output = tanl(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) tanl(%.4Lf) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) tanl(%.4Lf) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) tanl(%.4Lf) = %.17La, not %.17La, diff %.17La, ratio %.20Lg",
...
int main(void)
{
	test(1, 90.01, 0, 0, -0xf.9227c33dfdfda27p-3L, -0xf.9227c33dfdfda26p-3L, -0xf.9227c33dfdfda25p-3L, 0);
	test(2, -12.34, 0, 0, 0xe.bd88b31d321f789p-6L, 0xe.bd88b31d321f78ap-6L, 0xe.bd88b31d321f78bp-6L, 0);
