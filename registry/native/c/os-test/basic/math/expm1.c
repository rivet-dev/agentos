/* Test whether a basic expm1 invocation works. */
...
void test(int variant, double input1, int errnum, int fperr, double lower, double expected, double upper, int flags)
{
	...
	double output = expm1(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) expm1(%.4f) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) expm1(%.4f) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) expm1(%.4f) = %.14a, not %.14a, diff %.14a, ratio %.16g",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0x1.cfada9e9f4347p+129, 0x1.cfada9e9f4348p+129, 0x1.cfada9e9f4349p+129, 0);
	test(2, -12.34, 0, 0, -0x1.ffff6d41e9f1ap-1, -0x1.ffff6d41e9f19p-1, -0x1.ffff6d41e9f18p-1, 0);
	test(3, nan(""), 0, 0, nan(""), nan(""), nan(""), 0);
	test(4, strtod("inf", NULL), 0, 0, strtod("inf", NULL), strtod("inf", NULL), strtod("inf", NULL), 0);
	test(5, strtod("-inf", NULL), 0, 0, -0x1.0000000000001p+0, -0x1.0p+0, -0x1.fffffffffffffp-1, 0);
