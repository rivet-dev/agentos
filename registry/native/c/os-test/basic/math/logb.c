/* Test whether a basic logb invocation works. */
...
void test(int variant, double input1, int errnum, int fperr, double lower, double expected, double upper, int flags)
{
	...
	double output = logb(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) logb(%.4f) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) logb(%.4f) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) logb(%.4f) = %.14a, not %.14a, diff %.14a, ratio %.16g",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0x1.7ffffffffffffp+2, 0x1.8p+2, 0x1.8000000000001p+2, 0);
	test(2, -12.34, 0, 0, 0x1.7ffffffffffffp+1, 0x1.8p+1, 0x1.8000000000001p+1, 0);
	...
	test(6, 0.0, ERANGE, FE_DIVBYZERO, strtod("-inf", NULL), strtod("-inf", NULL), strtod("-inf", NULL), 0 | MF_MAYERR);
	return imprecise;
}
