/* Test whether a basic atanhf invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) atanhf(%.4f) = %.6a, not %.6a, diff %.6a, ratio %.8g",
...
int main(void)
{
	test(1, 0.9001, 0, 0, 0x1.7905ep+0, 0x1.7905e2p+0, 0x1.7905e4p+0, 0);
	test(2, -0.1234, 0, 0, -0x1.fc0924p-4, -0x1.fc0922p-4, -0x1.fc092p-4, 0);
