/* Test whether a basic nearbyintl invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) nearbyintl(%.4Lf) = %.17La, not %.17La, diff %.17La, ratio %.20Lg",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0xb.3ffffffffffffffp+3L, 0xb.4p+3L, 0xb.400000000000001p+3L, 0);
	test(2, -12.34, 0, 0, -0xc.000000000000001p+0L, -0xc.0p+0L, -0xb.fffffffffffffffp+0L, 0);
