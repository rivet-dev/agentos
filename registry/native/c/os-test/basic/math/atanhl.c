/* Test whether a basic atanhl invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) atanhl(%.4Lf) = %.17La, not %.17La, diff %.17La, ratio %.20Lg",
...
int main(void)
{
	test(1, 0.9001, 0, 0, 0xb.c82f15670bf673fp-3L, 0xb.c82f15670bf674p-3L, 0xb.c82f15670bf6741p-3L, 0);
	test(2, -0.1234, 0, 0, -0xf.e0490d7bbce3537p-7L, -0xf.e0490d7bbce3536p-7L, -0xf.e0490d7bbce3535p-7L, 0);
