/* Test whether a basic sinl invocation works. */
...
void test(int variant, long double input1, int errnum, int fperr, long double lower, long double expected, long double upper, int flags)
{
	...
	long double output = sinl(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) sinl(%.4Lf) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) sinl(%.4Lf) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) sinl(%.4Lf) = %.17La, not %.17La, diff %.17La, ratio %.20Lg",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0xe.3b4642ba4986927p-4L, 0xe.3b4642ba4986928p-4L, 0xe.3b4642ba4986929p-4L, 0);
	test(2, -12.34, 0, 0, 0xe.5d42e58f5a5c8abp-6L, 0xe.5d42e58f5a5c8acp-6L, 0xe.5d42e58f5a5c8adp-6L, 0);
