/* Test whether a basic asin invocation works. */
...
void test(int variant, double input1, int errnum, int fperr, double lower, double expected, double upper, int flags)
{
	...
	double output = asin(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) asin(%.4f) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) asin(%.4f) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) asin(%.4f) = %.14a, not %.14a, diff %.14a, ratio %.16g",
...
int main(void)
{
	test(1, 0.9001, 0, 0, 0x1.1eb840e489e81p+0, 0x1.1eb840e489e82p+0, 0x1.1eb840e489e83p+0, 0);
	test(2, -0.1234, 0, 0, -0x1.fabcf146836cbp-4, -0x1.fabcf146836cap-4, -0x1.fabcf146836c9p-4, 0);
