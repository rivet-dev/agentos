/*[XSI]*/
/* Test whether a basic y1 invocation works. */
...
void test(int variant, double input1, int errnum, int fperr, double lower, double expected, double upper, int flags)
{
	...
	double output = y1(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) y1(%.4f) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) y1(%.4f) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) y1(%.4f) = %.14a, not %.14a, diff %.14a, ratio %.16g",
...
int main(void)
{
	test(1, 90.01, 0, 0, -0x1.9fe9b81b83cc6p-6, -0x1.9fe9b81b71ef6p-6, -0x1.9fe9b81b60126p-6, 0);
	test(2, -12.34, EDOM, FE_INVALID, nan(""), nan(""), nan(""), 0 | MF_MAYERR);
	test(3, nan(""), 0, 0, nan(""), nan(""), nan(""), 0);
	test(4, strtod("inf", NULL), 0, 0, -0x0.0000000000001p-1022, 0x0.0p+0, 0x0.0000000000001p-1022, 0);
	test(5, 0.0, ERANGE, FE_DIVBYZERO, strtod("-inf", NULL), strtod("-inf", NULL), strtod("-inf", NULL), 0 | MF_MAYERR);
	return imprecise;
}
