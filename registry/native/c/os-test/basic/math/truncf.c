/* Test whether a basic truncf invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) truncf(%.4f) = %.6a, not %.6a, diff %.6a, ratio %.8g",
