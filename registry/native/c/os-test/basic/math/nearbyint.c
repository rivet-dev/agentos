/* Test whether a basic nearbyint invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) nearbyint(%.4f) = %.14a, not %.14a, diff %.14a, ratio %.16g",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0x1.67fffffffffffp+6, 0x1.68p+6, 0x1.6800000000001p+6, 0);
	test(2, -12.34, 0, 0, -0x1.8000000000001p+3, -0x1.8p+3, -0x1.7ffffffffffffp+3, 0);
