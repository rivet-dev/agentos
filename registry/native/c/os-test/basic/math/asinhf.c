/* Test whether a basic asinhf invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) asinhf(%.4f) = %.6a, not %.6a, diff %.6a, ratio %.8g",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0x1.4c5bbap+2, 0x1.4c5bbcp+2, 0x1.4c5bbep+2, 0);
	test(2, -12.34, 0, 0, -0x1.9a93a8p+1, -0x1.9a93a6p+1, -0x1.9a93a4p+1, 0);
