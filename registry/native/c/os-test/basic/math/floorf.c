/* Test whether a basic floorf invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) floorf(%.4f) = %.6a, not %.6a, diff %.6a, ratio %.8g",
...
int main(void)
{
	...
	test(2, -12.34, 0, 0, -0x1.a00002p+3, -0x1.ap+3, -0x1.9ffffep+3, 0);
