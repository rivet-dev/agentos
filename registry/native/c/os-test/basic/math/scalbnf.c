/* Test whether a basic scalbnf invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) scalbnf(%.4f, %d) = %.6a, not %.6a, diff %.6a, ratio %.8g",
