/* Test whether a basic expm1f invocation works. */
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) expm1f(%.4f) = %.6a, not %.6a, diff %.6a, ratio %.8g",
...
int main(void)
{
	test(1, 9.001, 0, 0, 0x1.fae31ap+12, 0x1.fae31cp+12, 0x1.fae31ep+12, 0);
	test(2, -12.34, 0, 0, -0x1.ffff7p-1, -0x1.ffff6ep-1, -0x1.ffff6cp-1, 0);
	test(3, nanf(""), 0, 0, nanf(""), nanf(""), nanf(""), 0);
	test(4, strtof("inf", NULL), 0, 0, strtof("inf", NULL), strtof("inf", NULL), strtof("inf", NULL), 0);
	test(5, strtof("-inf", NULL), 0, 0, -0x1.000002p+0, -0x1.0p+0, -0x1.fffffep-1, 0);
