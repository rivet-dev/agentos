/* Test whether a basic tanf invocation works. */
...
void test(int variant, float input1, int errnum, int fperr, float lower, float expected, float upper, int flags)
{
	...
	float output = tanf(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) tanf(%.4f) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) tanf(%.4f) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) tanf(%.4f) = %.6a, not %.6a, diff %.6a, ratio %.8g",
...
int main(void)
{
	test(1, 90.01, 0, 0, -0x1.f2444ep+0, -0x1.f2444cp+0, -0x1.f2444ap+0, 0);
	test(2, -12.34, 0, 0, 0x1.d7b0fep-3, 0x1.d7b1p-3, 0x1.d7b102p-3, 0);
