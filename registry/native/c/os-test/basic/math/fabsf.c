/* Test whether a basic fabsf invocation works. */
...
void test(int variant, float input1, int errnum, int fperr, float lower, float expected, float upper, int flags)
{
	...
	float output = fabsf(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) fabsf(%.4f) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) fabsf(%.4f) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) fabsf(%.4f) = %.6a, not %.6a, diff %.6a, ratio %.8g",
...
int main(void)
{
	test(1, 90.01, 0, 0, 0x1.680a3cp+6, 0x1.680a3ep+6, 0x1.680a4p+6, 0);
	test(2, -12.34, 0, 0, 0x1.8ae146p+3, 0x1.8ae148p+3, 0x1.8ae14ap+3, 0);
