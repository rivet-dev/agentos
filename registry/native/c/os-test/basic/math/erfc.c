/* Test whether a basic erfc invocation works. */
...
void test(int variant, double input1, int errnum, int fperr, double lower, double expected, double upper, int flags)
{
	...
	double output = erfc(input1);
	if ( errnum == 0 && errno )
		err(1, "(%d.) erfc(%.4f) failed",
                    variant, input1);
	if ( (math_errhandling & MATH_ERRNO) && errnum && errno != errnum )
		errx(1, "(%d.) erfc(%.4f) did not %s",
	...
	if ( !(flags & MF_UNSPEC1) )
	{
		       ...
		       output == expected) )
		{
			if ( imprecise && isfinite(output) && isfinite(expected) )
				return;
			warnx("(%d.) erfc(%.4f) = %.14a, not %.14a, diff %.14a, ratio %.16g",
...
int main(void)
{
	test(1, 1.01, 0, 0, 0x1.39bb6b2651f61p-3, 0x1.39bb6b2651f62p-3, 0x1.39bb6b2651f63p-3, 0);
	test(2, -0.1234, 0, 0, 0x1.2377413cccd2fp+0, 0x1.2377413cccd3p+0, 0x1.2377413cccd31p+0, 0);
	test(3, nan(""), 0, 0, nan(""), nan(""), nan(""), 0);
	test(4, strtod("inf", NULL), 0, 0, -0x0.0000000000001p-1022, 0x0.0p+0, 0x0.0000000000001p-1022, 0);
	test(5, strtod("-inf", NULL), 0, 0, 0x1.fffffffffffffp+0, 0x1.0p+1, 0x1.0000000000001p+1, 0);
