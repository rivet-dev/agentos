/* Test whether a basic utimensat invocation works. */
...
int main(void)
{
	...
	if ( utimensat(dirfd, basename, times, AT_SYMLINK_NOFOLLOW) < 0 )
		err(1, "utimensat");
	struct stat st;
	if ( fstat(fd, &st) < 0 )
		err(1, "fstat");
	if ( st.st_atim.tv_sec != times[0].tv_sec )
		errx(1, "utimensat did not set atim");
	if ( st.st_mtim.tv_sec != times[1].tv_sec )
		errx(1, "utimensat did not set mtim");
	return 0;
}
