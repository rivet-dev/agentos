...
const sourceDir = resolve(
	process.env.PI_RUST_SOURCE_DIR ?? join(homedir(), "misc", "pi_agent_rust"),
);
const outputDir = resolve(packageDir, "bin");
const outputBin = resolve(outputDir, "pi-rust");
const manifestPath = resolve(outputDir, "pi-rust.manifest.json");
const sqliteBuildDir = resolve(sourceDir, "target/pi-rust-sqlite");
...
try {
	...
	writeFileSync(
		stubPath,
		[
			"// Stub generated for local Agent OS pi-rust builds when the pi-mono submodule is absent.",
