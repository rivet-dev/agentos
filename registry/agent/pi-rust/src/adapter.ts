...
function createModes(session: PiRustSessionState) {
	return {
...
function createConfigOptions(session: PiRustSessionState) {
...
class RpcProcess {
	...
	constructor(
		...
		const effectiveLibraryPath =
			libraryPath ?? process.env.PI_RUST_LIBRARY_PATH;
		...
		if (this.closed || !this.child.stdin) {
			return Promise.reject(
				RequestError.internalError(
					{ stderr: this.stderr() },
					"pi-rust process is not available",
				),
			);
		}
	...
	private handleResponse(response: RpcResponse): void {
		...
		pending.reject(
			RequestError.internalError(
				...
				response.error ?? `pi-rust ${pending.command} failed`,
			),
		);
	}
...
async function materializePiRustLaunch(): Promise<{
	command: string;
	libraryPath?: string;
}> {
	const command = process.env.PI_RUST_COMMAND ?? "pi-rust";
	const libraryPath =
		process.env.PI_RUST_LIBRARY_PATH_HOST ??
		process.env.PI_RUST_LIBRARY_PATH;

	if (!isModuleOverlayPath(command) && !libraryPath) {
		return { command, libraryPath };
	}

	const tempDir = `/tmp/agent-os-pi-rust-${process.pid}`;
...
class PiRustAgent implements Agent {
	private readonly sessions = new Map<string, PiRustSessionState>();
	...
	async newSession(
		...
		const launch = await materializePiRustLaunch();
		const args = [
			"--mode",
			"rpc",
			"--no-session",
		...
		const session: PiRustSessionState = {
	...
	async setSessionConfigOption(
		...
		if (typeof params.value !== "string") {
			throw RequestError.invalidParams(
				{ value: params.value },
				"pi-rust config options must be strings",
			);
		}
		...
		if (params.configId === "model") {
			...
			throw RequestError.invalidParams(
				...
				"unsupported pi-rust config option",
			);
		}
	...
	private requireSession(sessionId: string): PiRustSessionState {
	...
	private async refreshState(session: PiRustSessionState): Promise<void> {
	...
	private handleRpcEvent(session: PiRustSessionState, event: JsonRecord): void {
		switch (event.type) {
			case "message_update": {
...
const connection = new AgentSideConnection(
	(conn: AgentSideConnection) => new PiRustAgent(conn),
	stream,
);
