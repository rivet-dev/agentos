...
| 82 | - [ ] A shared-core/native characterization sends a complete response-shaped object with `result` but no `id` and demonstrates the current loop ignores it until its operation timeout. | - [ ] Shared-core and native ACP tests prove immediate typed `invalid_state`, no incorrect `-32600` response, and complete adapter/session cleanup. | - [ ] Dedicated stacked `jj` revision; work-item row marked `done`. |
