...
Every non-`fs` scope matches by `patterns` instead of `paths`. For `network`, a pattern is a host (or `host:port`), and the operations are `fetch`, `http`, `dns`, and `listen`. Host patterns are globs: `*.example.com` matches every subdomain, `api.example.com:443` matches one port, and `*` matches every host. A pattern may also name the full connection URI, `tcp://<host>:<port>` for `fetch`, `http`, and `listen`, or `dns://<host>` for `dns`, when a rule should apply to one scheme only.
