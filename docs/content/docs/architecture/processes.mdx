---
title: "Processes"
description: "Internals of the kernel process model: the virtual process table, how spawns are serviced, stdio bridging, PTYs, and how WASM sh and coreutils map onto it."
skill: true
---
...
hosted API (`process.run`, `process.spawn`, `terminal.open`, lifecycle, the process tree), see
...
- **Holds every guest process,** not only the ones a client started explicitly. A `process.spawn` from the client, a child spawned by guest `node:child_process`, and the processes behind a shell pipeline are all table entries. This is why `process.tree` can show more than what the client launched.
...
- **Records a driver.** An entry knows which execution backend services it (for example a V8 isolate versus a WASM runtime). This is the `driver` field surfaced by `process.tree`. Drivers differ in *how* the code runs; they share the same table, the same kernel-owned stdio, and the same boundary.
...
A spawn, whether it originates from a client `spawn`/`run` call or from guest
...
2. **Permission check.** The kernel applies the VM's configured permission policy before doing anything. The policy is trusted input; the guest making the request is not.
...
5. **Start execution.** The driver begins running the program. For a one-shot `run`, output and exit status are collected for the call's result; for `spawn`, the process remains available for explicit waiting and output replay.
6. **Reap and record exit.** When the program finishes, the kernel records the exit code on the table entry and marks it exited, which is what `process.wait` resolves against and what the `process.exit` event reports.

Signals (`process.signal`) are the same shape: a request into the kernel, which
applies it to the virtualized process rather than to any host process.

## Process image replacement

Guest `execve` is distinct from the hosted one-shot `process.run` API. It
...
- **Capture vs. stream.** `process.run` returns bounded stdout and stderr after the process exits. For `process.spawn`, the hosted actor exposes `process.output` events with generation-scoped process handles and bounded `process.output.read` replay; `process.exit` signals completion. Process and terminal replay use the same sidecar-owned retention and cursor implementation, so `maxEvents`, `maxBytes`, truncation, and progress behavior are identical across Rust Core and TypeScript Core. Events are low-latency hints; replay is the authoritative recovery path.
- **Writable stdin.** `process.stdin.write` pushes bytes into the process's stdin endpoint; `process.stdin.close` closes the write side so programs that read to EOF (like `cat`) can finish. None of this touches a real pipe on the host.
...
- **A shell is a process plus a PTY.** `terminal.open` allocates a kernel PTY and starts a shell process attached to it, returning a generation-scoped terminal handle. The PTY is a virtualized terminal device, never a host `/dev/pts` entry.
- **Bidirectional terminal I/O.** `terminal.stdin.write` feeds keystrokes into the PTY master side; output is available through `terminal.output` events and `terminal.output.read` replay. This carries terminal control sequences, so full-screen TUIs behave correctly.
- **Resize is a terminal operation.** `terminal.pty.resize` updates the PTY's window size (columns and rows), which the kernel propagates to the foreground process the way a real terminal resize would, so programs relying on `TIOCGWINSZ`-style sizing redraw correctly.
...
For the full WASM execution model see [WASM VM](/agentos/docs/architecture/posix-syscalls); here is how it
...
- [Processes & Shell](/agentos/docs/processes): the client API for running and managing processes.
- [WASM VM](/agentos/docs/architecture/posix-syscalls): how WebAssembly guests get POSIX process, user, and network semantics.
- [Architecture](/agentos/docs/architecture): components, the trust boundary, and the request lifecycle.
- [Permissions](/agentos/docs/permissions): the policy the kernel checks on every spawn.
